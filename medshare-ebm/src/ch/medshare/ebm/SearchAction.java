...
public class SearchAction extends Action implements IKonsExtension, IHandler {
	...
	public void run(){
		if(textField != null){
			String search = textField.getSelectedText();
			if (search == null || search.isEmpty())
				search = textField.getWordUnderCursor();
			search = search.trim().replace("\r\n", " ");
			Hub.userCfg.set(Preferences.SEARCH, search);
		}
		else{
			Hub.userCfg.set(Preferences.SEARCH, "");
		}

		String url = new EbmLogIn().doPostLogin();
		...
		if(url == null){
			...
			if(url.contains("-1")){
				...
				SWTHelper.showError("EBM-Guidelines", "Fehlercode: -2\r\nKein bestehendes aktives Abo");
			}
			else if(url.contains("-3")){
				SWTHelper.showError("EBM-Guidelines", "Fehlercode: -3\r\nKein Benutzername oder Passwort");
			}
			else{

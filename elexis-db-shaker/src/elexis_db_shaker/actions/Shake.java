...
public class Shake implements IWorkbenchWindowActionDelegate {
	...
	public void run(IAction action) {
		if (SWTHelper
			...
			boolean zufallsnamen = false;
			Namen n = null;
			if (SWTHelper
					.askYesNo(
							"Demo Datenbank",
							"Wollen Sie Zufallsnahmen bei der Anonymisierung verwenden bzw. eine Demo-DB erstellen?")) {
				zufallsnamen = true;
				n = new Namen();
			}
			...
			for (Kontakt k : list) {
				...
				if (k.istPerson()) {
					...
					p.set(Person.SEX,
							StringTool.isFemale(vorname) ? Person.FEMALE
									: Person.MALE);
				}
			...
			SWTHelper.alert("Fertig", "Die Konversion ist beendet.");
		}

...
public class XMLExporter implements IRnOutputter {
	...
	public Document doExport(final Rechnung rechnung, final String dest,
		...
		validator.setAttribute("version_software", vi.maior() + vi.minor()
				+ vi.rev()); // 10130
		...
		invoice.setAttribute("invoice_date", new TimeTool(rn.getDatumRn()).toString(TimeTool.DATE_MYSQL)
				+ "T00:00:00"); // 10154
		...
		for (Konsultation b : lb) {
			...
			for (Verrechnet vv : lv) {
				...
				if (v == null) {
					log.log("Fehlerhafte Rechnung " + rn.getNr()
							+ " Null-Verrechenbar bei Kons " + b.getLabel(), Log.ERRORS);
					continue;
				}
				if (v instanceof TarmedLeistung) {
					...
					String scaleRaw = vv.getDetail("scale");
					double scale = 1.0;
					if (!StringTool.isNothing(scaleRaw)) {
						scale = Double.parseDouble(scaleRaw);
					}
		...
		mTotal.addMoney(mTarmedAL).addMoney(mTarmedTL).addMoney(mAnalysen).addMoney(mMedikament).addMoney(mUebrige).addMoney(mKant).addMoney(mPhysio).addMoney(mMigel);
		...
		if (StringTool.isNothing(gebDat)) { // make validator happy if we don't know the birthdate
			...
			patient.setAttribute("birthdate", new TimeTool(pat.getGeburtsdatum()).toString(TimeTool.DATE_MYSQL)
					+ "T00:00:00");
		}
		...
		if (gesetz.equalsIgnoreCase("ivg")) {
			...
			String nif = TarmedRequirements.getNIF(actMandant.getRechnungssteller()).replaceAll("[^0-9]", "");
			if (Hub.userCfg.get(Leistungscodes.BILLING_STRICT, true)
					&& (!nif.matches("[0-9]{1,7}"))) {
		...
		if (rn.getStatus() != RnStatus.FEHLERHAFT) {
			try {
				...
				ExHandler.handle(ex);
				SWTHelper.alert("Fehler", "Konnte Datei " + dest
						+ " nicht schreiben");
				return null;
			}
		}

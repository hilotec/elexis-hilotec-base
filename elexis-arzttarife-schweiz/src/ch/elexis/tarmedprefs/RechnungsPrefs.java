 ...
 *    
 * $Id: RechnungsPrefs.java 3989 2008-06-01 12:02:22Z rgw_ch $
 *******************************************************************************/
...
public class RechnungsPrefs extends PreferencePage implements
	...
	protected Control createContents(Composite parent) {
		...
		hBank.addHyperlinkListener(new HyperlinkAdapter(){
			@Override
			public void linkActivated(HyperlinkEvent e) {
				//KontaktSelektor dlg=new KontaktSelektor(getShell(),Organisation.class,"Bankverbindung auswählen","Bitte geben Sie an, auf welches Finanzinstitut\nIhre Einuahlungsscheine lauten sollen");
		...
		bUseTC.addSelectionListener(new SelectionAdapter(){
			@Override
			public void widgetSelected(SelectionEvent e) {
				actMandant.setInfoElement(PreferenceConstants.USETC, bUseTC.getSelection()==true ? "1" : "0"); //$NON-NLS-1$ //$NON-NLS-2$
			}
		...
		bUseEDA.addSelectionListener(new SelectionAdapter(){
			@Override
			public void widgetSelected(SelectionEvent e) {
				actMandant.setInfoElement(PreferenceConstants.USEEDA, bUseEDA.getSelection()==true ? "1" : "0"); //$NON-NLS-1$ //$NON-NLS-2$
			}
			
		});
		bWithImage=new Button(gTC,SWT.CHECK);
		bWithImage.setText(Messages.getString("RechnungsPrefs.ImagesToTrustCenter")); //$NON-NLS-1$
		bWithImage.addSelectionListener(new SelectionAdapter(){
			@Override
			public void widgetSelected(SelectionEvent e) {
				actMandant.setInfoElement(PreferenceConstants.TCWITHIMAGE, bWithImage.getSelection()==true ? "1" : "0"); //$NON-NLS-1$ //$NON-NLS-2$
			}
	...
	class BankLister extends TitleAreaDialog{
		final String[] flds={Messages.getString("RechnungsPrefs.department"),Messages.getString("RechnungsPrefs.POBox"), ta.ESRNUMBER, ta.ESRSUB}; //$NON-NLS-1$ //$NON-NLS-2$
		Label banklabel;
		...
		protected Control createDialogArea(Composite parent) {
			...
			Hyperlink hb=Desk.theToolkit.createHyperlink(ret,Messages.getString("RechnungsPrefs.FinanceInst"),SWT.NONE); //$NON-NLS-1$
			hb.addHyperlinkListener(new HyperlinkAdapter(){

				@Override
				public void linkActivated(HyperlinkEvent e) {
					KontaktSelektor ksl=new KontaktSelektor(getShell(),Organisation.class,Messages.getString("RechnungsPrefs.paymentinst"),Messages.getString("RechnungsPrefs.PleseChooseBank")); //$NON-NLS-1$ //$NON-NLS-2$
	...
	private void setFontsTextValues() {
		String normalFontName = Hub.localCfg.get(ESR.ESR_NORMAL_FONT_NAME, ESR.ESR_NORMAL_FONT_NAME_DEFAULT);
		int normalFontSize = Hub.localCfg.get(ESR.ESR_NORMAL_FONT_SIZE, ESR.ESR_NORMAL_FONT_SIZE_DEFAULT);
		...
		int ocrFontWeight = Hub.localCfg.get(ESR.ESR_OCR_FONT_WEIGHT, ESR.ESR_OCR_FONT_WEIGHT_DEFAULT);
		int index;

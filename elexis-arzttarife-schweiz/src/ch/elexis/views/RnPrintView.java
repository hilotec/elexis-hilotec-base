 ...
 *    
 * $Id: RnPrintView.java 5970 2010-01-27 16:43:04Z rgw_ch $
 *******************************************************************************/
...
public class RnPrintView extends ViewPart {
	...
	/*
	 * public static final int LinesFirstPage=15; public static final int
	 * LinesMiddlePage=25; public static final int LinesLastPage=22;
	 */
	...
	CTabItem addItem(final String template, final String title,
		...
		Brief actBrief = text.createFromTemplateName(null, template,
				Brief.RECHNUNG, adressat, Messages.RnPrintView_tarmedBill);
	...
	public void useItem(final int idx, final String template,
			final Kontakt adressat) {
		...
		if (template != null) {
			// don't associate a Konsultation
			Brief actBrief = text.createFromTemplateName(null, template,
					Brief.RECHNUNG, adressat, betreff);
	...
	public boolean doPrint(final Rechnung rn, final IRnOutputter.TYPE rnType,
			final boolean withESR, final boolean withForms,
		...
		ElexisEventDispatcher.fireSelectionEvent(rs);
		...
		Money mEZDue = XMLTool.xmlDoubleToMoney(balance
				.getAttributeValue("amount_obligations"));
		if (withESR == true) {
			CTabItem ctEZ;
			TextContainer text;
			...
			sb.append(Messages.RnPrintView_otherpoints).append(sOther).append(
					"\n");
			/*
			 * sb.append(Messages.RnPrintView_tarmedPoints).append(xmlex.mTarmed.
			 * getAmountAsString()).append("\n"); //$NON-NLS-2$
			 * sb.append(Messages
			 * .RnPrintView_medicaments).append(xmlex.mMedikament
			 * .getAmountAsString()).append("\n"); //$NON-NLS-2$
			 * sb.append(Messages
			 * .RnPrintView_labpoints).append(xmlex.mAnalysen.getAmountAsString
			 * ()).append("\n"); //$NON-NLS-2$
			 * sb.append(Messages.RnPrintView_migelpoints
			 * ).append(xmlex.mMigel.getAmountAsString()).append("\n");
			 * //$NON-NLS-2$
			 * sb.append(Messages.RnPrintView_physiopoints).append(
			 * xmlex.mPhysio.getAmountAsString()).append("\n"); //$NON-NLS-2$
			 * sb.
			 * append(Messages.RnPrintView_otherpoints).append(xmlex.mUebrige.
			 * getAmountAsString()).append("\n"); //$NON-NLS-2$
			 */
			for (Zahlung z : extra) {
		...
		if (--existing < 0) {
			ctP1 = addItem(
					"Tarmedrechnung_S1", Messages.RnPrintView_page1, adressat); //$NON-NLS-1$
		} else {
			ctP1 = ctab.getItem(1);
	...
	private TextContainer insertPage(final int page, final Kontakt adressat,
		...
		if (--existing < 0) {
			ctF = addItem(
					"Tarmedrechnung_S2", Messages.RnPrintView_page + page, adressat); //$NON-NLS-1$
		} else {
	...
	private void replaceHeaderFields(final TextContainer text, final Rechnung rn) {
		...
		if (fall.getAbrechnungsSystem().equals("IV")) { //$NON-NLS-1$
			String nif = TarmedRequirements.getNIF(m);
			text.replace("\\[NIF\\]", nif); //$NON-NLS-1$
			text.replace("\\[F60]\\", fall.getVersNummer()); //$NON-NLS-1$
		} else {
			String ksk = TarmedRequirements.getKSK(m);
			text.replace("\\[NIF\\]", ksk); //$NON-NLS-1$
	...
	/*
	 * public void save() { if(actBrief!=null){

...
// @requirement FUNC.PER/0.2
	 ...
	 * @deprecated Clients could use the getOOoConnection to obtain an OfficeConnection and use it
	 *             as argument in a constructor for another OOoBean instance. Calling the dispose
	 *             method of the OfficeConnection or the OOoBean's stopOOoConnection method would
	 *             make all instances of OOoBean stop working.
	 */
	 ...
	 */
	public void startOOoConnection(String aConnectionURL) throws java.net.MalformedURLException,
		HasConnectionException, NoConnectionException{
	 ...
	 * 
	 * @deprecated This method is not useful in a multithreaded environment. Then all threads
	 *             accessing the instance would have to be synchronized in order to make is method
	 *             work. It is better to call OOoBean's methods and be prepared to catch a
	 ...
	 * 
	 * When the OOoBean is displayed in an applet by a web browser, then this method must be called
	 * from within java.applet.Applet.stop.
	 */
	 ...
	 * 
	 * If no connection exists, a default connection will be created. An OfficeConnection can be
	 * used to register listeners of type com.sun.star.lang.EventListener, which are notified when
	 * the connection to the office dies. One should not call the dispose method, because this may
	 * result in receiving com.sun.star.lang.DisposedExceptions when calling
	 * {@link #stopOOoConnection stopOOoConnection} or other API methods. If other instances share
	 * the same connection then they will stop function properly, because they loose their
	 * connection as well. The recommended way to end the connection is calling
	 * {@link #stopOOoConnection stopOOoConnection}.
	 * 
	 * @throws NoConnectionException
	...
	public synchronized com.sun.star.lang.XMultiServiceFactory getMultiServiceFactory()
		...
		if (xServiceFactory == null) {
			...
			try {
				aConnectorThread.join(nOOoStartTimeOut);
	...
	public synchronized com.sun.star.frame.XDesktop getOOoDesktop() throws NoConnectionException{
		if (xDesktop == null) {
			try {
				...
				xDesktop =
					(com.sun.star.frame.XDesktop) UnoRuntime.queryInterface(
						com.sun.star.frame.XDesktop.class, aObject);
			} catch (com.sun.star.uno.Exception aExc) {} // TBD: what if no connection exists?
		}
	 ...
	 * 
	 * If a document is loaded and the content modified, the changes are dismissed. Otherwise
	 * nothing happens.
	 * 
	 * This method is intended to be overridden in derived classes. This implementation simply calls
	 * clear.
	 * 
	 * @param bClearStateToo
	 *            Not only the document content but also the state of the bean, like visibility of
	 *            child components is cleared.
	 * @deprecated There is currently no way to dismiss changes, except for loading of the unchanged
	 *             initial document. Furthermore it is unclear how derived classes handle this and
	 *             what exactly their state is (e.g. what members make up their state). Calling this
	 *             method on a derived class requires knowledge about their implementation.
	 *             Therefore a deriving class should declare their own clearDocument if needed.
	 *             Clients should call the clearDocument of the deriving class or {@link #clear}
	 *             which discards the currently displayed document.
	 */
	public synchronized void clearDocument(boolean bClearStateToo)
		throws com.sun.star.util.CloseVetoException, NoConnectionException{
		// TBD
		clear();
	}
	 ...
	 * 
	 * Any loaded document is unloaded, no matter whether it is modified or not. After calling this
	 * method, the OOoBean has no office document and no frame anymore. The connection will stay,
	 * though.
	 * 
	...
	public synchronized void clear(){
		...
		try {
			CallWatchThread aCallWatchThread = new CallWatchThread(nOOoCallTimeOut, "clear");
			...
			if (xFrameWindow != null) {
				try {
					releaseSystemWindow();
				} catch (NoConnectionException aExc) {} // ignore
				catch (SystemWindowException aExc) {} // ignore
	 ...
	 * 
	 * If no document is loaded and the instance is added to a Java container that is showing, then
	 * this method needs not to be called. If later one of the methods {@link #loadFromURL
	 * loadFromURL}, {@link #loadFromStream loadFromStream1}, or {@link #loadFromByteArray
	 * loadFromByteArray} is called, then the document is automatically displayed.
	 * 
	 * Should one of the load methods have been called before the Java container was showing, then
	 * this method needs to be called after the container window was made visible
	 * (java.lang.Component.setVisible(true)).
	 * <p>
	 * Another scenario is that a OOoBean contains a document and is removed from a Java container
	 * and later added again. Then aquireSystemWindow needs to be called after the container window
	 * is displayed.
	 * 
	 ...
	 *             if the connection is not established.
	 */
	public synchronized void aquireSystemWindow() throws SystemWindowException,
	...
	// @estimation 16h
	/**
	 * This method must be called when the OOoBean before the sytem window may be released by it's
	 * parent AWT/Swing component.
	 * 
	 * This is the case when java.awt.Component.isDisplayable() returns true. This is definitely the
	 * case when the OOoBean is removed from it's parent container.
	 * 
	 * @throws SystemWindowException
	 *             if system window is not aquired.
	 * @throws NoConnectionException
	 *             if the connection is not established.
	 * @deprecated When Component.removeNotify of the parent window of the actual office window is
	 *             called, then the actions are performed for which this method needed to be called
	 *             previously.
	 */
	public synchronized void releaseSystemWindow() throws SystemWindowException,
	 ...
	 *             if the currently displayed document cannot be closed because it is still be used,
	 *             for example it is printed.
	 */
	public void loadFromURL(final String aURL, final com.sun.star.beans.PropertyValue aArguments[])
		throws
		// @requirement FUNC.CON.LOST/0.2
		NoConnectionException, java.io.IOException, com.sun.star.lang.IllegalArgumentException,
		...
		try {
			...
			while (!bLoaded) {
				...
				try {
					...
					if (aFrame == null) {
						...
						aFrame =
							new Frame((com.sun.star.frame.XFrame) UnoRuntime.queryInterface(
						...
						com.sun.star.frame.XFrames xFrames =
							((com.sun.star.frame.XFramesSupplier) UnoRuntime.queryInterface(
								com.sun.star.frame.XFramesSupplier.class, getOOoDesktop()))
								.getFrames();
					...
					xURLTransformer =
						(com.sun.star.util.XURLTransformer) UnoRuntime.queryInterface(
							com.sun.star.util.XURLTransformer.class, xServiceFactory
					...
					if (xLoader == null) {
						throw new java.lang.RuntimeException("com.sun.star.frame.Frame(" + aFrame
					...
					com.sun.star.beans.PropertyValue aArgs[] =
						addArgument(aArguments, new com.sun.star.beans.PropertyValue(
							"MacroExecutionMode", -1, new Short(
					...
					com.sun.star.lang.XComponent xComponent =
						xLoader
							.loadComponentFromURL(aURL, /* aFrame.getName() */"_self", 0, aArgs);
					...
					aDocument =
						new OfficeDocument((com.sun.star.frame.XModel) UnoRuntime.queryInterface(
			...
			System.out.println("skip apply");
			/*
			 * this deprecated method seems to hang in ooo2.1 so we just take it out G. Weirich
			 * 29.1.2007 applyToolVisibilities();
			 */
		} catch (java.lang.InterruptedException aExc) {
			throw new NoConnectionException();
		}
	 ...
	 */
	public void loadFromStream(final java.io.InputStream iInStream,
		...
		com.sun.star.beans.PropertyValue[] aExtendedArguments =
			addArgument(aArguments, new com.sun.star.beans.PropertyValue("InputStream", -1,
				xStream, com.sun.star.beans.PropertyState.DIRECT_VALUE));
	 ...
	 * See loadFromURL() for further information.
	 */
	public void loadFromByteArray(final byte aInBuffer[],
		final com.sun.star.beans.PropertyValue aArguments[])
		throws
		// @requirement FUNC.CON.LOST/0.2
		NoConnectionException, java.io.IOException, com.sun.star.lang.IllegalArgumentException,
		...
		com.sun.star.io.XInputStream xStream =
			new com.sun.star.lib.uno.adapter.ByteArrayToXInputStreamAdapter(aInBuffer);
	 ...
	 * <p>
	 * Due due a bug (50651) calling this method may cause the office to crash, when at the same
	 * time the office writes a backup of the document. This bug also affects
	 * {@link #storeToByteArray storeToByteArray} and {@link #storeToStream storeToStream}. The
	 * workaround is to start the office with the option -norestore, which disables the automatic
	 * backup and recovery mechanism. OOoBean offers currently no supported way of providing startup
	 * options for OOo. But it is possible to set a Java property when starting Java, which is
	 * examined by OOoBean:
	 * 
	 * <pre>
	 ...
	 * </pre>
	 * 
	 * It is planned to offer a way of specifying startup options in a future version. The property
	 * can be used until then. When using this property only one option can be provided.
	 * 
	 * @throws IllegalArgumentException
	 *             if either of the arguments is out of the specified range.
	 * @throws java.io.IOException
	 ...
	 */
	public void storeToURL(final String aURL, final com.sun.star.beans.PropertyValue aArguments[])
		throws
		// @requirement FUNC.CON.LOST/0.2
		NoConnectionException, java.io.IOException, com.sun.star.lang.IllegalArgumentException,
		NoDocumentException{
		...
		try {
			...
			CallWatchThread aCallWatchThread = new CallWatchThread(nOOoCallTimeOut, "storeToURL");
	...
	public java.io.OutputStream storeToStream(java.io.OutputStream aOutStream,
		...
		throws
		// @requirement FUNC.CON.LOST/0.2
		NoConnectionException, NoDocumentException, java.io.IOException,
		com.sun.star.lang.IllegalArgumentException

	{
		// wrap Java stream into UNO stream
		com.sun.star.lib.uno.adapter.OutputStreamToXOutputStreamAdapter aStream =
			new com.sun.star.lib.uno.adapter.OutputStreamToXOutputStreamAdapter(aOutStream);
		
		// add stream to arguments
		com.sun.star.beans.PropertyValue[] aExtendedArguments =
			addArgument(aArguments, new com.sun.star.beans.PropertyValue("OutputStream", -1,
				aStream, com.sun.star.beans.PropertyState.DIRECT_VALUE));
		...
		try {
			aStream.closeOutput();
		} catch (com.sun.star.io.NotConnectedException aExc) { /* TDB */} catch (com.sun.star.io.BufferSizeExceededException aExc) { /* TDB */} catch (com.sun.star.io.IOException aExc) {
			throw new java.io.IOException();
		}
		return aOutStream;
	}
	...
	public byte[] storeToByteArray(byte aOutBuffer[],
		...
		com.sun.star.lib.uno.adapter.XOutputStreamToByteArrayAdapter aStream =
			new com.sun.star.lib.uno.adapter.XOutputStreamToByteArrayAdapter(aOutBuffer);
	 ...
	 * 
	 * @returns a Java class which implements all interfaces which the service <type
	 *          scope="com::sun::star::frame">Frame</a> implements. Thus, methods can be called
	 *          directly without queryInterface. This feature might be implemented by UNO or
	 *          explicitely coded.
	 * @throws NoConnectionException
	 *             if the connection is not established.
	...
	throws NoConnectionException // @requirement FUNC.CON.LOST/0.2
	{
		if (iConnection == null)
			throw new NoConnectionException();
		return aFrame;
	 ...
	 * 
	 * @returns a Java class which implements all interfaces which the service <type
	 *          scope="com::sun::star::frame">Controller</a> implements. Thus, methods can be called
	 *          directly without queryInterface. This feature might be implemented by UNO or
	 *          explicitely coded.
	 * @throws NoConnectionException
	 *             if the connection is not established.
	 ...
	 * 
	 * @returns a Java class which implements all interfaces which the service <type
	 *          scope="com::sun::star::document">OfficeDocument</a> implements. Thus, methods can be
	 *          called directly without queryInterface. This feature might be implemented by UNO or
	 *          explicitely coded.
	 * @throws NoConnectionException
	 *             if the connection is not established.
	 */
	 ...
	 * 
	 * Initially all tool bars are visible. By hiding all tool bars utilizing this method, it is
	 * possible to turn just a subset of tool bars on afterwards, no matter whether all available
	 * tool bars are known or not.
	 * <p>
	 * If an older OOoBean instance is used with a newer OOo instance, some tool bars might not be
	 * affected by this method.
	 * <p>
	 * If no connection is established or no document is loaded, the setting is memorized until a
	 * document is loaded. Same is valid when the connection dies within this function call.
	 * 
	 * @deprecated Clients should use the service com.sun.star.frame.LayoutManager, which can be
	 *             obtained from a frame, to control toolbars. For example:
	 * 
	 *             <pre>
	 ...
	 * 
	 * @deprecated Clients should use the service com.sun.star.frame.LayoutManager, which can be
	 *             obtained from a frame, to control toolbars. See also {@link #setAllBarsVisible
	 *             setAllBarsVisible}.
	 */
	protected void applyToolVisibilities() throws java.lang.InterruptedException{
		bIgnoreVisibility = true;
	 ...
	 * 
	 * @param bnewValue
	 *            If false, the tool bar is disabled, If true, the tool bar is visible.
	 * @deprecated Clients should use the service com.sun.star.frame.LayoutManager, which can be
	 *             obtained from a frame, to control toolbars. See also {@link #setAllBarsVisible}.
	 */
	protected boolean setToolVisible(String aProperty, String aResourceURL, boolean bOldValue,
		boolean bNewValue)

	throws java.lang.InterruptedException{
		// start runtime timeout
		CallWatchThread aCallWatchThread = new CallWatchThread(nOOoCallTimeOut, "setToolVisible");
	 ...
	 * <p>
	 * If not connected or no document loaded, the value is stored and automatically applied to the
	 * document after it is loaded. Same is valid when the connection dies within this function
	 * call.
	 * 
	 * @param bVisible
	...
	public void setMenuBarVisible(boolean bVisible){
		try {
			bMenuBarVisible =
				setToolVisible("MenuBarVisible", "private:resource/menubar/menubar",
	 ...
	 * 
	 * This method works independently from a connetion or loaded document. If no connection is
	 * established or no document is loaded, this method just returns a memorized status.
	 * 
	 ...
	 * 
	 * @param bVisible
	 *            If false, the main function bar is disabled, If true, the main function bar is
	 *            visible.
	 * @deprecated Clients should use the service com.sun.star.frame.LayoutManager, which can be
	 *             obtained from a frame, to control toolbars. See also {@link #setAllBarsVisible}.
	 */
	public void setStandardBarVisible(boolean bVisible){
		try {
			bStandardBarVisible =
				setToolVisible("StandardBarVisible", "private:resource/toolbar/standardbar",
	 ...
	 * 
	 * @param bVisible
	 *            If false, the tool function bar is disabled, If true, the tool function bar is
	 *            visible.
	 * @deprecated Clients should use the service com.sun.star.frame.LayoutManager, which can be
	 *             obtained from a frame, to control toolbars. See also {@link #setAllBarsVisible}.
	 */
	public void setToolBarVisible(boolean bVisible){
		try {
			bToolBarVisible =
				setToolVisible("ToolBarVisible", "private:resource/toolbar/toolbar",
	 ...
	 * 
	 * @param bVisible
	 *            If false, the status function bar is disabled, If true, the status function bar is
	 *            visible.
	 * @deprecated Clients should use the service com.sun.star.frame.LayoutManager, which can be
	 *             obtained from a frame, to control toolbars. See also {@link #setAllBarsVisible}.
	 */
	public void setStatusBarVisible(boolean bVisible){
		try {
			bStatusBarVisible =
				setToolVisible("StatusBarVisible", "private:resource/statusbar/statusbar",
	 ...
	 * established or no document is loaded, this method just returns a memorized status.
	 * 
	 * @return True if the status function bar is visible, false if the status function bar is
	 *         hidden.
	 * @deprecated Clients should use the service com.sun.star.frame.LayoutManager, which can be
	 *             obtained from a frame, to control toolbars. See also {@link #setAllBarsVisible}.
	 */
	 ...
	 * 
	 * If the argument by its name is already in aArguments it is exchanged and aArguments is
	 * returned.
	 * <p>
	 * If the argument by its name is not yet in aArguments, a new array is created, aArgument added
	 * and the new array returned.
	 */
	 ...
	 * @internal
	 */
	private class EventListener extends Thread implements com.sun.star.lang.XEventListener,
			com.sun.star.frame.XTerminateListener {
		String aTag;
		
		EventListener(String aTag) throws NoConnectionException{
			// init members
			this.aTag = aTag;
		...
		public void run(){
			...
			long n = 0;
			while (isInterrupted() == false && iConnection != null

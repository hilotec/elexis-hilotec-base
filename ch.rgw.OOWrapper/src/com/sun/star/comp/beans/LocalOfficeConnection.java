 ...
 *  $Revision: 1.4 $
 *
 *  last change: $Author: rgw_ch $ $Date: 2006/01/16 21:34:41 $
 *
...
public class LocalOfficeConnection implements OfficeConnection {
	...
	/**
	 * Constructor. Sets up paths to the office application and native libraries if values are
	...
	public void setUnoUrl(String url) throws java.net.MalformedURLException{
		...
		if (url.startsWith(prefix))
			...
			try {
				...
				throw new java.net.MalformedURLException("Invalid UNO connection URL.");
			}
		}
		mURL = url;
	}
	...
	/**
	 * Retrives the UNO component context. Establishes a connection if necessary and initialises the
	 * UNO service manager if it has not already been initialised. This method can return
	...
	/**
	 * Creates an office window. The window is either a sub-class of java.awt.Canvas (local) or
	...
	public void dispose(){
		...
		if (mBridge != null) {
			XComponent comp = (XComponent) UnoRuntime.queryInterface(XComponent.class, mBridge);
			if (comp != null)
				comp.dispose();
			else
		...
		try {
			...
			if (null != aInitialObject) {
				XPropertySet xPropertySet =
					(XPropertySet) UnoRuntime.queryInterface(XPropertySet.class, aInitialObject);
	...
	/**
	 * Retrives a path to the office program folder. G. Weirich: Adapted to take path from system
	 * property (to run from eclipse)
	 * 
	 * @return The path to the office program folder.
	 */
	static private String getProgramPath(){
		if (mProgramPath == null) {
			mProgramPath = System.getProperty("openoffice.path.name");
			/*
			 * // determine name of executable soffice String aExec = OFFICE_APP_NAME; // default
			 * for UNIX String aOS = System.getProperty("os.name");
			 * 
			 * // running on Windows? if (aOS.startsWith("Windows")) aExec = OFFICE_APP_NAME +
			 * ".exe";
			 * 
			 * // add other non-UNIX operating systems here // ...
			 * 
			 * // find soffice executable relative to this class's class loader: File path =
			 * NativeLibraryLoader.getResource( LocalOfficeConnection.class.getClassLoader(),
			 * aExec); if (path != null) mProgramPath = path.getParent();
			 * 
			 * // default is "" if ( mProgramPath == null ) mProgramPath = "";
			 */
		}
		return mProgramPath;
	}
	...
	private class OfficeService implements NativeService {
		...
		public void startupService() throws java.io.IOException{
			...
			if (mConnType.equals("pipe"))
				cmdArray[3] =
					"-accept=pipe,name=" + getIdentifier() + ";" + mProtocol + ";" + mInitialObject;
	...
	class StreamProcessor extends Thread {
		...
		public void run(){
			java.io.BufferedReader r =
				new java.io.BufferedReader(new java.io.InputStreamReader(m_in));
			try {
				for (;;) {
					String s = r.readLine();
					if (s == null) {

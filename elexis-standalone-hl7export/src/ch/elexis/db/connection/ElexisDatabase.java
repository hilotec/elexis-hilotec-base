...
public class ElexisDatabase extends AbstractConnection {
	...
	public Patient readPatient(final String id) throws SQLException {
		...
		List<DBRow> elexisPatList =
			select(MessageFormat
				.format(
					"SELECT * FROM KONTAKT WHERE ID = ''{0}''", //$NON-NLS-1$
					id));
		...
		if (elexisPatList.size() > 0) {
			...
			String patId = getIdForPatient(row.getString(PATIENT_FLD_ID), row.getString(PATIENT_FLD_PATNR));
			if (patId == null) {
				Logger
	...
	public Laboritem readLaborItem(final String id) throws SQLException {
		...
		List<DBRow> elexisLabItemList =
			select(MessageFormat
				.format(
					"SELECT * FROM LABORITEMS WHERE ID = ''{0}''", //$NON-NLS-1$
					id));
	 ...
	 * @throws IOException
	 */
	public List<Laborwert> readLaborWerte(final String matchingExportTag) throws SQLException, IOException{
		// Patienten suchen
		List<DBRow> elexisPatList =
		...
		for (DBRow row : elexisPatList) {
			...
			if (matchingExportTag != null && exportTag.toLowerCase().contains(matchingExportTag.toLowerCase())) {

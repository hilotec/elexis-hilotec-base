...
public abstract class AbstractConnection {
			...
			if (value != null) {
				...
				if (value.toString().trim().length() > 0) {
					try {
						...
						Logger.logError(Messages.getString("AbstractConnection.errorParseInt") + value, e); //$NON-NLS-1$
					}
				}
			}
			return null;
			...
			if (value != null) {
				...
				if (value.toString().trim().length() > 0) {
					try {
						...
						Logger.logError(Messages.getString("AbstractConnection.errorParseLong") + value, e); //$NON-NLS-1$
					}
				}
			}
			return null;
			...
			if (value != null) {
				...
				if (value.toString().trim().length() > 0) {
					try {
						...
						Logger.logError(Messages.getString("AbstractConnection.errorParseDate") + value, e); //$NON-NLS-1$
					}
				}
			}
			return null;
	 ...
	 * @throws SQLException 
	 */
	protected void openConnection(final String driverName, final String url,
			final String user, final String pwd) throws ClassNotFoundException, SQLException {

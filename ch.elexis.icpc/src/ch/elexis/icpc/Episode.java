...
public class Episode extends PersistentObject implements Comparable<Episode> {
	...
	private final static String createDB = "CREATE TABLE "
			+ TABLENAME
			+ " ("
			+ "ID				VARCHAR(25),"
			+ "lastupdate		BIGINT,"
			...
			// any text
			"Number         VARCHAR(10),"
			+ // number for individual, possibly hierarchical,
				// organization
			...
			"INSERT INTO " + TABLENAME + " (ID,Title) VALUES ('1',"
	...
	private final static String createLink = "CREATE TABLE " + LINKNAME + " ("
			+ "ID				VARCHAR(25)," + "lastupdate 	BIGINT,"
			+ "deleted		char(1) default '0'," + "Episode		VARCHAR(25),"
			+ "Diagnosis		VARCHAR(80)" + ");";

	private static final String upd041 = "ALTER TABLE " + TABLENAME
			+ " add lastupdate BIGINT;" + "ALTER TABLE " + LINKNAME
			+ " ADD lastupdate BIGINT;";

	private static final String upd042 = "CREATE INDEX " + TABLENAME + "2 ON "
			+ TABLENAME + " (Title);";

	static {
		addMapping(TABLENAME, FLD_PATIENT_ID, FLD_TITLE, FLD_START_DATE,
				FLD_NUMBER, FLD_STATUS, FLD_EXTINFO,
	...
	public Episode(final Patient pat, final String title) {
		create(null);
		set(new String[] { FLD_PATIENT_ID, FLD_TITLE }, pat.getId(), title);
	}
	...
	public String getLabel() {
		...
		/*
		 * if (!StringTool.isNothing(startDate)) { sb.append(startDate);
		 * sb.append(": "); }
		 */
	...
	public static Episode findEpisode(final String name) {
		List<Episode> res = new Query<Episode>(Episode.class, FLD_TITLE, name)
				.execute();
		if (res.size() == 1) {
			return res.get(0);
		}

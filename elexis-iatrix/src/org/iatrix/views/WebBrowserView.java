...
public class WebBrowserView extends ViewPart implements ISaveablePart2 {
	...
	private void makeActions(){
		newViewAction = new Action("Neues Fenster"){
			...
			public void run() {
				try {
					getViewSite().getPage().showView(ID, StringTool.unique(ID), IWorkbenchPage.VIEW_ACTIVATE);
		...
		editLocationAction = new Action("Adresse ändern") {
			...
			public void run() {
				...
				InputDialog dialog = new InputDialog(getSite().getShell(), "Adresse ändern", "Bitte geben Sie die neue Adresse ein", current, null);
		...
		editTitleAction = new Action("Titel ändern") {
			...
			public void run() {
				...
				InputDialog dialog = new InputDialog(getSite().getShell(), "Titel ändern", "Bitte geben Sie den neuen Titel ein", current, null);
				if (dialog.open() == InputDialog.OK) {

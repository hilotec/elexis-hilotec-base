...
public class TestAnalytica {
	...
	public void testAnalyticaSetup() throws Exception{
		...
		bot
			.menu(ch.elexis.uitests.Messages.getString("TestCore.File")).menu(ch.elexis.uitests.Messages.getString("TestCore.Preferences")).click(); //$NON-NLS-1$ //$NON-NLS-2$
		...
		SWTBotTreeItem treeItem =
			pBot.tree()
				.getTreeItem(ch.elexis.uitests.Messages.getString("TestCore.Datenaustausch"))
				.select().expand();

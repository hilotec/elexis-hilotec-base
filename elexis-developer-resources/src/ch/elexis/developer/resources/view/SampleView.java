 ...
 * 
 *    $Id: SampleView.java 6116 2010-02-11 21:40:19Z rgw_ch $
 *******************************************************************************/
...
/**
 * This is a sample view to demonstrate how to connect to elexis's event
 * scheduler and how to display elexis's data types. We implement
 * IActivationListener to be informed, wenn the user can see our View. All UI
 * funktions should only be active in that case.
 * 
 * @author gerry
 * 
 */
...
public class SampleView extends ViewPart implements IActivationListener {
	...
	/**
	 * CommonViewer is a "golden hammer". Use it for fast prototyping. In many
	 * cases you probably want to develop more spezialiced viewers.
	 */
	CommonViewer cv;

	/**
	 * A Common Viewer is always controlled by a ViewerConfigurer. So you'll
	 * never have only one of them
	 */
	ViewerConfigurer vc;
	...
	ElexisEventListenerImpl eeli_pat = new ElexisEventListenerImpl(
			Patient.class) {
		/**
		 * Wen can override runInUi (which runs in the UI thread) or
		 * catchElexisEvent which runs in a non-UI thread. In any case the
		 * method must return quickly. For lenghty operations, start a seperate
		 * thread.
		 */
	...
	/**
	 * This is a default implemetation of an ElexisEventListener. It listens for
	 * several types of events on Instances of the SampleDataType class.
	 */
	ElexisEventListenerImpl eeli_sdt = new ElexisEventListenerImpl(
			SampleDataType.class, ElexisEvent.EVENT_SELECTED
	...
	/**
	 * As the Elexis User changes, he or she has propably different righs to see
	 * and modify our data, than the previous user. So we catch the USER_CHANGED
	...
	public void createPartControl(Composite parent) {
		makeActions();
		cv = new CommonViewer();
		Query<SampleDataType> qbe = new Query<SampleDataType>(
		...
		fdl.addQueryFilter(new QueryFilter() {
			public void apply(Query<? extends PersistentObject> qbe) {
				...
				if (Hub.acl.request(ACLContributor.ReadSDT)) {
					...
					if (pat != null) {
						qbe.add(SampleDataType.FLD_PATIENT_ID, Query.EQUALS,
								pat.getId());
					}
				} else {
		...
		vc = new ViewerConfigurer(fdl, new SampleDataLabelProvider(),
	...
	/**
	 * From IActivationListener: The View changes visibility Our listeners need
	 * only to be active,if gthe view is visible. So we untregister them if the
	 * view disappears to save resources. If the view becomes visible again, we
	 * must send an event, because the listeners don't know what happend, before
	 * the start listening
	 * 
	 * @param mode
	 *            true: the view becomes visible. false: the view becomes
	 *            invisible
	 */
	public void visible(boolean mode) {
		if (mode) {
			eeli_pat.catchElexisEvent(ElexisEvent.createPatientEvent());
			eeli_sdt.catchElexisEvent(new ElexisEvent(null,
					SampleDataType.class, ElexisEvent.EVENT_RELOAD));
	...
	private void makeActions() {
		/**
		 * The Create Action (the green button on the upper right side of the
		 * view) can be pressed only if the user has the right to create SDT's
		 */
		newSDTAction = new RestrictedAction(ACLContributor.CreateSDT,
				Messages.SampleView_newSampleDataType) {
		 ...
		 */
		deleteSDTAction = new RestrictedAction(ACLContributor.DeleteSDT,
				Messages.SampleView_deleteItem) {

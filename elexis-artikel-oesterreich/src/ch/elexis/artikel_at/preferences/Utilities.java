...
public class Utilities {
	...
	public static void cleanPrescriptionTable() {
		...
		for (Iterator<Prescription> iterator = presList.iterator(); iterator
			...
			if (!validPerson && !validArticle && !validRecipe) {
				invalidRecipes++;
				Hub.log.log("Lösche Veschreibung "+prescription.getId()+". Weder gültiger Kontakt, Artikel noch Rezept gefunden.", Log.TRACE);
		...
		for (Iterator<Prescription> iterator = presList.iterator(); iterator
			...
			if(PhZNr!="" && PhZNr!=null) {						
				...
				for (Artikel artikel : artList) {
					...
					noOfUpdates++;
					pen.println(prescription.getId()+":[OK]:"+PhZNr+" Update to "+newest.getName()+" from "+newest.get(Artikel.FLD_LASTUPDATE));
					
					
				} catch(IndexOutOfBoundsException e) {
					pen.println(prescription.getId()+":[ERR]:"+PhZNr+" No Change - Kein passendes Medikament gefunden. artList.size():"+artList.size());
				}
			} else {
				pen.println(prescription.getId()+":[ERR]:"+"No Change - Referenzierter Artikel hat keine Pharma-ZentralNr.");
			}
		}
	...
	/**
	 * Dieses Skript sucht nach Medikamenten mit der gleichen Pharma-Zentralnummer. Von den gegebenen Einträgen
	 * wird der neueste gewählt, dieser wird nicht gelöscht. Von den älteren Einträgen wird überprüft ob ein
	 * Link auf diese vorhanden ist, falls nein, werden sie gelöscht.
	 * 
	 * TODO: BlackBox Medikamente können sofern keine Verschreibungen mehr darauf referenzieren auch gelöscht werden!
	 */
	public static void cleanMedikamente() {
				...
				if(mediListN.size()==1) {
					...
					for(Medikament medidel: mediListN) {
						if(!resultIDMap.contains(medidel)) { 
							...
							pen.println("Cant delete "+medidel.getLabel()+" from "+medidel.getLastUpdate()+"reference found!");
						}
					}
		...
		// Lösche alle Artikel vom Typ Vidal, die nicht mehr einer Verschreibung zugeordnet sind (Alter Datensatz)

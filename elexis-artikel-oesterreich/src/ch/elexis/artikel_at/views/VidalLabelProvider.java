...
public class VidalLabelProvider extends DefaultLabelProvider implements ITableColorProvider{
	public VidalLabelProvider(){
		if(Desk.theImageRegistry.get("VidalRed")==null){
			ImageDescriptor imd=getImageDescriptor("rsc/redbox.ico");
			if(imd==null){
				Hub.log.log("No red icon found", Log.ERRORS);
			}
			Desk.theImageRegistry.put("VidalRed", getImageDescriptor("rsc/redbox.ico"));
		}
		if(Desk.theImageRegistry.get("VidalGreen")==null){
			Desk.theImageRegistry.put("VidalGreen", getImageDescriptor("rsc/greenbox.ico"));
		}
		if(Desk.theImageRegistry.get("VidalYellow")==null){
			Desk.theImageRegistry.put("VidalYellow", getImageDescriptor("rsc/yellowbox.ico"));
		}
	}
		...
		if(element instanceof Medikament){
			...
			if(box!=null){
				if(box.startsWith("R")){
					...
					Image img= Desk.theImageRegistry.get("VidalRed");
					if(img==null){
						Hub.log.log("Image is null", Log.ERRORS);
					}
					//Hub.log.log(img.toString(), Log.INFOS);
				}else if(box.startsWith("G")){
					return Desk.theImageRegistry.get("VidalGreen");
				}else if(box.startsWith("Y")){
					return Desk.theImageRegistry.get("VidalYellow");
				}
			}else{
	...
	public String getColumnText(Object element, int columnIndex) {
		if(element instanceof Medikament){
			...
			if(art.isLagerartikel()){
				ret.append("(")
					.append(Integer.toString(art.getTotalCount())).append(")");
			}
			return ret.toString();
		}

...
{	
	...
	private void initialize() {
		...
		if(list.size()<1){
			myLab=new Labor(Messages.getString("Value.LabKuerzel"),Messages.getString("Value.LabName"));
		}else{
			myLab=list.get(0);
		}
		...
		if(itemList.size()<1){
			_labItem = new LabItem(_shortName,_longName,myLab,_refMann,_refFrau, _unit ,LabItem.typ.NUMERIC,Messages.getString("Value.LabName"),"50");
		}else{

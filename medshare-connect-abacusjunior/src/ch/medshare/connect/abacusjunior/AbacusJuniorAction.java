...
public class AbacusJuniorAction extends Action implements ComPortListener {
	...
	public AbacusJuniorAction(){
		...
		_ctrl =
			new Connection(Messages.getString("AbacusJuniorAction.ConnectionName"), Hub.localCfg
				.get(Preferences.PORT, Messages.getString("AbacusJuniorAction.DefaultPort")),
				Hub.localCfg.get(Preferences.PARAMS, Messages
					.getString("AbacusJuniorAction.DefaultParams")), this);
		...
		if (isChecked()) {
			KontaktSelektor ksl =
				new KontaktSelektor(Hub.getActiveShell(), Patient.class, Messages
					.getString("AbacusJuniorAction.Patient.Title"), Messages
					.getString("AbacusJuniorAction.Patient.Text"), Patient.DEFAULT_SORT);
			ksl.create();
	...
	public void gotBreak(final Connection connection){
		...
		_log.log("Break");
		_log.logEnd();
		SWTHelper.showError(Messages.getString("AbacusJuniorAction.RS232.Break.Title"), Messages
			.getString("AbacusJuniorAction.RS232.Break.Text"));
	}
	...
	public void timeout(){
		_ctrl.close();
		_log.log("Timeout");
		SWTHelper.showError(Messages.getString("AbacusJuniorAction.RS232.Timeout.Title"), Messages
			.getString("AbacusJuniorAction.RS232.Timeout.Text"));
		setChecked(false);
		_log.logEnd();
	}
}

...
public class AbacusJuniorAction extends Action implements ComPortListener {
	...
	public  AbacusJuniorAction(){
		...
		if (Hub.localCfg.get(Preferences.LOG, "n").equalsIgnoreCase("y"))
		{
			...
			{
				SWTHelper.showError(Messages.getString("AbacusJuniorAction.LogError.Title"), Messages.getString("AbacusJuniorAction.LogError.Text"));
				_log = new Logger();			
			}
		}
		else
	...
	public void run(){
		if(isChecked()){
			KontaktSelektor ksl=new KontaktSelektor(Hub.getActiveShell(),Patient.class,Messages.getString("AbacusJuniorAction.Patient.Title"),Messages.getString("AbacusJuniorAction.Patient.Text"));
			ksl.create();
			...
			if(ksl.open()==org.eclipse.jface.dialogs.Dialog.OK){
				...
				if(_ctrl.connect()){
					...
					_log.log("Error");	
					SWTHelper.showError(Messages.getString("AbacusJuniorAction.RS232.Error.Title"), Messages.getString("AbacusJuniorAction.RS232.Error.Text"));
				}
			}
		}else{
			if(_ctrl.isOpen()) {
	...
	public void timeout() {
		_ctrl.close();
		_log.log("Timeout");
		SWTHelper.showError(Messages.getString("AbacusJuniorAction.RS232.Timeout.Title"), Messages.getString("AbacusJuniorAction.RS232.Timeout.Text"));
		setChecked(false);
		_log.logEnd();
	}
}

...
public class AbacusJuniorAction extends Action implements ComPortListener {
	...
	public AbacusJuniorAction() {
		...
		_ctrl = new Connection(Messages
				.getString("AbacusJuniorAction.ConnectionName"), Hub.localCfg
				.get(Preferences.PORT, Messages
						.getString("AbacusJuniorAction.DefaultPort")),
		...
		if (Hub.localCfg.get(Preferences.LOG, "n").equalsIgnoreCase("y")) {
			try {
				...
				SWTHelper.showError(Messages
						.getString("AbacusJuniorAction.LogError.Title"),
			...
			if (ksl.open() == org.eclipse.jface.dialogs.Dialog.OK) {
				...
				if (_ctrl.connect()) {
					...
					SWTHelper
							.showError(
									Messages
											.getString("AbacusJuniorAction.RS232.Error.Title"),
									Messages
											.getString("AbacusJuniorAction.RS232.Error.Text"));
				}
			}
		} else {
			if (_ctrl.isOpen()) {
	...
	public void gotBreak(final Connection connection) {
		...
		_log.logEnd();
		SWTHelper.showError(Messages
				.getString("AbacusJuniorAction.RS232.Break.Title"), Messages
	...
	public void timeout() {
		...
		_log.log("Timeout");
		SWTHelper.showError(Messages
				.getString("AbacusJuniorAction.RS232.Timeout.Title"), Messages

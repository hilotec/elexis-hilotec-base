...
public class Importer extends ImporterPage {
	...
	private Result<String> parse(final HL7 hl7, final Kontakt labor,
		...
		return new Result<String>(ch.elexis.laborimport.analytica.Messages
	...
	private Result<?> importFile(final File file, final File archiveDir) {
		HL7 hl7 = new HL7(ch.elexis.laborimport.analytica.Messages
	...
	private Result<?> importDirectFtp() {
		...
		String downloadDir = FileUtility.getCorrectPath(Hub.globalCfg.get(
		...
		try {
			...
			try {
				...
				ftp
						.addSemaphore(downloadDir, PRAXIS_SEMAPHORE,
								LABOR_SEMAPHORE);
			...
			result = new Result<String>(SEVERITY.ERROR, 1, e.getMessage(),
					MY_LAB, true);
			ResultAdapter.displayResult(result,
					ch.elexis.laborimport.analytica.Messages
							.getString("Importer.error.import")); //$NON-NLS-1$
		} catch (FtpSemaException e) {
			result = new Result<String>(SEVERITY.WARNING, 1, e.getMessage(),
					MY_LAB, true);
			ResultAdapter.displayResult(result,
					ch.elexis.laborimport.analytica.Messages
							.getString("Importer.error.import")); //$NON-NLS-1$
	...
	public String getDescription() {
		return ch.elexis.laborimport.analytica.Messages
	...
	public String getTitle() {
		return ch.elexis.laborimport.analytica.Messages
				.getString("Importer.lab") + MY_LAB; //$NON-NLS-1$
	}
	...
	private class LabImporter extends Composite {
		...
		public LabImporter(final Composite parent, final ImporterPage home) {
			...
			String batchOrFtp = Hub.globalCfg.get(PreferencePage.BATCH_OR_FTP,
					PreferencePage.FTP);
			String direktHerkunft = ch.elexis.laborimport.analytica.Messages
					.getString("Importer.ftp.label"); //$NON-NLS-1$
			if (PreferencePage.BATCH.equals(batchOrFtp)) {
				direktHerkunft = ch.elexis.laborimport.analytica.Messages
						.getString("Importer.batch.label"); //$NON-NLS-1$
			}
			bDirect = new Button(this, SWT.RADIO);
			bDirect
					.setText(ch.elexis.laborimport.analytica.Messages

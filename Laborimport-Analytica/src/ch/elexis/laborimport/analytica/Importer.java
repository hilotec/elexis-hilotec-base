...
public class Importer extends ImporterPage {
	...
	private Result<String> parse(final HL7 hl7, final Kontakt labor, final Patient pat){
		...
		while (obr != null) {
			...
			while (obx != null) {
				...
				if (qr.execute().size() != 0) {
					if (SWTHelper.askYesNo(ch.elexis.laborimport.analytica.Messages
		...
		return new Result<String>(ch.elexis.laborimport.analytica.Messages.getString("Importer.ok")); //$NON-NLS-1$
	}
	...
	private Result<?> importFile(final File file, final File archiveDir){
		HL7 hl7 =
			new HL7(
				ch.elexis.laborimport.analytica.Messages.getString("Importer.lab") + MY_LAB, MY_LAB); //$NON-NLS-1$
	...
	private Result<?> importDirectFtp(){
		...
		String downloadDir =
			FileUtility.getCorrectPath(Hub.globalCfg.get(PreferencePage.DL_DIR,

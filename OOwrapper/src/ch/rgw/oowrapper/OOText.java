...
public class OOText implements ITextPlugin{
	 ...
	 */
	public void dispose() {
		// Ausserst unbefriedigend. Wieso  bleiben manchmal diese Textleichen zurück?
		if(bean!=null){
			try {
				//XDesktop desktop=bean.getOOoDesktop();	
				bean.getDocument().setModified(false);
				bean.clear();
				// bean.stopOOoConnection();  // Dies scheint zu akkumulation von Docs zu führen
				bean=null;
				/*
				if(desktop!=null){
					desktop.terminate();		// Ich weiss nicht, ob das notwendig ist
				}*/
			} catch (Throwable e) {
				ExHandler.handle(e);
			}
	...
	private class Interceptor implements XDispatchProviderInterceptor{
		...
		public XDispatch[] queryDispatches(DispatchDescriptor[] arg0) {
			...
			for(DispatchDescriptor d0:arg0){
				System.out.println(d0.FeatureURL.Complete);
			}*/
	...
	public boolean findOrReplace(String pattern, ReplaceCallback cb){
		...
		try{
			XReplaceable xReplaceable = (XReplaceable) UnoRuntime.queryInterface(XReplaceable.class, bean.getDocument());
	 ...
	 * @param properties Eigenschafen der Tabelle, mit | verknüpft
	 * @param marke Ein Text als Platzhalter innerhalb des Texts. Die Tabelle wird anstelle 
	 * @param columnSizes ein Array mit der relativen Breite der Spalten (muss zusammen 100 ergeben)
	 * dieses Platzhalters gesetzt
	 * @param values Inhalt der Tabelle
	...
	public boolean insertTable(String marke, int properties, String[][] values, int[] columnSizes){
        ...
        //		  XPropertySet.class,cell);
          ...
          if(columnSizes!=null){
        	  ...
        	  TableColumnSeparator[] sep=(TableColumnSeparator[])xTableProps.getPropertyValue("TableColumnSeparators"); // */new TableColumnSeparator[columnSizes.length];
        	  int percent=sum/100;
        	  int total=0;
	...
	*/
		...
		try {
			...
			for(PropertyValue p:props){
				if(p.Name.equals("PaperFormat")){
					...
					switch(pf.getValue()){
					case PaperFormat.A4_value: return PageFormat.A4;
					case PaperFormat.A5_value: return PageFormat.A5;
					default: return PageFormat.USER;
					}
				}
			}
		} catch (NoConnectionException e) {
	...
	public void setFormat(ch.elexis.text.ITextPlugin.PageFormat f) {
		...
		try {
			...
			for(PropertyValue p:props){
				if(p.Name.equals("PaperFormat")){
					switch (f){
					case A4: p.Value=PaperFormat.A4; break;
					case A5: p.Value=PaperFormat.A5; break;
					default: p.Value=PaperFormat.USER;
					}
				}
			}
		} catch (NoConnectionException e) {
	...
	public Object insertTextAt(int x, int y, int w, int h, String text, int adjust){
	    	    ...
	    	    try
	    {
	      ...
	      switch (adjust) {
			case SWT.LEFT:
				paradj=ParagraphAdjust.LEFT; break;
			case SWT.RIGHT:
				paradj=ParagraphAdjust.RIGHT; break;
			default:
	...
	public Object insertTextAt(int x, int y, int w, int h, String text,
		...
		try {
			...
			Object textShape = documentFactory
					.createInstance("com.sun.star.drawing.TextShape");

			XShape xShape = (XShape) UnoRuntime.queryInterface(
					XShape.class, textShape);
			
			// insert TextShape into document
			XDrawPageSupplier xDrawPageSupplier = (XDrawPageSupplier) UnoRuntime
					.queryInterface(XDrawPageSupplier.class, myDoc);
			XShapes xShapes = (XShapes) UnoRuntime
					.queryInterface(XShapes.class, xDrawPageSupplier.getDrawPage());
			xShapes.add(xShape);
	...
	private com.sun.star.beans.XPropertySet setFormat(XTextCursor xtc) throws UnknownPropertyException, PropertyVetoException, IllegalArgumentException, WrappedTargetException {
		com.sun.star.beans.XPropertySet charProps = (com.sun.star.beans.XPropertySet)
	      ...
	      if(font!=null){
	    	  ...
	    	  switch(stil){
	    	  case SWT.MIN: 	charProps.setPropertyValue("CharWeight",15f /*FontWeight.ULTRALIGHT*/); break;
	    	  case SWT.NORMAL:	charProps.setPropertyValue("CharWeight",FontWeight.LIGHT); break;

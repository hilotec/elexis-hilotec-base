 ...
 *    
 *  $Id: EditEpisodeDialog.java 5970 2010-01-27 16:43:04Z rgw_ch $
 *******************************************************************************/
...
public class EditEpisodeDialog extends TitleAreaDialog {
	 ...
	 *            the parent shell
	 * @param episode
	 *            the episode to edit, or null if a new episode should be
	 *            created
	 */
	...
	protected void okPressed() {
		...
		if (episode == null) {
			...
			if (actPatient != null) {
				...
				episode.set(new String[] { "StartDate", "Number" },
						new String[] { startDate, number });
			...
			episode.set(new String[] { "Title", "StartDate", "Number" },
					new String[] { title, startDate, number });
			episode.setStatus(status);
			ElexisEventDispatcher.update(episode);
		}

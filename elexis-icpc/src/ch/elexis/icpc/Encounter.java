...
public class Encounter extends PersistentObject {
	...
	private static final String createDB =
		"CREATE TABLE " + TABLENAME + " (" + "ID				VARCHAR(25)," + "lastupdate 	BIGINT,"
			+ "deleted		CHAR(1) default '0'," + "KONS			VARCHAR(25)," + "EPISODE		VARCHAR(25),"
			+ "RFE			CHAR(4)," + "DIAG			CHAR(4)," + "PROC			CHAR(4)," + "ExtInfo		BLOB);"
			+ "CREATE INDEX " + TABLENAME + "1 ON " + TABLENAME + " (KONS);" + "CREATE INDEX "
			+ TABLENAME + "2 ON " + TABLENAME + " (EPISODE);" + "INSERT INTO " + TABLENAME
			+ " (ID,KONS) VALUES ('1'," + JdbcLink.wrap(VERSION) + ");";
	...
	public String getLabel(){
		...
		sb.append(getKons().getDatum()).append(": ").append(get("RFE")).append(", ").append(
			get("Diag")).append(", ").append(get("Proc"));
		return sb.toString();
	}

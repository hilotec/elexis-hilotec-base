...
public class Synchronizer{
	...
	/**
	 * Einen neuen Synchronizer erstellen
		Wenn unter agenda/sync eine gültige Verbindung besteht, und enable auf true ist,
	...
	public Synchronizer(BaseAgendaView dayView){
		...
		if(Hub.globalCfg.get(PreferenceConstants.AG_SYNC_ENABLED, false)==true){
			...
			String connect=Hub.globalCfg.get(PreferenceConstants.AG_SYNC_CONNECTOR, "jdbc:hsqldb:"+base+"/db"); //$NON-NLS-1$ //$NON-NLS-2$
        	...
        	if(sync!=null){
        		if(!sync.connect(Hub.globalCfg.get(PreferenceConstants.AG_SYNC_DBUSER,"sa"),  //$NON-NLS-1$
        			...
        			Activator.log.log(Messages.Synchronizer_connctNotSuccessful+sync.lastErrorString, Log.WARNINGS); 
        			sync=null;
        		}else{
	...
	/**
	 * Synchronisation durchführen. Falls eine Verbidnung zu einer externen JavaAgenda besteht, werden
	 * 1. Neue Daten der externen Agenda hierher repliziert
	 * 2. Neue Daten der hiesigen Agenda auf die externe repliziert
	 *
		...
		if(sync!=null){
			...
			try{
				...
				while(res.next()){
					...
					int otherLastSeconds=stmOther.queryInt("SELECT lastedit FROM agnTermine WHERE ID='"+res.getString("ID")+"'"); //$NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$
					int otherLastMinutes=otherLastSeconds/60;
					if(otherLastSeconds==-1){				// Termin existiert remote noch nicht -> erstellen
						ps=psInsert;

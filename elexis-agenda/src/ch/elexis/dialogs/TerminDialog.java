...
public class TerminDialog extends TitleAreaDialog {
	...
	protected Control createDialogArea(final Composite parent) {
		...
		/*
		bPrev=new Button(topRight,SWT.PUSH);
		bPrev.setText("<--"); //$NON-NLS-1$
		bPrev.setToolTipText(Messages.TerminDialog_earlier); 
		bNext=new Button(topRight,SWT.PUSH);
		bNext.setText("-->"); //$NON-NLS-1$
		...
		hl.addHyperlinkListener(new HyperlinkAdapter(){
			...
			public void linkActivated(final HyperlinkEvent e) {
				...
				if(inp.open()==Dialog.OK){
					...
					actPatient=null;
					//actTermin=null;
					actPlannable=new Termin.Free(actDate.toString(TimeTool.DATE_COMPACT),
							tiVon.getTimeAsMinutes(),niDauer.getValue());
	...
	private void enable(final boolean mode){
		if(mode){
			...
			slider.setBackground(Desk.theColorRegistry.get("hellgrau")); //$NON-NLS-1$
		}else{
			bChange.setEnabled(false);
			bSave.setEnabled(false);
			getButton(IDialogConstants.OK_ID).setEnabled(false);
			slider.setBackground(Desk.theColorRegistry.get("dunkelgrau")); //$NON-NLS-1$
		}	
	}
...
/*	
	...
	*/
	...
	private void createTermin(final boolean bMulti){
		...
		if(actPlannable instanceof Termin.Free){
			actTermin=new Termin(actBereich,actDate.toString(TimeTool.DATE_COMPACT),von,bis,typ,status);
		}else{
			...
			actTermin.set(new String[]{"BeiWem","Tag","Beginn","Dauer","Typ","Status"}, 
					new String[]{actBereich,actDate.toString(TimeTool.DATE_COMPACT),

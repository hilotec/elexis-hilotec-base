 ...
 *    
 *  $Id: NOAText.java 3672 2008-02-13 21:18:05Z rgw_ch $
 *******************************************************************************/
...
public class NOAText implements ITextPlugin {
	...
	/**
	 * Create an empty text document. We simply use an empty template 
	 * and save it immediately into a temporary file to 
	 * avoid OOo's complaints when we close the Container or overwrite its contents.
	 */
	...
	/**
	 * Store the contents of the OOo-Frame into a byte array. We save it into a temporary disk file first to
	 *  ensure OOo, that the file ist really saved. That way OOo will not complain about corrupted or lost files.
	 */
	...
	public boolean insertTable(final String place, final int properties,
		...
		if(!searchResult.isEmpty()) {
			...
			try{
				ITextTable textTable = doc.getTextTableService().constructTextTable(contents.length+offset, contents[0].length);				
				doc.getTextService().getTextContentService().insertTextContent(r,textTable);
				r.setText("");
				ITextTablePropertyStore props=textTable.getPropertyStore();
				long w=props.getWidth();
	...
	/**
	 * Insert Text and return a cursor describing the position 

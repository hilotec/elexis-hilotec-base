...
/*
 * Change by G.Weirich: Inserted the method getDocument()
 */
...
/**
 * The office panel can be used in order to integrate
...
public class OfficePanel extends Composite {
	...
	/**
	 * We need access to the IDocument inside the frame
	 * @return the Document
	 * @author Gerry
	 */
  ...
  //----------------------------------------------------------------------------
	/**
	 * Returns current office frame. Returns null
	 * if an office frame is not available.
	 * 
	 * @return current office frame or null
	 * if an office frame is not available
	 * 
	 * @author Andreas Br�ker
	 * @date 28.06.2006
  ...
  //----------------------------------------------------------------------------
	/**
	 * Sets information whether a new frame should be builded for 
	 * new loaded documents. The default value is <code>false</code>.
	 * 
	 * @param buildAlwaysNewFrames information whether a new frame should be builded for 
	 * new loaded documents
	 * 
	 * @author Andreas Br�ker
	 * @date 28.06.2006
	 */
  ...
  //----------------------------------------------------------------------------
	/**
	 * Sets information whether a progress indicator should
	 * be shown during document loading. The default value is <code>true</code>.
	 * 
	 * @param showProgressIndicator information whether a progress indicator should
	 * be shown during document loading
	 * 
	 * @author Andreas Br�ker
	 * @date 28.06.2006
	 */
  ...
  //----------------------------------------------------------------------------
	 ...
	 * 
	 * @param fork information whether the loading should be done in an
	 * own thread
	 * @param documentPath path of the document to be loaded
	...
	public final void loadDocument(boolean fork, final String documentPath, final IDocumentDescriptor documentDescriptor) {
		...
		if(documentPath != null && (currentDocumentPath == null || !currentDocumentPath.equals(documentPath))) {		
			try {
				...
				if(!fork) {
					...
					loadingThread = new Thread() {						
						...
						public void run() {
							...
							asyncProgressMonitorWrapper = new AsyncProgressMonitorWrapper(getProgressMonitor(), getDisplay());
							try {
								loadDocument(documentPath, documentDescriptor, asyncProgressMonitorWrapper);
								if(document != null) 
									lastLoadingStatus = Status.OK_STATUS;
								display.asyncExec(new Runnable() {
									public void run() {
				...
				lastLoadingStatus = new Status(IStatus.ERROR, NOAUIPlugin.PLUGIN_ID, IStatus.ERROR, 
  ...
  //----------------------------------------------------------------------------
	/**
	 * Returns status of the last document loading. Returns null
	 * if a status is not available.
	 * 
	 * @return status of the last document loading or null
	 * if a status is not available
	 * 
	 * @author Andreas Br�ker
	 * @date 28.06.2006
  ...
  //----------------------------------------------------------------------------
	/**
	 * Is called after a document loading operation was done. This method
	 * can be overwriten by subclasses in order to do some work 
	 * after a document loading operation was done. 
	 * 
	 * @author Andreas Br�ker
	 * @date 28.06.2006
	 */
  ...
  //----------------------------------------------------------------------------
	/**
	 * Returns progress monitor. Subclasses can
	 * overwrite this method in order to provide their
	 * own progress monitor.
	 * 
  ...
  //----------------------------------------------------------------------------
	/**
	 * Builds progress indicator. Subclasses can overwrite this
	 * method in order to provide their own progress indicator.
	 * 
	 * @param parent parent to be used
	 * 
	 * @author Andreas Br�ker
    ...
    linkCancel.setText("<a>" + "Abbruch" + "</a>"); //$NON-NLS-1$ //$NON-NLS-3$
    progressMonitorPart.attachToCancelComponent(linkCancel);
	}
  //----------------------------------------------------------------------------
	/**
	 * Shows progress indicator. Subclasses can overwrite this method
	 * in order to show their own progress indicator. 
	 * 
	 * @author Andreas Br�ker
	 * @date 28.06.2006
	 */
  ...
  //----------------------------------------------------------------------------
	/**
	 * Hides the progress indicator. Subclasses can overwrite this method
	 * in order to hide their own progress indicator. 
	 * 
	 * @author Andreas Br�ker
	 * @date 28.06.2006
	 */
  ...
  //----------------------------------------------------------------------------	
	 ...
	 * Loads document.
	 * 
	 * @param documentPath document path to be used
	 * @param documentDescriptor document descriptor to be used
	 * @param progressMonitor progress monitor to be used
	 * 
	 * @throws CoreException if the document can not be loaded
	 * 
	 * @author Andreas Br�ker
	 * @date 28.06.2006
	 */
	private void loadDocument(String documentPath, IDocumentDescriptor documentDescriptor, IProgressMonitor progressMonitor) throws CoreException {
		...
		try {
			LoadDocumentOperation loadDocumentOperation = new LoadDocumentOperation(null, officeApplication, 
			  		officeFrame, url, documentDescriptor);
			...
			throw new CoreException(new Status(IStatus.ERROR, NOAUIPlugin.PLUGIN_ID, IStatus.ERROR, 
					invocationTargetException.getCause().getMessage(), invocationTargetException.getCause()));
		}
  ...
  //----------------------------------------------------------------------------	
	 ...
	 * 
	 * @throws CoreException if a new office frame can not be activated
	 * 
	 * @author Andreas Br�ker
	 * @date 28.06.2006
	 */

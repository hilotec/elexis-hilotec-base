...
public class AfinionAS100Action extends Action implements ComPortListener {
	...
	private void initConnection() {
		...
		_ctrl = new AfinionConnectionOld(Messages.getString("AfinionAS100Action.ConnectionName"), //$NON-NLS-1$
		Hub.localCfg.get(Preferences.PORT, Messages.getString("AfinionAS100Action.DefaultPort")), Hub.localCfg.get( //$NON-NLS-1$
	...
	public void run() {
		if (isChecked()) {
			...
			if (msg == null) {
				String timeoutStr = Hub.localCfg.get(Preferences.TIMEOUT, Messages.getString("AfinionAS100Action.DefaultTimeout")); //$NON-NLS-1$
				int timeout = 20;
				try {
				...
				_ctrl.awaitFrame(Desk.getTopShell(), Messages.getString("AfinionAS100Action.WaitMsg"), 1, 4, 0, timeout, background); //$NON-NLS-1$
				return;
			} else {
				_log.log("Error"); //$NON-NLS-1$
				SWTHelper.showError(Messages.getString("AfinionAS100Action.RS232.Error.Title"), msg); //$NON-NLS-1$
			}
		} else {
			if (_ctrl.isOpen()) {
				_ctrl.sendBreak();
	...
	private void processRecord(final Record record) {
		Desk.getDisplay().syncExec(new Runnable() {
			...
			public void run() {
				...
				String vorname = null;
				String name = null;
				String patientStr = Messages.getString("AfinionAS100Action.UnknownPatientHeaderString") + record.getId() + ")"; //$NON-NLS-1$ //$NON-NLS-2$
				...
				if (record.getId() != null) {
					...
					if (patId != null) {
						...
						if (parts.length > 1) {
							...
							if (parts[1].length() > 1) {
								vorname = parts[1].substring(0, 1).toUpperCase()
										+ parts[1].substring(1);
							}
					...
					if (patientList.size() == 1) {
						...
						patientStr = Messages.getString("AfinionAS100Action.PatientHeaderString") + probePat.getName() + ", " + probePat.getVorname() + " (" //$NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$
								+ record.getId() + ")"; //$NON-NLS-1$
					}
				}

				String text = MessageFormat.format(Messages.getString("AfinionAS100Action.ValueInfoMsg"), patientStr, record.getRunNr(), record //$NON-NLS-1$
				.getText());

				boolean ok = MessageDialog.openConfirm(Desk.getTopShell(), Messages.getString("AfinionAS100Action.DeviceName"), text); //$NON-NLS-1$
				if (ok) {
					...
					if (showSelectionDialog) {
						Desk.getDisplay().syncExec(new Runnable() {
							public void run() {
								...
								KontaktSelektor ksl = new KontaktSelektor(Hub.getActiveShell(), Patient.class, Messages.getString("AfinionAS100Action.Patient.Title"), //$NON-NLS-1$
								Messages.getString("AfinionAS100Action.Patient.Text")); //$NON-NLS-1$
								ksl.create();
								ksl.getShell().setText(Messages.getString("AfinionAS100Action.Patient.Title")); //$NON-NLS-1$
					...
					if (selectedPatient != null) {
						try {
							...
							SWTHelper.showError(Messages.getString("AfinionAS100Action.ProbeError.Title"), e //$NON-NLS-1$
							.getMessage());
						}
					} else {
	...
	public void gotData(final AbstractConnection connection, final byte[] data) {
		...
		if (validRecords == 10) { // Read next 10 records
			...
			if (lastRecord != null) {
				...
				SWTHelper.showInfo(Messages.getString("AfinionAS100Action.DeviceName"), Messages.getString("AfinionAS100Action.NoValuesMsg")); //$NON-NLS-2$
			}

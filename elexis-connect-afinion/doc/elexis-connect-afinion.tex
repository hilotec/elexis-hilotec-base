...
Hier stellen Sie den seriellen Port und die Schnittstellenparameter ein. Die Werte m\"ussen mit den Einstellungen auf dem Afinion Ger\"at \"ubereinstimmen (siehe oben). Der Wert Timeout (Sek) bestimmt, wie lange Elexis maximal auf Resultate warten soll, bevor die Verbindung getrennt wird. Die Option Logging verwenden Sie bitte nur auf Anweisung des Supports, ansonsten wird Ihr Computer mit unnötigen Daten gefülllt. Wichtig: Nach dem \"Andern dieser Parameter m\"ussen Sie Elexis neu starten.
\begin{figure}[h]
...
\\
\textbf{Wichtig:}\\
Es werden nur Werte an Elexis \"ubertragen, welche innerhalb der letzten Stunde gemessen wurden. \"Altere Messwerte m\"ussen Sie manuell in Elexis eingeben.
\begin{figure}[h]
...
Damit eine automatische Zuweisung des Patienten m\"oglich wird, muss auf dem Afinion der Patient der Messung zugeordnet werden (siehe auch 'Afinion Funktion Patienten-ID'). Auf dem Afinion kann die Patienten-ID numerisch oder als Text (Handy-Tastatur) eingegeben werden. Wenn Sie den Patientennamen eingeben, m\"ussen Sie zwingend folgendes Eingabeformat einhalten:\\
\\
...
Es kann vorkommen, dass das Ger\"at nicht den tats\"achlich zuletzt gemessenen Wert \"ubertr\"agt (Kontrollieren Sie dies bei JEDER Messung). Wenn dieses Problem auftritt, starten Sie sowohl Elexis, wie auch das Afinion Ger\"at neu. Wenn der Effekt anh\"alt, enth\"alt das Ger\"at korrupte Daten. Warten Sie in diesem Fall 1 Stunde bis Sie die n\"achste Messung durchf\"uhren (vorher Afinion neu starten). 
\section{Plattformen}

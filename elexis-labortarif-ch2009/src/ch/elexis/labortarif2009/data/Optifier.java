...
public class Optifier implements IOptifier {
	...
	public Result<IVerrechenbar> add(IVerrechenbar code, Konsultation kons) {
		if (code instanceof Labor2009Tarif) {
			...
			if(res.isOK()){
				...
				return new Result<IVerrechenbar>(res.getSeverity(),res.getCode(),res.toString(),code,true);
			}
		}
		return new Result<IVerrechenbar>(SEVERITY.ERROR, 2, "No Lab2009Tariff",
				null, true);
	}
	...
	public Result<Object> optify(Konsultation kons) {
		try {
			...
			if (date.isBefore(new TimeTool("01.07.2009"))) {
				return new Result<Object>(SEVERITY.WARNING, 3,
						"Code not yet valid", null, false);
			}
			...
			for (Verrechnet v : list) {
				...
				if (iv instanceof Labor2009Tarif) {
					...
					if (cc.equals("4708.00")) { // Übergangszuschlag
						...
						if (vlt.get(Labor2009Tarif.FLD_FACHBEREICH)
								.indexOf("C") > -1) {
							z470710+=v.getZahl();
						} else {
							z470720+=v.getZahl();
			...
			ExHandler.handle(ex);
			return new Result<Object>(SEVERITY.ERROR, 1,
					"Tariff not installed correctly", null, true);
	...
	public Result<Verrechnet> remove(Verrechnet code, Konsultation kons) {
		...
		if(res.isOK()){
			...
			return new Result<Verrechnet>(res.getSeverity(),res.getCode(),res.toString(),code,true);
		}
	}

	private Verrechnet doCreate(Konsultation kons, String code)
			throws Exception {
		String z = new Query<Labor2009Tarif>(Labor2009Tarif.class).findSingle(
				Labor2009Tarif.FLD_CODE, Query.EQUALS, code);
		if (z != null) {

		...
		if (specs != null) {
			...
			if (exw.load(results[0], langdef)) {
				...
				for (int i = first + 1; i <= last; i++) {
					...
					String name = StringTool.limitLength(StringTool.getSafe(line, 4), 254);
					String lim = StringTool.getSafe(line, 5);
					String fach = StringTool.getSafe(line, 6);
					
					String id =
						new Query<Labor2009Tarif>(Labor2009Tarif.class).findSingle(
							Labor2009Tarif.FLD_CODE, Query.EQUALS, code);
					if (id != null) {
						Labor2009Tarif lt = Labor2009Tarif.load(id);
						lt.set(new String[] {
							Labor2009Tarif.FLD_CHAPTER, Labor2009Tarif.FLD_CODE,
							Labor2009Tarif.FLD_TP, Labor2009Tarif.FLD_NAME,
							Labor2009Tarif.FLD_LIMITATIO, Labor2009Tarif.FLD_FACHBEREICH,
							Labor2009Tarif.FLD_FACHSPEC
						}, chapter, code, tp, name, lim, fach, Integer.toString(Fachspec
							.getFachspec(specs, i)));
						
					} else {
						new Labor2009Tarif(chapter, code, tp, name, lim, fach, Fachspec
							.getFachspec(specs, i));
					}
					monitor.worked(1);

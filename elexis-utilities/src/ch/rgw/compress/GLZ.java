 ...
 * It compresses much less than zip, but is fast enough to be implemented in
 * pure Java. (I did program it in assembler originally in the early 90's though; 
 * this alorithm programmed in assembler and running on a 80386 of those days
 * was even slower than running on a computer of today in this java programm, which
 * is essentially a transscript of the original Assembler source)
 * @author Gerry
 */
public class GLZ
    ...
    newdic:
        while(true) {
            ...
            {   dx=Prev1=(short)bis.pullBits(bitcount);
                switch(Prev1)
                { case 0x101: bitcount++;      //  Extend codesize and fall thru
                  case 0x100: continue;        // EOF; will break at "while" 
                  case 0x102: continue newdic; //  Begin new dictionary
                  default:
                    int si=ofb;
                    if(Prev1>=LZCode)
                    {  buff1[si++]=Act;

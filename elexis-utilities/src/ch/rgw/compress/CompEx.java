...
public class CompEx {
    ...
    public static final byte[] Compress(String in, int mode){
        ...
        try{
        	return Compress(in.getBytes(StringTool.default_charset),mode);
        }catch(Exception ex){
        	ExHandler.handle(ex);
        	return null;
        }
    ...
    public static final byte[] Compress(InputStream in, int mode){
        try{
            switch(mode){
            case GLZ: return CompressGLZ(in);
            case BZIP2: return CompressBZ2(in);
            case ZIP:	return CompressZIP(in);

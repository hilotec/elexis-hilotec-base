...
public class SAT {
	 ...
	 */
	public Map<String, Serializable> unwrap(byte[] encrypted,
			boolean bCheckSignature) throws CryptologistException {
		if (encrypted == null) {
		...
		if ((dec == null) || (!dec.isOK())) {
			throw new CryptologistException(
					"Decryption failed: " + dec == null ? "dec is null" : dec
							.toString(),
	 ...
	 * @throws Exception
	 */
	public byte[] wrap(Map<String, Serializable> var, String dest)
		...
		try {
			...
			throw new CryptologistException("Internal Cryptologist error: "
					+ ex.getMessage(), CryptologistException.ERR_INTERNAL);
		}

...
public class Test_SqlWithUiRunner {
	...
	public void testUiRunSqlStringsFail() {
		...
		try {
			...
			// expected exception ch.rgw.tools.JdbcLinkSyntaxException: Fehler bei: SELECT * FROM TEST_SQLWITHUIRUNNER; (SQLState: 42S02)
			// Caused by: org.h2.jdbc.JdbcSQLException: Tabelle "TEST_SQLWITHUIRUNNER" nicht gefunden
		} finally {
			if(link != null && statement != null)
				link.releaseStatement(statement);
		}

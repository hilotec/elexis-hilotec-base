...
public class DocHandle extends PersistentObject implements IOpaqueDocument {
	...
	public static final String createDB = "CREATE TABLE " + TABLENAME + " (" + //$NON-NLS-1$ //$NON-NLS-2$
			...
			"Doc			BLOB);"
			+ //$NON-NLS-1$
			"CREATE INDEX OMN1 ON " + TABLENAME
			+ " (PatID);" + //$NON-NLS-1$ //$NON-NLS-2$
			"CREATE INDEX OMN2 ON " + TABLENAME
			+ " (Keywords);" + //$NON-NLS-1$ //$NON-NLS-2$
			"INSERT INTO " + TABLENAME
			+ " (ID, TITLE) VALUES ('1','" + DBVERSION + "');"; //$NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$

	public static final String upd120 = "ALTER TABLE " + TABLENAME + " MODIFY Mimetype VARCHAR(255);" + //$NON-NLS-1$ //$NON-NLS-2$
	...
	static {
		addMapping(
				TABLENAME,
				"PatID", "Datum=S:D:Datum", "Titel=Title", "Keywords", "Path", "Doc", //$NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$ //$NON-NLS-4$ //$NON-NLS-5$ //$NON-NLS-6$
				"Mimetype"); //$NON-NLS-1$
		...
		if (start == null) {
			...
			if (vi.isOlder(DBVERSION)) {
				if (vi.isOlder("1.1.0")) { //$NON-NLS-1$
					getConnection()
							.exec("ALTER TABLE " + TABLENAME + " ADD deleted CHAR(1) default '0';"); //$NON-NLS-1$ //$NON-NLS-2$
	...
	public DocHandle(byte[] doc, Patient pat, String title, String mime,
			String keyw) {
		...
		try {
			...
			}, pat.getId(), new TimeTool().toString(TimeTool.DATE_GER), title,
					keyw, mime);
		} catch (PersistenceException pe) {
			log.log(Messages.DocHandle_dataNotWritten + "; " + pe.getMessage(),
					Log.ERRORS);
		}
	}
	...
	public static void assimilate(String f) {
		...
		if (!file.canRead()) {
			SWTHelper
					.showError(Messages.DocHandle_cantReadCaption,
		...
		if (fid.open() == Dialog.OK) {
			try {
				...
				new DocHandle(baos.toByteArray(), act, fid.title,
						file.getName(), fid.keywords);
			} catch (Exception ex) {
				ExHandler.handle(ex);
				SWTHelper.showError(Messages.DocHandle_importErrorCaption,

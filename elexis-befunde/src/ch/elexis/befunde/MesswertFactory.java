 ...
 * A Factory is necessary, when elexis needs to transfer instances of a class via dra&drop or load instances
 ...
 * Datenbank holen und andererseits auch ein Objekt als nicht gespeichertes Template erzeugen.
 * Eine Factory wird in einem Plugin immer dann benötigt, wenn Elexis Objekte dieses Plugins
 * per Drag&Drop transferieren oder aus der Datenbank einlesen muss. Dies deswegen, weil der Eclipse Class loader
 * einen Classpath nicht über Plugin-Grenzen hinweg auflösen kann
 * (ch.elexis,data im Kernprogramm referenziert nicht dasselbe Package wie ch.elexis.data in einem Plugin)
 * 
 * Die Klasse selbst ist sehr einfach und kann direkt per Copy&Paste aus dieser Vorlage

...
public class MedikamentImporterVidal extends ImporterPage {
	...
	public IStatus doImport(IProgressMonitor monitor) throws Exception {
		...
		boolean currenttsok = currentts.set(Hub.globalCfg.get(
				PreferenceConstants.ARTIKEL_AT_RPHEADER_PUBDATE, "20010101"));
		...
		if (eSubstances != null) {
			...
			int counter = 1;
			for (Element eSubstance : (List<Element>) eSubstances
					.getChildren("SubstRef")) {
				...
				monitor.subTask("Lese Substanzen ein " + "[" + counter + "/"
						+ noOfSubstances + "]: " + Name);
				try {
					...
					// CH_ELEXIS_AUSTRIAMEDI_SUBSTANCE(ID) VALUES ('100')
					// (SQLState: 23505)
					// 10.03.2011 18:01:56 ch.rgw.tools.Log log
					// INFO: Loading database driver org.postgresql.Driver
					// Fehler bei: INSERT INTO
					// CH_ELEXIS_AUSTRIAMEDI_SUBSTANCE(ID) VALUES ('1002')
					// (SQLState: 23505)
					// 10.03.2011 18:01:56 ch.rgw.tools.Log log
					// INFO: Loading database driver org.postgresql.Driver
					// Fehler bei: INSERT INTO
					// CH_ELEXIS_AUSTRIAMEDI_SUBSTANCE(ID) VALUES ('1003')
					// (SQLState: 23505)
					// 10.03.2011 18:01:56 ch.rgw.tools.Log log
					// INFO: Loading database driver org.postgresql.Driver
		...
		if (eMedis != null) {
			...
			for (Element eMedi : (List<Element>) eMedis.getChildren("RpEntry")) {
				...
				String mediStatus = eMedi.getAttributeValue("Status"); // H(zugelassen,
																		// im
																		// Handel,
																		// lieferbar)
				...
				String codeClass = eMedi.getChild("SSigns").getAttribute("Box")
						.getValue().trim();
				monitor.subTask("Lese Medikamente ein " + "[" + counter + "/"
						+ noOfMedicaments + "]: " + SName);
				...
				act.put("OName", eMedi.getChildText("OName")); // offizieller
																// Produktname
				act.put("Status", mediStatus);
				act.put("DoLC", eMedi.getChildText("DoLC")); // Date of Last
																// Change
				...
				act.put("SUnit",
						eMedi.getChild("Unit").getAttributeValue("SUnit"));
				...
				act.put("ZNrNum",
						eMedi.getChild("ZNr").getAttributeValue("ZNrNum"));
				act.put("Remb",
						eMedi.getChild("SSigns").getAttributeValue("Remb"));
				Hashtable<String, Object> RSigns = new Hashtable<String, Object>();
				for (String val : Medikament.RSIGNS) {
					String cont = eMedi.getChild("RSigns").getAttributeValue(
							val);
				...
				for (String val : Medikament.SSIGNS) {
					String cont = eMedi.getChild("SSigns").getAttributeValue(
							val);
					SSigns.put(val, cont == null ? "0" : cont);
				}
				act.put("SSigns", SSigns);
				String INDText = eMedi.getChild("SSigns").getChildText(
						"INDText");
				if (INDText != null)
					act.put("INDText", INDText);
				String RuleText = eMedi.getChild("SSigns").getChildText(
						"RuleText");
				if (RuleText != null)
					act.put("RuleText", RuleText);
				String RemarkText = eMedi.getChild("SSigns").getChildText(
						"RemarkText");
				if (RemarkText != null)
					act.put("RemarkText", RemarkText);

				// Substances: n/SN/SN/SN/... where n = Anzahl elemente, SN =
				// Substanz
				int noOfSubstances = eMedi.getChild("Substances")
						.getChildren("Substance").size();
				StringBuilder eMediSubstances = new StringBuilder();
				eMediSubstances.append(noOfSubstances);
				for (Element eSubstance : (List<Element>) eMedi.getChild(
						"Substances").getChildren("Substance")) {
					eMediSubstances.append("/").append(
							eSubstance.getValue().trim());
				}
				act.put("Substances", eMediSubstances.toString());

				if (actType.equals(ENTRYTYPE_INSERT)) { // Datensatz ist zum
														// anfuegen markiert
					medi = new Medikament(SName, ENTRYTYPE_NAME, PhZNr);

				} else { // Datensatz ist zum loeschen/aktualisieren markiert
					Query<Medikament> qbe = new Query<Medikament>(
							Medikament.class);
					qbe.clear();
					...
					if (list.size() > 0) {
						...
						if (actType.equals(ENTRYTYPE_DELETE)) {
							// Set blackbox, will be cleaned by cleanMedikamente
							// script, if no more
							// references to it!
				...
				sb.append(SName).append(" (")
						.append(eMedi.getChildText("Quantity")).append(")");
				...
				medi.set(new String[] { Artikel.FLD_CODECLASS,
						Artikel.FLD_VK_PREIS, Artikel.FLD_EK_PREIS,
						Artikel.EIGENNAME }, codeClass, AVP.getCentsAsString(),
						KVP.getCentsAsString(), sb.toString());
		...
		Hub.globalCfg.set(PreferenceConstants.ARTIKEL_AT_RPHEADER_FILENAME,
				RpHeader.getChildText("Filename"));
		Hub.globalCfg.set(PreferenceConstants.ARTIKEL_AT_RPHEADER_GENERATOR,
				RpHeader.getChildText("Generator"));
		Hub.globalCfg.set(PreferenceConstants.ARTIKEL_AT_RPHEADER_PUBTITLE,
				RpHeader.getChildText("PubTitle"));
		Hub.globalCfg.set(
				PreferenceConstants.ARTIKEL_AT_RPHEADER_PUBTITLE_COUNTRY,
				RpHeader.getChild("PubTitle").getAttributeValue("Country"));
		Hub.globalCfg.set(PreferenceConstants.ARTIKEL_AT_RPHEADER_PUBAUTHOR,
				RpHeader.getChildText("PubAuthor"));
		Hub.globalCfg.set(PreferenceConstants.ARTIKEL_AT_RPHEADER_PUBDATE,
				RpHeader.getChildText("PubDate"));
		Hub.globalCfg.set(PreferenceConstants.ARTIKEL_AT_RPHEADER_PUBCOPYRIGHT,

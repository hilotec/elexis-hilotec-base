...
public class Utilities {
	...
	public static void cleanPrescriptionTable(){
		...
		for (Iterator<Prescription> iterator = presList.iterator(); iterator.hasNext();) {
			...
			if (!validPerson && !validArticle && !validRecipe) {
				invalidRecipes++;
				Hub.log.log("Lösche Veschreibung " + prescription.getId()
					+ ". Weder gültiger Kontakt, Artikel noch Rezept gefunden.", Log.TRACE);
				...
				if (PhZNr != "" && PhZNr != null) {
					...
					try {
						...
						if (newest.equals(currArtikel)) {
							pen.println(prescription.getId() + ":[OK]:" + PhZNr
								+ " No update necessary.");
							continue;
						}
	...
	public static void cleanMedikamente(){
		...
		try {
			...
			for (Iterator<Medikament> iterator = mediList.iterator(); iterator.hasNext();) {
				...
				if (PhZNr != "" && PhZNr != null) {
					...
					if (mediListN.size() == 1) {
						...
						pen.println("Keeping " + newest.getLabel() + " from "
							+ newest.getLastUpdate());
						pen.flush();
			...
			// Lösche alle Artikel vom Typ Vidal, die nicht mehr einer Verschreibung zugeordnet sind
			// (Alter Datensatz)
			...
			if (deleteOldVidal) {
				...
				for (Artikel art : artikelList) {
					if (!resultIDMap.contains(art)) {
						pen.println("Deleting " + art.getId() + " as its "
							+ art.get(Artikel.FLD_TYP));
						art.delete();
					} else {
			...
			if (deleteNonReferencedBlackBoxes) {
				...
				for (Medikament medi : blackMedis) {
					if (!resultIDMap.contains(medi)) {
						pen.println("Deleting black box " + medi.getId()
							+ " as no more references..");
						medi.delete();
					} else {
						pen.println("Cant delete blackbox medi " + medi.getId()
							+ " as still referenced.");
					}
				}
			}

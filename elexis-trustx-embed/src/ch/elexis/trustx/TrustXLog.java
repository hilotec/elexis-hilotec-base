...
public class TrustXLog {
		...
		try{
			...
			while(true){									// Wait until trustx finished the job
				if(System.currentTimeMillis()>timeout){		// wait not longer than "timeout"
					SWTHelper.alert("Trustx-Fehler: Timeout", "Empfange keine Rückmeldung von Trustx");
					return false;
				}
				lines=readLines();
				if(lines==null){
	...
	boolean isFinished(String[] lines){
		...
		if(fields[OFFSET_MSG_LEVEL].equals("1") 
				&& fields[OFFSET_MSG_CLASS].equals("2")
				&& fields[OFFSET_MSG_ORIGIN].equals("4")
				&& fields[OFFSET_MSG_MESSAGE].equals("1")){
					return true;
				}
		return false;
	}

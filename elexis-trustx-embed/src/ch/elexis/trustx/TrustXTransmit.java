						...
						for (Rechnung rn : rnn) {
							...
							if (status) {
								rn.addTrace(Rechnung.OUTPUT, getDescription() + ":"
									+ trustx.trustCenter() + ":" + trustx.asasLogin());
								continue;
							}
							rn.reject(RnStatus.REJECTCODE.REJECTED_BY_PEER, xlog
								.getLastErrorString());
							res.add(Result.SEVERITY.ERROR, 1, "Fehler beim Übertragen: "
								+ xlog.getLastErrorString(), rn, true);
							errors++;
						}
						monitor.done();
						if (errors > 0) {

...
public class Importer extends ImporterPage {
	...
	public Composite createPage(Composite parent) {
		...
		if (jarPath != null) {
			...
			if (jar.canRead()) {
				try {
					URLClassLoader urlLoader = getURLClassLoader(new URL(
							"file", null, jar.getAbsolutePath()));

					Class openmedicalClass = urlLoader
							.loadClass(OPENMEDICAL_MAINCLASS);

					// try to get the download method
					Method meth;
					try {
						meth = openmedicalClass.getMethod("download",
								String[].class);
					} catch (Throwable e) {
						throw e;
					}
	...
	/**
	 * Create reminder for mandant, so that he knows that new lab results are
	 * avilable. To be replaced by a separate view.
	 * 
	 * @param lr
	 *            the added LabResult
	 */
	 ...
	 * Query<Reminder>(Reminder.class); query.add("IdentID", "=",
	 * lr.getPatient().getId()); query.and(); query.add("Due", "=",
	 * lr.getDate()); query.and(); query.add("Message", "=", "Neue Laborwerte");
	 * List<Reminder> reminders = query.execute(); if (reminders == null ||
	 * reminders.size() == 0) { // create new reminder Mandant mandant =
	 * Hub.actMandant; Reminder reminder = new Reminder(lr.getPatient(), new
	 * TimeTool(lr.getDate()).toString(TimeTool.DATE_GER),
	 * Reminder.Typ.anzeigeTodoAll, "", "Neue Laborwerte");
	 * reminder.addToList("Responsibles", mandant.getId(), (String[]) null); }
	 * else { // reset status Reminder reminder = reminders.get(0);
	 * Reminder.Status status = reminder.getStatus(); if (status ==
	 * Reminder.Status.erledigt || status == Reminder.Status.unerledigt) {
	 * reminder.setStatus(Reminder.Status.geplant); } } }
	 */
		...
		while (obr != null) {
			...
			while (obx != null) {
				...
				if (list.size() < 1) {
					...
					// number++);
					li = new LabItem(code, name, labor, obx.getRefRange(), obx
							.getRefRange(), obx.getUnits(), typ, group, order);
				} else {
					li = list.get(0);
				}
				LabResult lr;
				...
				if (existingResults != null && existingResults.size() != 0) {
					 ...
					 * , "Weitermachen?")){ obx=obr.nextOBX(obx); continue;
					 * }else{ return new Result<String>("Cancelled"); }
					 */
					...
					for (LabResult existingResult : existingResults) {
						...
						if (obx.isFormattedText()) {
							newResult = "text";
							newComment = PersistentObject.checkNull(obx
									.getResultValue());
						} else {
							newResult = PersistentObject.checkNull(obx
									.getResultValue());
							newComment = PersistentObject.checkNull(obx
									.getComment());
						}
					...
					if (lr != null) {
						...
						// create a new result
						if (obx.isFormattedText()) {
							lr = new LabResult(pat, sampleDate, li, "text", obx
									.getResultValue());
						} else {
							lr = new LabResult(pat, sampleDate, li, obx
		...
		if (!StringTool.isNothing(comments)) {
			...
			if (obr != null) {
				...
				if (list.size() < 1) {
					...
					String order = Groups.getCodeOrderByGroupName(
							COMMENT_GROUP, labor);
					li = new LabItem(COMMENT_CODE, COMMENT_NAME, labor, "", "",
							"", typ, COMMENT_GROUP, order);
				} else {
					li = list.get(0);
				}
	...
	private Result importDirect() {
		...
		String downloadDirPath = Hub.localCfg.get(PreferencePage.DL_DIR, System
				.getenv("temp"));
		...
		if (iniPath != null) {
			try {
				Object omResult = openmedicalDownloadMethod.invoke(
						openmedicalObject, new Object[] { new String[] {
								"--download", downloadDirPath, "--logPath",
								downloadDirPath, "--ini", iniPath, "--verbose",
								"INF", "-#OpenMedicalKey#", "-allInOne" } });
				if (omResult instanceof Integer) {
					res = ((Integer) omResult).intValue();
					System.out.println(res + " files downoladed");
					if (res < 1) {
						SWTHelper
								.showInfo("Verbindung mit Labor " + MY_LAB
										+ " erfolgreich",
		...
		if (downloadDir.isDirectory()) {
			...
			SWTHelper.showInfo("Verbindung mit Labor " + MY_LAB
					+ " erfolgreich", "Es wurden " + Integer.toString(res)
					+ " Dateien verarbeitet");
		} else {
			SWTHelper
					.showError("Falsches Verzeichnis",
			...
			SelectionAdapter sa = new SelectionAdapter() {
				public void widgetSelected(SelectionEvent e) {
					...
					if (type == FILE) {
						...
						Hub.localCfg
								.set(
										"ImporterPage/" + home.getTitle() + "/filename", ""); //$NON-NLS-1$ //$NON-NLS-2$
					}
				}
			};

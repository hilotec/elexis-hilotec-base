...
public class Importer extends ImporterPage {
	...
	/**
	 * Create reminder for mandant, so that he knows that new lab results are avilable.
	...
	private void notifyMandant(LabResult lr) {
		...
		query.and();
		query.add("Due", "=", lr.getDate());
		query.and();
		query.add("Message", "=", "Neue Laborwerte");
		List<Reminder> reminders = query.execute();
		if (reminders == null || reminders.size() == 0) {
			// create new reminder
			Mandant mandant = Hub.actMandant;
			Reminder reminder = new Reminder(lr.getPatient(), new TimeTool(lr.getDate()).toString(TimeTool.DATE_GER), Reminder.Typ.anzeigeTodoAll, "", "Neue Laborwerte");
			reminder.addToList("Responsibles", mandant.getId(), (String[]) null);
		} else {
			// reset status
			Reminder reminder = reminders.get(0);
			Reminder.Status status = reminder.getStatus();
			if (status == Reminder.Status.erledigt || status == Reminder.Status.unerledigt) {

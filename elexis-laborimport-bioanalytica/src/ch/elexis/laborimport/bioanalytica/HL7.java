...
public class HL7 {
	...
	/**
	 * Find the patient denoted by this HL7-record. If it can't be found in the database,
	 * ask the user to choose one.
	 * @return the Patient or null if it has not been found, or
	...
	public Result<Patient> getPatient() {
		...
		if(pat==null){
			...
			if(list.size()==0){
				...
				PatientSelektorAdapter psa = new PatientSelektorAdapter("Patient wählen", 
						"Der Patient \"" + labLabel + "\" wurde anhand der Nummer des Labors nicht gefunden. Bitte wählen Sie"
						+ " einen Patienten aus.");
				psa.open();
				...
				pat= list.get(0);
				if(!pat.getName().equalsIgnoreCase(name[0]) || !pat.getVorname().equalsIgnoreCase(name[1])
					...
					// use PatientSelektorAdapter, since we don't run in the main thread
					PatientSelektorAdapter psa = new PatientSelektorAdapter("Patient wählen", 
							"Der Patient \"" + labLabel + "\" stimmt nicht mit dem in Elexis gefundene Patienten \"" + elexisLabel + "\" überein."
	...
	class OBR{
		 ...
		 * positions. 
		 * @return the OBR's date. If none was found, it will be the date of
		 *         the first OBX found
		 */
		TimeTool getDate(){
			String date=field[7];

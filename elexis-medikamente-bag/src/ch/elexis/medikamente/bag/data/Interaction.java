...
public class Interaction extends PersistentObject implements Comparable<Interaction> {
	...
	static final String createDB =
		"CREATE TABLE " + TABLENAME + " (" + "ID			VARCHAR(25)	primary key,"
			...
			+ "(ID,Type) VALUES ('VERSION','" + VERSION + "');" + "CREATE INDEX CEMBI1 ON "
			+ TABLENAME + " (Subst1);" + "CREATE INDEX CEMBI2 ON " + TABLENAME + " (Subst2);";
	...
	public Interaction(final Substance s1, final Substance s2, final String desc, final int t,
		...
		}, s1.getId(), s2.getId(), desc, Integer.toString(t), Integer.toString(sev), Hub.actMandant
			.getId(), new TimeTool().toString(TimeTool.DATE_GER));
	}
	...
	public String getLabel(){
		...
		int type = getType();
		if (type > 0) {
			sb.append(": ").append(INTERAKTIONSTYPEN[getType()]).append(" (Relevanz: ").append(
				RELEVANCE[getRelevance()]).append(")");
		}
		return sb.toString();
	}

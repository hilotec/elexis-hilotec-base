...
public class PatientDetailView extends ViewPart {
	...
	private final static String FIXMEDIKATION = Messages
			.getString("Patientenblatt2.fixmedication");
	...
	public void createPartControl(Composite parent) {
		...
		scrldfrm.setLayoutData(new GridData(SWT.FILL, SWT.FILL, true, true, 1,
				1));
		TableWrapLayout td = new TableWrapLayout();
		scrldfrm.setText(Messages
				.getString("Patientenblatt2.noPatientSelected"));
		scrldfrm.getBody().setLayout(td);
		Composite scrldfrmComposite = scrldfrm.getBody();

		// custom area for user
		{
			compClientCustomText = new ClientCustomTextComposite(
					scrldfrm.getBody(), SWT.None, toolkit, scrldfrm);
		}

		// sticker composite
		{
			stickerComposite = new StickerComposite(scrldfrm.getBody(),
					SWT.NONE, toolkit);
		}

		// additional addresses
		{
			ExpandableComposite ecZA = WidgetFactory.createExpandableComposite(
					toolkit, scrldfrm,
					Messages.getString("Patientenblatt2.additionalAdresses")); //$NON-NLS-1$
			ecZA.setExpanded(Hub.localCfg.get(KEY_PATIENTENBLATT
					+ ecZA.getText(), false));	
			ecZA.addExpansionListener(new SectionExpansionHandler());
			inpZusatzAdresse = new ListDisplay<BezugsKontakt>(ecZA, SWT.NONE,
					new ListDisplay.LDListener() {
						/*
						 * public boolean dropped(final PersistentObject
						...
						public void hyperlinkActivated(final String l) {
							...
							if (ksl.open() == Dialog.OK) {
								...
								if (bza.open() == Dialog.OK) {
									...
									BezugsKontakt bk = ElexisEventDispatcher
											.getSelectedPatient()
											.addBezugsKontakt(k, bezug);
						...
						public String getLabel(Object o) {
							...
							Kontakt other = Kontakt.load(bezugsKontakt
		...
		// diagnosis
		{
			Section sectDiagnosen = toolkit.createSection(scrldfrmComposite,
					Section.EXPANDED | Section.TWISTIE);
			TableWrapData twd_sectDiagnosen = new TableWrapData(
			...
			sectDiagnosen.setExpanded(Hub.localCfg.get(KEY_PATIENTENBLATT
					+ sectDiagnosen.getText(), false));

			txtDiagnosen = toolkit.createText(sectDiagnosen, "", SWT.WRAP
					| SWT.MULTI);
			txtDiagnosen.addListener(SWT.Modify, new MultiLineAutoGrowListener(
					txtDiagnosen));
		...
		// personal anamnesis
		{
			Section sectAnamnese = toolkit.createSection(scrldfrmComposite,
					Section.EXPANDED | Section.TWISTIE);
			TableWrapData twd_sectDiagnosen = new TableWrapData(
					TableWrapData.LEFT, TableWrapData.TOP, 1, 1);
			twd_sectDiagnosen.grabHorizontal = true;
			...
			sectAnamnese.setExpanded(Hub.localCfg.get(KEY_PATIENTENBLATT
					+ sectAnamnese.getText(), false));

			txtAnamnese = toolkit.createText(sectAnamnese, "", SWT.WRAP
					| SWT.MULTI);
			txtAnamnese.addListener(SWT.Modify, new MultiLineAutoGrowListener(
					txtAnamnese));
		...
		// family anamnesis
		{
			Section sectFamAnamnese = toolkit.createSection(scrldfrmComposite,
					Section.EXPANDED | Section.TWISTIE);
			TableWrapData twd_sectDiagnosen = new TableWrapData(
					TableWrapData.LEFT, TableWrapData.TOP, 1, 1);
			twd_sectDiagnosen.grabHorizontal = true;
			...
			sectFamAnamnese.setExpanded(Hub.localCfg.get(KEY_PATIENTENBLATT
					+ sectFamAnamnese.getText(), false));

			txtFamAnamnese = toolkit.createText(sectFamAnamnese, "", SWT.WRAP
					| SWT.MULTI);
			txtFamAnamnese.setText("");
			txtFamAnamnese.addListener(SWT.Modify,
					new MultiLineAutoGrowListener(txtFamAnamnese));
		...
		// allergy
		{
			Section sectAllergien = toolkit.createSection(scrldfrmComposite,
					Section.EXPANDED | Section.TWISTIE);
			TableWrapData twd_sectDiagnosen = new TableWrapData(
					TableWrapData.LEFT, TableWrapData.TOP, 1, 1);
			twd_sectDiagnosen.grabHorizontal = true;
			...
			sectAllergien.setExpanded(Hub.localCfg.get(KEY_PATIENTENBLATT
					+ sectAllergien.getText(), false));

			txtAllergien = toolkit.createText(sectAllergien, "", SWT.WRAP
					| SWT.MULTI);
			txtAllergien.setText("");
			txtAllergien.addListener(SWT.Modify, new MultiLineAutoGrowListener(
					txtAllergien));
		...
		// risks
		{
			Section sectRisiken = toolkit.createSection(scrldfrmComposite,
					Section.EXPANDED | Section.TWISTIE);
			TableWrapData twd_sectDiagnosen = new TableWrapData(
					TableWrapData.LEFT, TableWrapData.TOP, 1, 1);
			twd_sectDiagnosen.grabHorizontal = true;
			...
			sectRisiken.setExpanded(Hub.localCfg.get(KEY_PATIENTENBLATT
					+ sectRisiken.getText(), false));

			txtRisiken = toolkit.createText(sectRisiken, "", SWT.WRAP
					| SWT.MULTI);
			txtRisiken.setText("");
			txtRisiken.addListener(SWT.Modify, new MultiLineAutoGrowListener(
					txtRisiken));
		...
		// remarks
		{
			Section sectBemerkungen = toolkit.createSection(scrldfrmComposite,
					Section.EXPANDED | Section.TWISTIE);
			TableWrapData twd_sectDiagnosen = new TableWrapData(
					TableWrapData.LEFT, TableWrapData.TOP, 1, 1);
			twd_sectDiagnosen.grabHorizontal = true;
			...
			sectBemerkungen.setExpanded(Hub.localCfg.get(KEY_PATIENTENBLATT
					+ sectBemerkungen.getText(), false));

			txtBemerkungen = toolkit.createText(sectBemerkungen, "", SWT.WRAP
					| SWT.MULTI);
			txtBemerkungen.setText("");
			txtBemerkungen.addListener(SWT.Modify,
					new MultiLineAutoGrowListener(txtBemerkungen));
	...
	private final class MultiLineAutoGrowListener implements Listener {
		...
		public void handleEvent(Event event) {
			...
			if (t.getLineCount() != lines) {
				...
				t.setSize(t.getSize().x, lines
						* (int) t.getFont().getFontData()[0].height);
				scrldfrm.reflow(true);
			}
		}
	}
	...
	private final class SectionExpansionHandler extends ExpansionAdapter {
		...
		public void expansionStateChanged(ExpansionEvent e) {
			...
			Hub.localCfg.set(KEY_PATIENTENBLATT + src.getText(),
					src.isExpanded());
			scrldfrm.reflow(true);
		}
	}
	...
	protected void initDataBindings() {
		...
		Text[] control = { txtAllergien, txtAnamnese, txtBemerkungen,
				txtDiagnosen, txtRisiken, txtFamAnamnese };
		String[] property = { "allergies", "personalAnamnese", "comment",
				"diagnosen", "risk", "familyAnamnese" };
	...
	private void bindValue(Text text, String property,
			DataBindingContext bindingContext) {
		IObservableValue textObserveWidget = SWTObservables
				.observeDelayedValue(5,
						SWTObservables.observeText(text, SWT.Modify));
		IObservableValue observeValue = PojoObservables.observeDetailValue(
				patientObservable, property, String.class);

...
public class ClientCustomTextComposite extends Composite {
	...
	public ClientCustomTextComposite(Composite parent, int style,
			FormToolkit toolkit, ScrolledForm scrldfrm) {
		super(parent, style);
		...
		TableWrapData twd_txtClientCustomText = new TableWrapData(
		...
		txtClientCustomText.addListener(SWT.Modify,
				new StyledTextMultiLineAutoGrowListener());
		...
		txtClientCustomTextdropTarget = new DropTarget(txtClientCustomText,
				DND.DROP_COPY);
		txtClientCustomTextdropTarget.setTransfer(new Transfer[] { TextTransfer
				.getInstance() });
		txtClientCustomTextdropTarget.addDropListener(new DropTargetAdapter() {
			...
			public void drop(DropTargetEvent event) {
				txtClientCustomText.setText(txtClientCustomText.getText()
						+ event.data);
			}
		});

		btnEditCustomText = toolkit.createButton(this, "", SWT.TOGGLE);
		TableWrapData twd_btnEditCustomText = new TableWrapData(
				TableWrapData.LEFT, TableWrapData.TOP, 1, 1);
		...
		btnEditCustomText.addSelectionListener(new SelectionAdapter() {
			...
			public void widgetSelected(SelectionEvent e) {
				if (btnEditCustomText.getSelection()) {
					txtClientCustomText.setBackground(Desk.getDisplay()
							.getSystemColor(SWT.COLOR_INFO_BACKGROUND));
				} else {
					txtClientCustomText.setBackground(null);
				}
				txtClientCustomText.setEditable(btnEditCustomText
						.getSelection());
				setClientCustomAreaContent(btnEditCustomText.getSelection());
			}
		});
		btnEditCustomText.setImage(Desk.getImage(Desk.IMG_PENCIL_8PX));
		GridData gd_btnEditCustomText = new GridData(SWT.LEFT, SWT.TOP, false,
				false, 1, 1);
	...
	public void updateClientCustomArea() {
		tokenMap.clear();
		styleList.clear();
		String outputParsed = findAndReplaceTemplates(Hub.globalCfg.get(
				ClientCustomTextComposite.class.getName(), ""));
		String output = initializeStyleRanges(outputParsed);
		txtClientCustomText.setText(output);
		txtClientCustomText
				.setStyleRanges(styleList.toArray(new StyleRange[0]));
	}
...
//		}
		...
		for (TokenMap tm : tokenMap) {
			...
			if (tokenAttribute.equals(Kontakt.FLD_E_MAIL)
					|| tokenAttribute.equals("Balance")) {
				StyleRange sr = new StyleRange();
				sr.start = tm.start;
				sr.length = tm.end - tm.start;
				sr.foreground = PlatformUI.getWorkbench().getDisplay()
...
//		return sb.toString();
	...
	/**
	 * Finds templates in the form of "[class.property]" in a string and returns
	 * a string with the respective replaced values
	 * 
	 * @param replaceString
	...
	/**
	 * resolves the position of the caret within the {@link StyledText} widget
	 * to the associated {@link TokenMap}
	 * 
	 * @param caretPosition
	...
	/**
	 * stores tokens used within the custom text composite. This is used to
	 * provide a back-forth mapping between the values and tokens according to
	 * their position.
	 */
	...
	/**
	 * This {@link Listener} automatically grows and shrinks a
	 * {@link StyledText} according to the number of lines contained. It handles
	 * {@link SWT#Modify} events only.
	 * 
		...
		public void handleEvent(Event event) {
			...
			if (txtClientCustomText.getLineCount() != lines) {
				...
				txtClientCustomText.setSize(txtClientCustomText.getSize().x,
						lines
								* (int) txtClientCustomText.getFont()

...
public class Importer extends ImporterPage {
	...
	/**
	 * The import process starts when the user has selected a file and clicked
	 * "OK". Warning: We can not read fields of the page created in createPage
	 * here! (The page is already disposed when doImport is called). If we have
	 * to transfer field values between createPage and doImport, we must
	 * override collect(). Our file based importer saves the user input in
	 * results[0]
	 */
	@Override
	public IStatus doImport(final IProgressMonitor monitor) throws Exception{
		...
		if (!file.canRead()) {
			...
			return new Status(Status.ERROR, "ch.elexis.privatrechnung",
	...
	private Result<String> importExcel(final String file,
		...
		xl.setFieldTypes(new Class[] {
			...
			Integer.class, Integer.class, TimeTool.class, TimeTool.class,
			Double.class
		});
	...
	private Result<String> importCSV(final String file,
		final IProgressMonitor mon){
		try {

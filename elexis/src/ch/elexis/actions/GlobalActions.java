 ...
 *    
 * $Id: GlobalActions.java 5970 2010-01-27 16:43:04Z rgw_ch $
 *******************************************************************************/
...
public class GlobalActions {
	...
	public static IWorkbenchAction exitAction, newWindowAction, copyAction,
			cutAction, pasteAction;
	public static IAction loginAction, importAction, testAction, aboutAction,
			helpAction, prefsAction;
	public static IAction connectWizardAction, changeMandantAction,
			savePerspectiveAction, savePerspectiveAsAction;
	public static IAction savePerspectiveAsDefaultAction,
			resetPerspectiveAction, homeAction, fixLayoutAction;
	public static IAction printEtikette, printBlatt, printAdresse,
			printVersionedEtikette;
	public static IAction printRoeBlatt;
	public static IAction delFallAction, delKonsAction, openFallaction,
			filterAction, reopenFallAction, makeBillAction,
			planeRechnungAction;
	public static IAction moveBehandlungAction, redateAction, neueKonsAction,
			neuerFallAction;
	...
	public GlobalActions(final IWorkbenchWindow window) {
		...
		savePerspectiveAction = new Action(Messages
				.getString("GlobalActions.SavePerspective")) { //$NON-NLS-1$
			{
				setId("savePerspektive"); //$NON-NLS-1$
				// setActionDefinitionId(Hub.COMMAND_PREFIX+"savePerspektive"); //$NON-NLS-1$
				setToolTipText(Messages
						.getString("GlobalActions.SavePerspectiveToolTip")); //$NON-NLS-1$
		...
		helpAction = new Action(Messages.getString("GlobalActions.ac_handbook")) { //$NON-NLS-1$
			{
				...
				setToolTipText(Messages
						.getString("GlobalActions.ac_openhandbook")); //$NON-NLS-1$

			}

			@Override
			public void run() {
				File base = new File(Hub.getBasePath()).getParentFile()
						.getParentFile();
				String book = base.getAbsolutePath() + File.separator
						+ "elexis.pdf"; //$NON-NLS-1$
		...
		resetPerspectiveAction.setImageDescriptor(Desk
		...
		savePerspectiveAsDefaultAction = new Action(Messages
				.getString("GlobalActions.saveasstartperspective")) { //$NON-NLS-1$
			{
			...
			@Override
			public void run() {
				IPerspectiveDescriptor p = mainWindow.getActivePage()
						.getPerspective();
				Hub.localCfg
						.set(Hub.actUser + DEFAULTPERSPECTIVECFG, p.getId());
		...
		loginAction = new Action(Messages.getString("GlobalActions.Login")) { //$NON-NLS-1$
			...
			public void run() {
				try {
					...
					dlg.create();
					dlg.setTitle(Messages
							.getString("GlobalActions.LoginDialogTitle")); //$NON-NLS-1$
					dlg.setMessage(Messages
							.getString("GlobalActions.LoginDialogMessage")); //$NON-NLS-1$
					// dlg.getButton(IDialogConstants.CANCEL_ID).setText("Beenden");
					dlg
							.getShell()
							.setText(
									Messages
											.getString("GlobalActions.LoginDialogShelltext")); //$NON-NLS-1$
		...
		importAction = new Action(Messages.getString("GlobalActions.Import")) { //$NON-NLS-1$
			...
			public void run() {
				// cnv.open();
				Importer imp = new Importer(mainWindow.getShell(),
						"ch.elexis.FremdDatenImport"); //$NON-NLS-1$
				imp.create();
				imp.setMessage(Messages
						.getString("GlobalActions.ImportDlgMessage")); //$NON-NLS-1$
				imp.getShell().setText(
						Messages.getString("GlobalActions.ImportDlgShelltext")); //$NON-NLS-1$
				imp
						.setTitle(Messages
								.getString("GlobalActions.ImportDlgTitle")); //$NON-NLS-1$
				imp.open();
			}
		};

		connectWizardAction = new Action(Messages
				.getString("GlobalActions.Connection")) { //$NON-NLS-1$
			{
			...
			@Override
			public void run() {
				WizardDialog wd = new WizardDialog(mainWindow.getShell(),
						new DBConnectWizard());
				wd.open();
			}
		...
		changeMandantAction = new Action(Messages
				.getString("GlobalActions.Mandator")) { //$NON-NLS-1$
			{
		...
		printKontaktEtikette = new Action(Messages
				.getString("GlobalActions.PrintContactLabel")) { //$NON-NLS-1$
			{
				setToolTipText(Messages
						.getString("GlobalActions.PrintContactLabelToolTip")); //$NON-NLS-1$
				setImageDescriptor(Desk
						.getImageDescriptor(Desk.IMG_ADRESSETIKETTE)); //$NON-NLS-1$
			}

			@Override
			public void run() {
				printAdr((Kontakt) ElexisEventDispatcher
						.getSelected(Kontakt.class));
			}
		};

		printAdresse = new Action(Messages
				.getString("GlobalActions.PrintAddressLabel")) { //$NON-NLS-1$
			{
				setImageDescriptor(Desk
						.getImageDescriptor(Desk.IMG_ADRESSETIKETTE)); //$NON-NLS-1$
				setToolTipText(Messages
						.getString("GlobalActions.PrintAddressLabelToolTip")); //$NON-NLS-1$
			}

			@Override
			public void run() {
				printAdr((Patient) ElexisEventDispatcher
		...
		printVersionedEtikette = new Action(Messages
				.getString("GlobalActions.PrintVersionedLabel")) { //$NON-NLS-1$
			{
				setToolTipText(Messages
						.getString("GlobalActions.PrintVersionedLabelToolTip")); //$NON-NLS-1$
				setImageDescriptor(Desk
						.getImageDescriptor(Desk.IMG_VERSIONEDETIKETTE));
			}
			...
			public void run() {
				...
				if (pd != null) {
					...
					String sOrientation = Hub.localCfg.get(
							"Drucker/Etiketten/Ausrichtung", null); //$NON-NLS-1$
					try {
					...
					if (prn.startJob(Messages
							.getString("GlobalActions.PrintLabelJobName")) == true) { //$NON-NLS-1$
						GC gc = new GC(prn);
						int y = 0;
						prn.startPage();
						...
						Patient actPatient = (Patient) ElexisEventDispatcher
								.getSelected(Patient.class);
						String pid = StringTool.addModulo10(actPatient
								.getPatCode())
								+ "-" //$NON-NLS-1$
								+ new TimeTool()
										.toString(TimeTool.TIME_COMPACT);
						gc
								.drawString(
										Messages
												.getString("GlobalActions.OrderID") + ": " + pid, 0, 0); //$NON-NLS-1$ //$NON-NLS-2$
						FontMetrics fmt = gc.getFontMetrics();
						y += fmt.getHeight();
						String pers = actPatient.getPersonalia();
						gc.drawString(pers, 0, y);
						y += fmt.getHeight();
						gc.drawString(actPatient.getAnschrift().getEtikette(
								false, false), 0, y);
						y += fmt.getHeight();
						...
						tel
								.append(
										Messages
												.getString("GlobalActions.PhoneHomeLabelText")).append(actPatient.get("Telefon1")) //$NON-NLS-1$ //$NON-NLS-2$
								.append(
										Messages
												.getString("GlobalActions.PhoneWorkLabelText")).append(actPatient.get("Telefon2")) //$NON-NLS-1$ //$NON-NLS-2$
								.append(
										Messages
												.getString("GlobalActions.PhoneMobileLabelText")).append(actPatient.get("Natel")); //$NON-NLS-1$ //$NON-NLS-2$
						...
						MessageDialog
								.openError(
										mainWindow.getShell(),
										Messages
												.getString("GlobalActions.PrinterErrorTitle"), Messages.getString("GlobalActions.PrinterErrorMessage")); //$NON-NLS-1$ //$NON-NLS-2$
		...
		printEtikette = new Action(Messages
				.getString("GlobalActions.PrintLabel")) { //$NON-NLS-1$
			{
				setImageDescriptor(Desk
						.getImageDescriptor(Desk.IMG_PATIENTETIKETTE));
				setToolTipText(Messages
						.getString("GlobalActions.PrintLabelToolTip")); //$NON-NLS-1$
			}
				...
				if (pd != null) {
					...
					if (prn.startJob(Messages
						...
						gc
								.drawString(
										Messages
												.getString("GlobalActions.PatientIDLabelText") + actPatient.getPatCode(), 0, 0); //$NON-NLS-1$
						FontMetrics fmt = gc.getFontMetrics();
						y += fmt.getHeight();
						String pers = actPatient.getPersonalia();
						gc.drawString(pers, 0, y);
		...
		printBlatt = new Action(Messages.getString("GlobalActions.PrintEMR")) { //$NON-NLS-1$
			...
			public void run() {
				Patient actPatient = (Patient) ElexisEventDispatcher
						.getSelected(Patient.class);
				String printer = Hub.localCfg.get(
						"Drucker/Einzelblatt/Name", null); //$NON-NLS-1$
				String tray = Hub.localCfg.get(
						"Drucker/Einzelblatt/Schacht", null); //$NON-NLS-1$

				MessageBox mb = new MessageBox(Desk.getDisplay()
						.getActiveShell(), SWT.ICON_INFORMATION | SWT.OK
						| SWT.CANCEL);
				mb.setText("Papier einlegen"); //$NON-NLS-1$
				mb
						.setMessage("Bitte legen Sie im Einzelblatteinzug Papier ein."); //$NON-NLS-1$
				if (mb.open() == SWT.OK) {
					new TemplateDrucker("KG-Deckblatt", printer, tray).doPrint(actPatient); //$NON-NLS-1$
				}
			}
		};
		printRoeBlatt = new Action(Messages
				.getString("GlobalActions.PrintXRay")) { //$NON-NLS-1$
			@Override
			public void run() {
				Patient actPatient = (Patient) ElexisEventDispatcher
						.getSelected(Patient.class);
		...
		fixLayoutAction = new Action(
				Messages.getString("GlobalActions.LockPerspectives"), Action.AS_CHECK_BOX) { //$NON-NLS-1$
			{
				setToolTipText(Messages
						.getString("GlobalActions.LockPerspectivesToolTip")); //$NON-NLS-1$
			}

			@Override
			public void run() {
				// store the current value in the user's configuration
				Hub.userCfg.set(PreferenceConstants.USR_FIX_LAYOUT,
						fixLayoutAction.isChecked());
			}
		};
		makeBillAction = new Action(Messages
				.getString("GlobalActions.MakeBill")) { //$NON-NLS-1$
			@Override
			public void run() {
				Fall actFall = (Fall) ElexisEventDispatcher
				...
				if (actFall != null && mnd != null) {
					...
					ArrayList<Konsultation> lBehdl = new ArrayList<Konsultation>(
							bhdl.length);
		...
		moveBehandlungAction = new Action(Messages
				.getString("GlobalActions.AssignCase")) { //$NON-NLS-1$
			@Override
			public void run() {
				...
				if (k == null) {
					MessageDialog
							.openInformation(
									mainWindow.getShell(),
									Messages
											.getString("GlobalActions.NoKonsSelected"), Messages.getString("GlobalActions.NoKonsSelectedMessage")); //$NON-NLS-1$ //$NON-NLS-2$
					return;
				}

				SelectFallDialog dlg = new SelectFallDialog(mainWindow
						.getShell());
		...
		redateAction = new Action(Messages.getString("GlobalActions.Redate")) { //$NON-NLS-1$
			...
			public void run() {
				...
				DateSelectorDialog dlg = new DateSelectorDialog(mainWindow
						.getShell());
				if (dlg.open() == Dialog.OK) {
		...
		delFallAction = new Action(Messages
				.getString("GlobalActions.DeleteCase")) { //$NON-NLS-1$
			@Override
			public void run() {
				Fall actFall = (Fall) ElexisEventDispatcher
						.getSelected(Fall.class);
				if ((actFall != null) && (actFall.delete(false) == false)) {
					SWTHelper
							.alert(
									...
									Messages
											.getString("GlobalActions.CouldntDeleteCaseExplanation") + //$NON-NLS-1$
											Messages
													.getString("GlobalActions.93")); //$NON-NLS-1$
				}
				ElexisEventDispatcher.reload(Fall.class);
			}
		};
		delKonsAction = new Action(Messages
				.getString("GlobalActions.DeleteKons")) { //$NON-NLS-1$
			@Override
			public void run() {
				Konsultation k = (Konsultation) ElexisEventDispatcher
						.getSelected(Konsultation.class);
				if ((k != null) && (k.delete(false) == false)) {
					SWTHelper
							.alert(
									Messages
											.getString("GlobalActions.CouldntDeleteKons"), //$NON-NLS-1$
									Messages
											.getString("GlobalActions.CouldntDeleteKonsExplanation") + //$NON-NLS-1$
											Messages
													.getString("GlobalActions.97")); //$NON-NLS-1$
				}
		...
		openFallaction = new Action(Messages
				.getString("GlobalActions.EditCase")) { //$NON-NLS-1$

			@Override
			public void run() {
				try {
					Hub.plugin.getWorkbench().getActiveWorkbenchWindow()
							.getActivePage().showView(FallDetailView.ID);
		...
		reopenFallAction = new Action(Messages
				.getString("GlobalActions.ReopenCase")) { //$NON-NLS-1$
			@Override
			public void run() {
				Fall actFall = (Fall) ElexisEventDispatcher
						.getSelected(Fall.class);
		...
		neueKonsAction = new Action(Messages.getString("GlobalActions.NewKons")) { //$NON-NLS-1$
			{
				...
				setToolTipText(Messages
						.getString("GlobalActions.NewKonsToolTip")); //$NON-NLS-1$
			}
		...
		neuerFallAction = new Action(Messages
				.getString("GlobalActions.NewCase")) { //$NON-NLS-1$
			{
				setImageDescriptor(Desk.getImageDescriptor(Desk.IMG_NEW)); //$NON-NLS-1$
				setToolTipText(Messages
						.getString("GlobalActions.NewCaseToolTip")); //$NON-NLS-1$
			}
			...
			public void run() {
				...
				if (pat != null) {
					NeuerFallDialog nfd = new NeuerFallDialog(mainWindow
							.getShell(), null);
		...
		planeRechnungAction = new Action(Messages
				.getString("GlobalActions.plaBill")) { //$NON-NLS-1$
			public void run() {
		...
		if (actFall == null) {
			...
			if (actPatient == null) {
				SWTHelper
						.showError(
								Messages
										.getString("GlobalActions.CantCreateKons"), Messages.getString("GlobalActions.DoSelectPatient")); //$NON-NLS-1$ //$NON-NLS-2$
				return;
			}
			if (actFall == null) {
				...
				if (k != null) {
					...
					if (actFall == null) {
						SWTHelper
								.showError(
										Messages
												.getString("GlobalActions.CantCreateKons"), Messages.getString("GlobalActions.DoSelectCase")); //$NON-NLS-1$ //$NON-NLS-2$
					}
				} else {
					Fall[] faelle = actPatient.getFaelle();
					if ((faelle == null) || (faelle.length == 0)) {
						actFall = actPatient.neuerFall(Fall
	...
	/**
	 * Return a PrinterData object according to the given type (e. g.
	 * "Etiketten") and the user settings. Shows a printer selection dialog if
	 * required.
	 * 
	 * @param type
	...
	private PrinterData getPrinterData(final String type) {
		...
		if (choose || StringTool.isNothing(printer)
				|| StringTool.isNothing(driver)) {
	...
	/**
	 * Verfügbarkeit der einzelnen Menuepunkte an den angemeldeten Anwender
	 * anpassen Menueeinstellungen wiederherstellen
	 */
	public void adaptForUser() {
		...
		if (Hub.actUser != null) {
			boolean fixLayoutChecked = Hub.userCfg.get(
					PreferenceConstants.USR_FIX_LAYOUT,
			...
			fixLayoutAction
					.setChecked(PreferenceConstants.USR_FIX_LAYOUT_DEFAULT);
	 ...
	 * Site's HandlerService, i. e. binds the action to the command so that key
	 * bindings get activated. You need to set the action's actionDefinitionId
	 * to the command id.
	 * 
	 * @param action
	 *            the action to activate. The action's actionDefinitionId must
	 *            have been set to the command's id (using
	 *            <code>setActionDefinitionId()</code>)
	 * @param part
	 *            the view this action should be registered for
	 */
	public static void registerActionHandler(final ViewPart part,
	...
	class ChangeMandantDialog extends TitleAreaDialog {
		...
		public Control createDialogArea(final Composite parent) {
			lbMandant = new org.eclipse.swt.widgets.List(parent, SWT.BORDER
					| SWT.SINGLE);
			lbMandant
		...
		public void create() {
			...
			setMessage(Messages
					.getString("GlobalActions.ChangeMandatorMessage")); //$NON-NLS-1$
		}

...
/**
 * A job that does not execute immediately on launch but waits if there comes
 * another call - e.g a key press of the user. The time the job waits is
 * configurable but can also be adaptive - it remembers the time between earlier
 * calls and decides accordingly, how long it should wait next time. The use if
 * this class is to prevent lengthy operations to run unnecessarily - only the
 * last of a series of calls will be executed
 * 
 * @author gerry
 * 
 */
public class DelayableJob extends Job {
	...
	/**
	 * Launch the job after a specified delay. If a re-launch occurs within the
	 * delay time, the counter is reset and will wait for the specified time
	 * again. If the Job was already launched when a re-launch occurs, the Job
	 * will be stopped if possible.
	 * 
	 * If the delay is DELAY_ADAPTIVE, the Job will try to find the optimal
	 * delay by analyzing earlier calls. So different typing speeds of different
	...
	public void launch(int delayMillis) {
		...
		if (bDebug) {
			...
			sb.append("DelayableJob: ").append("delayMillis: ")
					.append(delayMillis).append("\n").append("lastDelay: ")
					.append(lastDelay).append("\n").append("jobstate: ")
					.append(getState()).append("\n").append("lastcall: ")
					.append(lastCall).append("\n")
	 ...
	 * @param key
	 *            the unique key
	 * @return the object associated with this key. This can be null if no such
	 *         object exists, or if null was associated with this key
	 */

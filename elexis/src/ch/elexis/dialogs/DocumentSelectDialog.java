...
public class DocumentSelectDialog extends TitleAreaDialog {
	...
	/**
	 * select an existing document out of the list of all documtents of the
	 * given mandator
	 */
	...
	/**
	 * Create a new DocumentSelector. If the user clicks OK, the selected Brief
	 * will be in result.
	 * 
	 * @param p
	...
	public void create() {
		...
		case TYPE_LOAD_DOCUMENT:
			...
			setMessage(Messages
					.getString("DocumentSelectDialog.pleaseSelectDocument")); //$NON-NLS-1$
			getShell().setText(OPEN_DOCUMENT);
			break;
		case TYPE_CREATE_DOC_WITH_TEMPLATE:
			setTitle(Messages
					.getString("DocumentSelectDialog.createLetterWithTemplate")); //$NON-NLS-1$
			setMessage(Messages
					.getString("DocumentSelectDialog.selectTemplate")); //$NON-NLS-1$
			getShell()
					.setText(
							Messages.getString("DocumentSelectDialog.schooseTemplateForLetter")); //$NON-NLS-1$
			break;
		case TYPE_LOAD_TEMPLATE:
			setTitle(Messages.getString("DocumentSelectDialog.openTemplate")); //$NON-NLS-1$
			setMessage(Messages
					.getString("DocumentSelectDialog.pleaseSelectTemplateFromList")); //$NON-NLS-1$
			getShell().setText(
					Messages.getString("DocumentSelectDialog.openTemplate")); //$NON-NLS-1$
			break;
		case TYPE_LOAD_SYSTEMPLATE:
			setTitle(Messages.getString("DocumentSelectDialog.loadSysTemplate")); //$NON-NLS-1$
			setMessage(Messages
					.getString("DocumentSelectDialog.sysTemplateExplanation")); //$NON-NLS-1$
			getShell().setText(
					Messages.getString("DocumentSelectDialog.loadTemplate")); //$NON-NLS-1$
		}
	}
	...
	protected Control createDialogArea(Composite parent) {
		...
		if ((type & TEMPLATE) != 0) {
			new Label(ret, SWT.NONE).setText(Messages
					.getString("DocumentSelectDialog.subject")); //$NON-NLS-1$
		...
		tv.setContentProvider(new IStructuredContentProvider() {
			...
			public Object[] getElements(Object inputElement) {
				...
				if (type == TYPE_LOAD_DOCUMENT) {
					...
					String sys = type == TYPE_LOAD_SYSTEMPLATE ? Query.EQUALS
							: Query.NOT_EQUAL;
		...
		menu.add((type & TEMPLATE) != 0 ? deleteTemplateAction
				: deleteTextAction);
	...
	private void makeActions() {
		...
		deleteTemplateAction = new Action(DELETE_TEMPLATE) {
			@Override
			public void run() {
				Brief sel = (Brief) ((IStructuredSelection) tv.getSelection())
						.getFirstElement();
				if (MessageDialog
						.openConfirm(
								getShell(),
								...
								MessageFormat.format(
										Messages.getString("DocumentSelectDialog.reallyDeleteTemplate"), sel.getBetreff())) //$NON-NLS-1$
				== true) {
					sel.delete();
					tv.refresh();
				}
			...
			public void run() {
				...
				if (MessageDialog
						.openConfirm(
								...
								DELETE_DOCUMENT,
								MessageFormat.format(
										Messages.getString("DocumentSelectDialog.reallyDeleteDocument"), sel.getBetreff())) //$NON-NLS-1$
				== true) {

 ...
 *    
 *  $Id: ITextPlugin.java 1658 2007-01-24 15:58:04Z rgw_ch $
 *******************************************************************************/
...
/**
 * Contract for embedding a text plugin
 * Warning: Preliminary interface
 *
 */
public interface ITextPlugin extends IExecutableExtension{

	public static enum PageFormat{USER,A4,A5};
	...
	/**
	 * create a document from a byte array. The array contains the document in the (arbitrary) specific format
	 * of the text component. Prefarably, but not necessarily OpenDocumentFormat (odf)
	 * @param bs the byte array with the document in a fotmat the compnent can interpret
	 ...
	 * Replace the found pattern with the replacment String received from ReplaceCallback.
	 * @param pattern a regular expression 
	 * @param cb a ReplaceCallback or null if no Replacement should be performed
	 * @return true if pattern was found at least once
	 */
	public boolean findOrReplace(String pattern, ReplaceCallback cb);

	/**
	 * Store the document into the byte array. The format can be any component specific type. The only
	 * requiremend is, that loadFromByteArray() must be able to recreate the document out of this byte array
	 * identically.
	 ...
	 * @param place a regular expression in the document, that will be replaced by the table. Only the first
	 * match will be used.
	 ...
	 * @param x,y,w,h position and size of the rectangle relative to the page bounds. Measured as millimeters.
	 * The effective position and size must match the given values at +- 2mm.
	 ...
	 * the inserted Text will replace the found string. 
	...
	/**
	 * Print the document
	 * @param toPrinter Name of the Printers or null (then to default printer)
	 * @param toTray Name of the tray or null (then tray as defined y the driver or the template) 

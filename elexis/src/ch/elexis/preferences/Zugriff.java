 ...
 *    
 *  $Id: Zugriff.java 4945 2009-01-13 17:50:00Z rgw_ch $
 *******************************************************************************/
...
/*
 * public class Zugriff extends FieldEditorPreferencePage implements IWorkbenchPreferencePage {
 * 
 * private static final String[] fields={ACTION_EXIT,ACTION_PREFS,ACTION_LOGIN, ACTION_CONNECT,
 ...
 * "Programm beenden: ","Einstellungsdialog: ","Benutzer wechseln", "Datenbankverbindung ändern",
 ...
 * 
 * public Zugriff() { super(GRID); setPreferenceStore(new SettingsPreferenceStore(new
 * InMemorySettings())); setDescription("Zuweisung von Rechten"); }
 * 
 * @Override protected void createFieldEditors() { String
 * grps=Hub.globalCfg.get(PreferenceConstants.ACC_GROUPS, "Admin"); String[] grp=grps.split(",");
 * for(int i=0;i<fields.length;i++){ addField(new
 * ComboFieldEditor(fields[i],texte[i],grp,getFieldEditorParent())); //addField(new
 * StringFieldEditor(fields[i],texte[i],getFieldEditorParent())); } } public void init(IWorkbench
 * workbench) { for(String s:fields){ List<String> g=Hub.acl.groupsForGrant(s);
 * getPreferenceStore().setValue(s,StringTool.join(g,",")); } }
 * 
 * @Override public boolean performOk() { super.performOk();
 * 
 * for(String grant:fields){ String pref=getPreferenceStore().getString(grant); for(String
 * s:pref.split(",")){ Hub.acl.deleteGrant(grant); Hub.acl.grant(s,grant); } } Hub.acl.flush();
 * return true; }
 * 
 * 
 * @Override protected void performDefaults() { Hub.acl.reset(); this.initialize(); }
 * 
 * }
 */

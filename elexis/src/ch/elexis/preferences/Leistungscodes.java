...
public class Leistungscodes extends PreferencePage implements
	...
	List<IConfigurationElement> ll = Extensions
			.getExtensions("ch.elexis.Verrechnungscode"); //$NON-NLS-1$
	String[] systeme = Hub.globalCfg.nodes(CFG_KEY);
	Table table;
	String[] tableCols = { Messages.Leistungscodes_nameOfBillingSystem,
			Messages.Leistungscodes_billingSystem,
			Messages.Leistungscodes_defaultOutput,
			Messages.Leistungscodes_multiplier };
	...
	protected Control createContents(final Composite parent) {
		...
		SWTHelper.createHyperlink(ret, Messages.Leistungscodes_new,
				new HyperlinkAdapter() {
					@Override
					public void linkActivated(final HyperlinkEvent e) {
						AbrechnungsTypDialog at = new AbrechnungsTypDialog(
								getShell(), null);
						if (at.open() == Dialog.OK) {
							...
							Hub.globalCfg.set(
									key + "/leistungscodes", result[1]); //$NON-NLS-1$
							Hub.globalCfg.set(
									key + "/standardausgabe", result[2]); //$NON-NLS-1$
		...
		SWTHelper.createHyperlink(ret, Messages.Leistungscodes_delete,
				new HyperlinkAdapter() {
					@Override
					public void linkActivated(HyperlinkEvent e) {
						TableItem sel = table.getSelection()[0];
						String bName = sel.getText(0);
						if (SWTHelper.askYesNo(MessageFormat.format(
								Messages.Leistungscodes_reallyDelete, bName),
						...
						if (s1.equals(ssel)) {
							...
							if (at.open() == Dialog.OK) {
								...
								Hub.globalCfg.set(
										key + "/standardausgabe", result[2]); //$NON-NLS-1$
								Hub.globalCfg.set(
										key + "/bedingungen", result[3]); //$NON-NLS-1$
								Hub.globalCfg.set(
										key + "/fakultativ", result[4]); //$NON-NLS-1$
								Hub.globalCfg.set(key + "/unused", result[5]); //$NON-NLS-1$
								systeme = Hub.globalCfg.nodes(CFG_KEY);
								reload();
							}
	...
	/**
	 * (re)build the table for the billing systems, populate the table with
	 * items
	 */
	public void reload() {
		...
		if (systeme != null) {
			for (String s : systeme) {
				...
				String tp = PersistentObject.getConnection().queryString(
						sql.toString());
				if (StringTool.isNothing(tp)) {
					if (Hub.getSystemLogLevel() > Log.INFOS) {
						SWTHelper.alert(
								Messages.Leistungscodes_didNotFindMulitplier,
	...
	class AbrechnungsTypDialog_InputDialog extends Dialog {
		 ...
		 *            ListDisplay<String>[], array of ListDisplay's containing
		 *            item names which should not be duplicated
		 * @param text
		 *            String, the initial text value for the text editor field
		 *            (for the item values)
		 ...
		 *            String[], list of types to which this field can be changed
		 *            to - if null, then display no combo
		 */
		public AbrechnungsTypDialog_InputDialog(Shell parentShell,
				final String dialogTitle, final String dialogMessage,
				final String initialValue,
				ListDisplay<String>[] noDuplicatesList, String text,
				boolean isNumericChecked, boolean isStyledChecked,
				boolean isMultilineChecked, String[] changeTypeItems) {
			super(parentShell);
			cCurrentFieldType = dialogTitle.replaceAll(
					"\\.\\.\\..*", StringTool.leer) + "..."; //$NON-NLS-1$ //$NON-NLS-2$;
		...
		protected void calcFieldPresence() {
			...
			if (cCurrentFieldType
					.equalsIgnoreCase(Messages.Leistungscodes_contactHL)) {
				...
				cHasMultilineCheckbox = true;
					...
					.equalsIgnoreCase(Messages.Leistungscodes_dateHL)) {
			} else if (cCurrentFieldType
					.equalsIgnoreCase(Messages.Leistungscodes_comboHL)) {
				cHasTextEditor = true;
				cHasNumericCheckbox = true;
			} else if (cCurrentFieldType
					.equalsIgnoreCase(Messages.Leistungscodes_listHL)) {
				cHasTextEditor = true;
				cHasNumericCheckbox = true;
			} else if (cCurrentFieldType
					.equalsIgnoreCase(Messages.Leistungscodes_checkboxHL)) {
				cHasTextEditor = true;
				cTextNeeded = false;
			} else if (cCurrentFieldType
					.equalsIgnoreCase(Messages.Leistungscodes_radioHL)) {
				cHasTextEditor = true;
				cHasNumericCheckbox = true;
			}
		}
		 ...
		 * @initialValue the initial input value, or null if none (equivalent to
		 *               the empty string)
		 */
		protected Control createDialogArea(final Composite parent) {
			...
			if (cHasTextEditor) {
				...
				teLabel.setLayoutData(SWTHelper.getFillGridData(2, true, 1,
						false));
				tTextEditor = new Text(ret, SWT.BORDER + SWT.MULTI
						+ SWT.V_SCROLL);
				tTextEditor.setLayoutData(SWTHelper.getFillGridData(2, true, 1,
						true));
			...
			if (cHasNumericCheckbox) {
				...
				chNumeric.setLayoutData(SWTHelper.getFillGridData(2, true, 1,
						false));
			}
			if (cHasMultilineCheckbox) {
				chMultiline = new Button(ret, SWT.CHECK);
				chMultiline
						.setText(Messages.Leistungscodes_SaveAsMultiplelinesText);
				chMultiline.setSelection(cIsMultilineChecked);
				chMultiline.setLayoutData(SWTHelper.getFillGridData(2, true, 1,
						false));
			}
			// *** +++++ styled text not yet ok - just for testing now
			if (cHasStyledCheckbox
					& (Hub.actUser.getLabel().equalsIgnoreCase("a"))) { //$NON-NLS-1$
				...
				chStyled.setLayoutData(SWTHelper.getFillGridData(2, true, 1,
						false));
			}
			...
			if (cChangeTypeItems != null) {
				...
				new Label(ret, SWT.SEPARATOR | SWT.HORIZONTAL)
						.setLayoutData(SWTHelper.getFillGridData(2, true, 1,
								false));
				...
				currentType.setText(cCurrentFieldType.replaceAll(
						"\\.\\.\\.", StringTool.leer)); //$NON-NLS-1$);
				...
				changeCombo.addSelectionListener(new SelectionListener() {
					...
					public void widgetSelected(SelectionEvent e) {
						...
						cDialogTitle = cCurrentFieldType.replaceAll(
								"\\.\\.\\.", StringTool.leer); //$NON-NLS-1$
						calcFieldPresence();
		...
		protected void okPressed() {
			...
			result[1] = (tTextEditor == null || tTextEditor.isDisposed()) ? StringTool.leer
					: tTextEditor.getText();
			result[2] = (chNumeric == null || chNumeric.isDisposed()) ? "0" : ((chNumeric.getSelection()) ? "1" : "0"); //$NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$
			result[3] = (chMultiline == null || chMultiline.isDisposed()) ? "0" : ((chMultiline.getSelection()) ? "1" : "0"); //$NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$
			result[4] = (chStyled == null || chStyled.isDisposed()) ? "0" : ((chStyled.getSelection()) ? "1" : "0"); //$NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$
			...
			if (cHasTextEditor) {
				if (cTextNeeded) {
					if ((result[1].length() > 4)
						...
						if ((tmp.isEmpty())
								|| (tmp.split(DEFINITIONSDELIMITER).length < 2)) {
							errorString = errorString
									+ Messages.Leistungscodes_ErrorAtLeast2Items;
						}
						if (!tmp.isEmpty()) {
							if ((tmp.substring(0, 1)
									.equalsIgnoreCase(DEFINITIONSDELIMITER))
									|| (tmp.indexOf(DEFINITIONSDELIMITER
											+ DEFINITIONSDELIMITER) >= 0)) {
								errorString = errorString
										+ Messages.Leistungscodes_ErrorNoEmptyItemsAllowed;
							}
						}
						if ((result[1].indexOf(ARGUMENTSSDELIMITER) >= 0)
							...
							errorString = errorString
									+ Messages.Leistungscodes_ErrorItemsNoSpecialChars;
						}
					}
				}
			}
			...
			if (errorString.isEmpty()) {
				...
				SWTHelper.alert(Messages.Leistungscodes_ErrorMessageTitlebar,
						errorString);
			}
		}
	...
	class AbrechnungsTypDialog extends TitleAreaDialog {
		...
		protected Control createDialogArea(final Composite parent) {
			...
			new Label(upperPartComp, SWT.NONE)
					.setText(Messages.Leistungscodes_nameLabel);
			...
			new Label(upperPartComp, SWT.NONE)
					.setText(Messages.Leistungscodes_billingSystemLabel);
			...
			new Label(upperPartComp, SWT.NONE)
					.setText(Messages.Leistungscodes_defaultOutputLabel);
			...
			leftMiddlePart.setLayoutData(SWTHelper.getFillGridData(1, true, 1,
					true));
			Composite rightMiddlePart = new Composite(middlePartComp, SWT.NONE);
			rightMiddlePart.setLayout(grid1);
			rightMiddlePart.setLayoutData(SWTHelper.getFillGridData(1, true, 1,
					true));
			...
			new Label(rightMiddlePart, SWT.NONE)
					.setText(Messages.Leistungscodes_caseConstants);
			ldConstants = new ListDisplay<String>(rightMiddlePart, SWT.NONE,
					new ListDisplay.LDListener() {
						...
						public void hyperlinkActivated(String l) {
							...
							InputDialog inp = new InputDialog(getShell(), l
									+ Messages.Leistungscodes_add, msg,
							...
							if (inp.open() == Dialog.OK) {
								...
								if (req.length != 2) {
									SWTHelper
											.showError(
													Messages.Leistungscodes_badEntry,
									...
									if (StringTool.isNothing(bs)) {
										SWTHelper
												.showError(
														Messages.Leistungscodes_badEntryCaptiob,
														Messages.Leistungscodes_badEntryText);
									} else {
										Fall.addBillingSystemConstant(bs,
												inp.getValue());
									}
								}
							}
			...
			ldConstants.setLayoutData(SWTHelper.getFillGridData(1, true, 1,
					true));
			...
			// *** separator
			Label separator = new Label(middlePartComp, SWT.SEPARATOR
					| SWT.HORIZONTAL);
			separator.setLayoutData(SWTHelper
			...
			FieldDefsDisplay fdReq = new FieldDefsDisplay(lowerPartComp,
					SWT.BORDER, data);
			...
			FieldDefsDisplay fdOpt = new FieldDefsDisplay(lowerPartComp,
					SWT.BORDER, data);
			...
			if (Hub.acl.request(AccessControlDefaults.CASE_DEFINE_SPECIALS) == true) {
				...
				FieldDefsDisplay fdUnused = new FieldDefsDisplay(lowerPartComp,
						SWT.BORDER, data);
			...
			fdReq.addMoveToAction(ldRequirements,
					Messages.Leistungscodes_moveItemToRequiredData,
					Desk.IMG_MOVETOUPPERLIST, false);
			fdReq.addMoveToAction(ldOptional,
					Messages.Leistungscodes_moveItemToOptionalData,
					Desk.IMG_MOVETOLOWERLIST, true);
			...
			fdOpt.addMoveToAction(ldRequirements,
					Messages.Leistungscodes_moveItemToRequiredData,
					Desk.IMG_MOVETOUPPERLIST, true);
			fdOpt.addMoveToAction(ldOptional,
					Messages.Leistungscodes_moveItemToOptionalData,
	...
	class FieldDefsDisplay extends Composite {
		...
		List<ListDisplay<String>> moveTo_DestinationLists = new ArrayList<ListDisplay<String>>(
				30);
		IAction[] actions = new IAction[LASTFIXEDITEM + 1];

		/**
		 * set the noDuplicatesList value, which contains the Lists containing
		 * items not to be duplicated for the "move to other list action"
		 * 
		 * @param listArray
		 */
		...
		/**
		 * set the noDuplicatesList value, which contains the Lists containing
		 * items not to be duplicated for the "create new field action"
		 * 
		 * @param listArray
		 */
		 ...
		 * @param image
		 */
		public void addMoveToAction(ListDisplay<String> destinationList,
				String toolTipText, String image, boolean enabled) {
			if (toolBar != null) {
				...
				ListPopUpMenuAction newAction = new ListPopUpMenuAction(
						toolTipText, image, toolTipText, newItemIx,
						destinationList, enabled);
				IAction[] extended = new IAction[LASTFIXEDITEM
						+ additionalItemsCount + 1 + 1];
				System.arraycopy(actions, 0, extended, 0, LASTFIXEDITEM
		...
		public FieldDefsDisplay(Composite parent, int style, String[] listItems) {
			...
			listDisplay = new ListDisplay<String>(this, SWT.NONE,
					new ListDisplay.LDListener() {
			...
			listDisplay.addHyperlinks(Messages.Leistungscodes_contactHL,
					Messages.Leistungscodes_textHL,
					Messages.Leistungscodes_dateHL,
					Messages.Leistungscodes_comboHL,
					Messages.Leistungscodes_listHL,
					Messages.Leistungscodes_checkboxHL,
					Messages.Leistungscodes_radioHL);
			listDisplay.setLayoutData(SWTHelper.getFillGridData(1, true, 1,
					true));
			...
			navigator.setLayoutData(SWTHelper
					.getFillGridData(1, false, 1, true));
			...
			addToolItem(toolBar, Desk.IMG_ARROWUP,
					Messages.Leistungscodes_moveItemUp, MOVEITEMUP, true);
			addToolItem(toolBar, Desk.IMG_ARROWDOWN,
					Messages.Leistungscodes_moveItemDown, MOVEITEMDOWN, true);
			addToolItem(toolBar, null, Messages.Leistungscodes_moveItemDown,
					-1, false);
			addToolItem(toolBar, Desk.IMG_REMOVEITEM,
					Messages.Leistungscodes_deleteItem, DELETEITEM, true);
			addToolItem(toolBar, Desk.IMG_EDIT,
					Messages.Leistungscodes_editItem, EDITITEM, true);
			addToolItem(toolBar, null, Messages.Leistungscodes_moveItemDown,
					-1, false);

			// ****** create popupmenu for this list (same actions)
			ListPopUpMenuAction moveItemUpAction = new ListPopUpMenuAction(
					Messages.Leistungscodes_moveItemUp, Desk.IMG_ARROWUP,
					Messages.Leistungscodes_moveItemUp, MOVEITEMUP,
					listDisplay, true);
			ListPopUpMenuAction moveItemDownAction = new ListPopUpMenuAction(
					Messages.Leistungscodes_moveItemDown, Desk.IMG_ARROWDOWN,
					Messages.Leistungscodes_moveItemDown, MOVEITEMDOWN,
					listDisplay, true);
			ListPopUpMenuAction delItemAction = new ListPopUpMenuAction(
					Messages.Leistungscodes_deleteAction, Desk.IMG_REMOVEITEM,
					Messages.Leistungscodes_removeConstraintTT, DELETEITEM,
					listDisplay, true);
			ListPopUpMenuAction changeItemAction = new ListPopUpMenuAction(
					Messages.Leistungscodes_editItem, Desk.IMG_EDIT,
					Messages.Leistungscodes_editItem, EDITITEM, listDisplay,
					true);
			...
			if (child != null) {
				...
				child.addMouseListener(new MouseListener() {
					...
					public void mouseDown(MouseEvent e) {
						...
						if (e.button != 1) {
							int itemIx = e.y / child.getItemHeight()
									+ child.getTopIndex();
		 ...
		 * @param actionType
		 *            the action type (MOVEITEMUP, MOVEITEMDOWN, DELETEITEM,
		 *            EDITITEM, LASTFIXEDITEM, ...)
		 * @param enabled
		 *            enabled or disabled...
		 */
		private void addToolItem(ToolBar parent, String image,
				String toolTipText, int actionType, boolean enabled) {
			if (actionType == -1) {
		 ...
		 * @return
		 */
		private org.eclipse.swt.widgets.List getListPart(
		...
		public void changeItemType() {
			if (listDisplay.getSelection() == null) {
				SWTHelper.alert(StringTool.leer,
						Messages.Leistungscodes_mustSelectALine);
			} else {
			}
		}
		...
		public void moveListItem(int step) {
			if (listDisplay.getSelection() == null) {
				...
				if (conditionOk) {
					allRequirements.set(selIx,
							allRequirements.get(selIx + step));
		...
		public void deleteListItem() {
			...
			if (sel == null) {
				...
				if (SWTHelper.askYesNo(StringTool.leer,
					...
					if ((deletedList != null)
							&& (!deletedList.equals(listDisplay))) {
						deletedList.add(sel);
					}
		...
		class toolBarSelectionListener implements SelectionListener {
			@Override
			public void widgetSelected(SelectionEvent e) {
				int type = (Integer) (((ToolItem) (e.getSource()))
						.getData("type")); //$NON-NLS-1$
				...
				default:
					if (type > LASTFIXEDITEM) {
						moveItemToOtherList(
								listDisplay,
								moveTo_DestinationLists.get(type
		...
		class ListPopUpMenuAction extends Action {
			...
			public ListPopUpMenuAction(String actionName, String image,
					String toolTipText, int actionType,
					ListDisplay<String> listDisplay, boolean enabled) {
				super(actionName);
			...
			public void run() {
				...
				default:
					if (actionType > LASTFIXEDITEM) {
						moveItemToOtherList(
								listDisplay,
								moveTo_DestinationLists.get(actionType
										- LASTFIXEDITEM - 1), noDuplicatesList);
					}
					break;
				}
		 ...
		 * the object is the String in the form
		...
		public String _FieldsGetLabel(final Object o) {
			...
			if (l.length > 1) {
				...
				String opt = (l.length >= 3) ? "   (" + l[2].replaceAll(ITEMDELIMITER, "; ") + ")" : StringTool.leer; //$NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$
				if (opt.trim().equalsIgnoreCase("(SQL)")) { //$NON-NLS-1$
					opt = (l.length >= 4) ? "   (SQL: " + l[3].replaceAll(ITEMDELIMITER, "; ") + ")" : StringTool.leer; //$NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$
		 ...
		 * 
		 * @param l
		 *            if creating a field, then this contains the field type
		 *            name
		 * @param fieldToBeChanged
		 *            if changing a field, then this contains the full
		 *            definition for the field to be changed
		 *            [fieldName]:[fieldType]:[itemsList]
		 */
		public void _FieldsHyperlinkActivated(final String l,
			...
			String[] changeItems = { Messages.Leistungscodes_contactHL,
					Messages.Leistungscodes_textHL,
					Messages.Leistungscodes_dateHL,
					Messages.Leistungscodes_comboHL,
					Messages.Leistungscodes_listHL,
					Messages.Leistungscodes_checkboxHL,
					Messages.Leistungscodes_radioHL };
			AbrechnungsTypDialog_InputDialog inputDlg = new AbrechnungsTypDialog_InputDialog(
					getShell(),
					ll
							+ (isChanging ? Messages.Leistungscodes_changeTextInTitleBar
									: Messages.Leistungscodes_add), msg,
					fieldName, noDuplicatesListCreate, optionsIn,
					isNumericChecked, isStyledChecked, isMultilineChecked,
			...
			if (inputDlg.open() == Dialog.OK) {
				...
				if ((options.length() > 4)
						&& (options.substring(0, 4).equalsIgnoreCase("SQL:"))) { //$NON-NLS-1$
		...
		/**
		 * moving items from one list to another list the moved item MUST NOT be
		 * in the destination list - will not be allowed the moved item MUST NOT
		 * be in one of the noDuplicatesDisplays list - will not be allowed
		 * 
		 * @param listDisplay
		 ...
		 *            an array of ListDisplay's which contain the values which
		 *            should not be duplicated
		 */
		@SuppressWarnings("unchecked")
		public void moveItemToOtherList(ListDisplay<String> listDisplay,
				ListDisplay<String> destDisplay,
				ListDisplay<String>... noDuplicatesDisplays) {
			// no duplicates allowed for destination list
			String sel = listDisplay.getSelection();
			if (sel.isEmpty())
				return;
			java.util.ArrayList<String> allItems = (ArrayList<String>) ((ArrayList<String>) destDisplay
					.getAll()).clone();
			...
			for (ListDisplay<String> noDuplicatesList : noDuplicatesDisplays) {
				...
				for (int li = 0; li < allItems2.size(); li++) {
					allItems2.set(li,
							allItems2.get(li).split(ARGUMENTSSDELIMITER)[0]);
				}

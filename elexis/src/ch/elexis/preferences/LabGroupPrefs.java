...
public class LabGroupPrefs extends PreferencePage implements
	...
	protected Control createContents(Composite parent) {
		...
		label.setText(Messages.LabGroupPrefs_ExplanationsLine1
				+ Messages.LabGroupPrefs_ExplanationsLine2
				...
				if (actGroup != null) {
					if (SWTHelper
							.askYesNo(
									...
									MessageFormat
											.format(Messages.LabGroupPrefs_reallyDeleteGroup,
													actGroup.getLabel())))
						;
					{
		...
		itemsViewer = new ListViewer(bottomArea, SWT.MULTI | SWT.H_SCROLL
				| SWT.V_SCROLL | SWT.BORDER);
		itemsViewer.getControl().setLayoutData(
		...
		addItemButton.setLayoutData(SWTHelper
		...
		removeItemButton.setLayoutData(SWTHelper.getFillGridData(1, true, 1,
				false));
		...
		removeItemButton.addSelectionListener(new SelectionListener() {
			public void widgetSelected(SelectionEvent e) {
				if (actGroup != null) {
					IStructuredSelection sel = (IStructuredSelection) itemsViewer
							.getSelection();
	...
	class ItemsSelectionDialog extends TitleAreaDialog {
		...
		protected Control createDialogArea(Composite parent) {
			...
			viewer = new ListViewer(composite, SWT.MULTI | SWT.H_SCROLL
					| SWT.V_SCROLL | SWT.BORDER);
		...
		class ItemsContentProvider implements IStructuredContentProvider {
			public Object[] getElements(Object inputElement) {
				...
				query.orderBy(false, new String[] { "Gruppe", "prio", "titel" }); //$NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$

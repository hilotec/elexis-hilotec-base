...
public abstract class CodeSelectorFactory implements IExecutableExtension {
		...
		if (list != null) {
			for (IConfigurationElement ic : list) {
				try {
					...
					ICodeElement codeElement = (ICodeElement) po
							.createTemplate(codeSelectorFactory
									.getElementClass());
					if (codeElement == null) {
	...
	/**
	 * If the user resizes the parts of the selector, we'll remember the new
	 * size
	 * 
	 * @author gerry
	 * 
	...
	public static class cPage extends Composite {
		...
		cPage(final CTabFolder ctab, final ICodeElement codeElement,
			...
			if (actions != null) {
				...
				menu.addMenuListener(new IMenuListener() {
					public void menuAboutToShow(IMenuManager manager) {
						Iterable<IAction> actions = codeElement
								.getActions(null);
			...
			new PersistentObjectDragSource(lbUserMFU,
					new DragEnabler(lbUserMFU));
			new PersistentObjectDragSource(lbPatientMFU, new DragEnabler(
					lbPatientMFU));
		...
		public void refresh() {
			...
			lUserMFU = Hub.actUser
					.getStatForItem(template.getClass().getName());
			...
			for (int i = 0; i < ITEMS_TO_SHOW_IN_MFU_LIST; i++) {
				...
				PersistentObject po = Hub.poFactory.createFromString(lUserMFU
						.get(i));
				alUser.add(po);
			...
			for (int i = 0; i < ITEMS_TO_SHOW_IN_MFU_LIST; i++) {
				...
				PersistentObject po = Hub.poFactory
						.createFromString(lPatientMFU.get(i));
				if (po != null) {
					alPatient.add(po);
	...
	static class DragEnabler implements
			PersistentObjectDragSource.ISelectionRenderer {
		List list;

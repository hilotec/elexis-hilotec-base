 ...
 * 
 * $Id: RechnungsBlatt.java 5970 2010-01-27 16:43:04Z rgw_ch $
 *******************************************************************************/
...
public class RechnungsBlatt extends Composite implements IActivationListener {
	...
	static final InputData[] rndata = {
			...
			new InputData(
					Messages.getString("RechnungsBlatt.billState"), Rechnung.BILL_STATE, //$NON-NLS-1$
					new LabeledInputField.IContentProvider() {
						...
						public void reloadContent(PersistentObject po,
								InputData ltf) {
							if (new RnDialogs.StatusAendernDialog(Hub.plugin
									.getWorkbench().getActiveWorkbenchWindow()
									.getShell(), (Rechnung) po).open() == Dialog.OK) {
			...
			new InputData(
					...
					new LabeledInputField.IContentProvider() {
						...
						public void reloadContent(PersistentObject po,
								InputData ltf) {
							try {
								if (new RnDialogs.BuchungHinzuDialog(Hub.plugin
										.getWorkbench()
								...
								SWTHelper
										.showError(
												"Buchung kann nicht hinzugefügt werden",
	...
	private ElexisEventListenerImpl eeli_rn = new ElexisEventListenerImpl(
			Rechnung.class, ElexisEvent.EVENT_CREATE | ElexisEvent.EVENT_DELETE
		...
		public void runInUi(ElexisEvent ev) {
			...
			case ElexisEvent.EVENT_DELETE:
				if (actRn != null
						&& actRn.getId().equals(ev.getObject().getId())) {
					actRn = null;
					display();
				}
				break;
	...
	public RechnungsBlatt(Composite parent, IViewSite site) {
		...
		IExpansionListener ecExpansionListener = new ExpansionAdapter() {
			...
			public void expansionStateChanging(final ExpansionEvent e) {
				...
				saveExpandedState(KEY_RECHNUNGSBLATT + src.getText(), e
						.getState());
			}
		...
		buchungen.setContentProvider(new IStructuredContentProvider() {
			...
			public Object[] getElements(Object inputElement) {
				Rechnung actRn = (Rechnung) ElexisEventDispatcher.getInstance()
						.getSelected(Rechnung.class);
				if (actRn == null) {
					return new String[] { Messages
							.getString("RechnungsBlatt.noBillSelected") //$NON-NLS-1$
					};
				}
						...
						if (actRn != null) {
							...
							if (konsultationen != null) {
								for (Konsultation konsultation : konsultationen) {
									...
									List<Verrechnet> leistungen = konsultation
											.getLeistungen();
									if (leistungen != null) {
			...
			public String getText(Object element) {
				if (element instanceof Konsultation) {
					...
					return konsultation.getLabel()
							+ " (" + sum.toString() + ")"; //$NON-NLS-1$ //$NON-NLS-2$
	...
	private void setExpandedState(ExpandableComposite ec, String field) {
		String mode = Hub.userCfg.get(UserSettings2.EXPANDABLE_COMPOSITES,
				UserSettings2.REMEMBER_STATE);
		if (mode.equals(UserSettings2.OPEN)) {
			ec.setExpanded(true);
		} else if (mode.equals(UserSettings2.CLOSED)) {
			ec.setExpanded(false);
		} else {
			String state = Hub.userCfg.get(UserSettings2.STATES + field,
					UserSettings2.CLOSED);
	...
	public void visible(boolean mode) {
		if (mode) {
			ElexisEventDispatcher.getInstance()
					.addListeners(eeli_rn, eeli_user);
		} else {
	...
	public void display() {
		...
		setExpandedState(ecBuchungen, KEY_RECHNUNGSBLATT
				+ ecBuchungen.getText());
		setExpandedState(ecBemerkungen, KEY_RECHNUNGSBLATT
				+ ecBemerkungen.getText());

...
public class KonsZumVerrechnenView extends ViewPart implements ISaveablePart2 {
	...
	ViewMenus menu;
	private IAction billAction, printAction, clearAction, wizardAction,
			refreshAction, detailAction;
	...
	public void createPartControl(final Composite parent) {
		...
		tvSel.getControl().setLayoutData(
		...
		tvSel.setLabelProvider(new LabelProvider() {
			...
			public String getText(final Object element) {
				return ((PersistentObject) ((Tree) element).contents)
						.getLabel();
			}

		});
		tvSel.addDropSupport(DND.DROP_MOVE | DND.DROP_COPY,
		...
		tvSel.addSelectionChangedListener(GlobalEventDispatcher.getInstance()
				.getDefaultListener());
		...
		tvSel.getControl().setMenu(
				selMenu.createContextMenu(tvSel.getControl()));
		menu = new ViewMenus(getViewSite());
		menu.createToolbar(refreshAction, wizardAction, printAction,
				clearAction, null, billAction);
					...
					progressService.runInUI(PlatformUI.getWorkbench()
						...
						public void run(final IProgressMonitor monitor) {
							...
							monitor.subTask(Messages
									.getString("KonsZumVerrechnenView.readIn")); //$NON-NLS-1$
							try {
	 ...
	 */
	@SuppressWarnings("unchecked")
	private Tree selectPatient(final Patient pat, final Tree tSource,
			final Tree tDest) {
	...
	@SuppressWarnings("unchecked")
	private Tree selectBehandlung(final Konsultation bh, final Tree tSource,
			final Tree tDest) {
		Fall f = bh.getFall();
		Patient pat = f.getPatient();
		Tree tPat = tDest.find(pat, false);
	...
	private void makeActions() {
		billAction = new Action(Messages
				.getString("KonsZumVerrechnenView.createInvoices")) { //$NON-NLS-1$
			{
			...
			public void run() {
				if (((StructuredSelection) tvSel.getSelection()).size() > 0) {
					if (!SWTHelper
							.askYesNo(
									...
									Messages
											.getString("KonsZumVerrechnenView.ReallyCreateBillsBody"))) { //$NON-NLS-1$
						return;
					}
				}
				 ...
				 * getViewSite().getService(ICommandService.class); try {
				 * Command command = cmdService.getCommand("bill.create");
				 * Parameterization px = new
				 * Parameterization(command.getParameter
				 * ("ch.elexis.RechnungErstellen.parameter" ), new
				 * TreeToStringConverter().convertToString(tSelection));
				 * ParameterizedCommand parmCommand = new
				 * ParameterizedCommand(command, new Parameterization[] { px });
				 * 
				 * handlerService.executeCommand(parmCommand, null);
				 * 
				 * } catch (Exception ex) { throw new
				 * RuntimeException("add.command not found"); }
				 */

				tvSel.refresh();
			}
		};
		clearAction = new Action(Messages
				.getString("KonsZumVerrechnenView.clearSelection")) { //$NON-NLS-1$
			{
				setImageDescriptor(Desk.getImageDescriptor("delete")); //$NON-NLS-1$
				setToolTipText(Messages
						.getString("KonsZumVerrechnenView.deleteList")); //$NON-NLS-1$
		...
		refreshAction = new Action(Messages
				.getString("KonsZumVerrechnenView.reloadAction")) { //$NON-NLS-1$
			{
				setImageDescriptor(Desk.getImageDescriptor(Desk.IMG_REFRESH));
				setToolTipText(Messages
						.getString("KonsZumVerrechnenView.reloadToolTip")); //$NON-NLS-1$
			}
		...
		wizardAction = new Action(Messages
				.getString("KonsZumVerrechnenView.autoAction")) { //$NON-NLS-1$
			{
				setImageDescriptor(Desk.getImageDescriptor(Desk.IMG_WIZARD));
				setToolTipText(Messages
						.getString("KonsZumVerrechnenView.autoToolTip")); //$NON-NLS-1$
			}

			@Override
			public void run() {
				KonsZumVerrechnenWizardDialog kzvd = new KonsZumVerrechnenWizardDialog(
				...
				if (kzvd.open() == Dialog.OK) {
					...
					try {
						progressService
								.runInUI(progressService, new Rechnungslauf(
										self, kzvd.bMarked, kzvd.ttFirstBefore,
										kzvd.ttLastBefore, kzvd.mAmount,
		...
		printAction = new Action(Messages
				.getString("KonsZumVerrechnenView.printSelection")) { //$NON-NLS-1$
			{
				setImageDescriptor(Desk.getImageDescriptor(Desk.IMG_PRINTER));
				setToolTipText(Messages
						.getString("KonsZumVerrechnenView.printToolTip")); //$NON-NLS-1$
			}
		...
		removeAction = new Action(Messages
				.getString("KonsZumVerrechnenView.removeFromSelection")) { //$NON-NLS-1$
			@SuppressWarnings("unchecked")
			@Override
			public void run() {
				IStructuredSelection sel = (IStructuredSelection) tvSel
						.getSelection();
				if (!sel.isEmpty()) {
		...
		expandSelAction = new Action(Messages
				.getString("KonsZumVerrechnenView.expand")) { //$NON-NLS-1$
			@SuppressWarnings("unchecked")
			@Override
			public void run() {
				IStructuredSelection sel = (IStructuredSelection) tvSel
		...
		expandSelAllAction = new Action(Messages
				.getString("KonsZumVerrechnenView.expandAll")) { //$NON-NLS-1$
			@Override
			public void run() {
				tvSel.expandAll();
			}
		...
		selectByDateAction = new Action(Messages
				.getString("KonsZumVerrechnenView.selectByDateAction")) { //$NON-NLS-1$
			TimeTool fromDate;
			TimeTool toDate;
			...
			public void run() {
				// select date
				SelectDateDialog dialog = new SelectDateDialog(getViewSite()
						.getShell());
						...
						progressService.runInUI(PlatformUI.getWorkbench()
								...
								new IRunnableWithProgress() {
									public void run(
										...
										doSelectByDate(monitor, fromDate,
												toDate);
									}
								}, null);
					} catch (Throwable ex) {
		...
		detailAction = new RestrictedAction(
				AccessControlDefaults.LSTG_VERRECHNEN, Messages
			...
			public void doRun() {
				...
				if ((sel != null) && (sel.length > 0)) {
					new VerrDetailDialog(getViewSite().getShell(),
							(Tree) sel[0]).open();
				}
			}
		};
	 ...
	 */
	@SuppressWarnings("unchecked")
	private void doSelectByDate(final IProgressMonitor monitor,
			final TimeTool fromDate, final TimeTool toDate) {
		...
		monitor
				.beginTask(
						Messages
								.getString("KonsZumVerrechnenView.selectByDateTask"), lAll.size() + 1); //$NON-NLS-1$
		for (Tree tP : lAll) {
			monitor.worked(1);
			for (Tree tF : (List<Tree>) tP.getChildren()) {
				...
				for (Tree tk : tK) {
					...
					if (actDate.isAfterOrEqual(date1)
							&& actDate.isBefore(date2)) {
						selectBehandlung((Konsultation) tk.contents, tAll,
								tSelection);
					}
				}
	...
	public class SelectDateDialog extends TitleAreaDialog {
		...
		public void create() {
			...
			setMessage(Messages
					.getString("SelectDateDialog.choosePeriodMessage")); //$NON-NLS-1$
			getShell().setText(
					Messages.getString("SelectDateDialog.description")); //$NON-NLS-1$
		}
		...
		protected Control createDialogArea(final Composite parent) {
			...
			new Label(com, SWT.NONE).setText(Messages
					.getString("SelectDateDialog.from")); //$NON-NLS-1$
			new Label(com, SWT.NONE).setText(Messages
					.getString("SelectDateDialog.to")); //$NON-NLS-1$
	...
	class SelectionPrintDialog extends TitleAreaDialog implements ICallback {
		...
		protected Control createDialogArea(final Composite parent) {
			...
			for (int i = 0; i < all.length; i++) {
				...
				for (Tree tFall : (Tree[]) tr.getChildren()
						.toArray(new Tree[0])) {
					Fall fall = (Fall) tFall.contents;
					sb
							.append(
									Messages
											.getString("KonsZumVerrechnenView.case")).append(fall.getLabel()); //$NON-NLS-1$
					for (Tree tRn : (Tree[]) tFall.getChildren().toArray(
							new Tree[0])) {
						Konsultation k = (Konsultation) tRn.contents;
						sb
								.append(
										Messages
												.getString("KonsZumVerrechnenView.kons")).append(k.getLabel()); //$NON-NLS-1$
					}
				}
		...
		public void create() {
			super.create();
			getShell().setText(
					Messages.getString("KonsZumVerrechnenView.billsList")); //$NON-NLS-1$
			setTitle(Messages
					.getString("KonsZumVerrechnenView.printListCaption")); //$NON-NLS-1$
			setMessage(Messages
					.getString("KonsZumVerrechnenView.printListMessage")); //$NON-NLS-1$

 ...
 *    				  actions added
 *    
 *  $Id: AccountView.java 2389 2007-05-18 12:32:36Z danlutz $
 *******************************************************************************/
...
public class AccountView extends ViewPart implements SelectionListener,
	...
	public void createPartControl(Composite parent) {
        ...
        accountViewer.setContentProvider(new IStructuredContentProvider() {
        	public Object[] getElements(Object inputElement) {
        		...
        		qa.orderBy(false, new String[]{"Datum"});
     ...
     /*   		
        		...
        		sql.append(actPatient.getWrappedId());
        		sql.append("  ORDER BY DATUM");
        		...
        		try {
        			...
        			while (res.next()) {
        				...
        				String sAmount = res.getString(2);
        				String remarks = res.getString(3);
        				
        				TimeTool date = new TimeTool(sDate);
        				Money amount = new Money(0);
        				amount.addCent(sAmount);
        				
        				AccountEntry entry = new AccountEntry(date, amount, remarks);
        				entries.add(entry);
        			}
        		} catch (Exception ex) {
        			ExHandler.handle(ex);
...
*/
    ...
    public void dispose() {
        ...
        accountViewer.removeSelectionChangedListener(GlobalEvents.getInstance().getDefaultListener());
        super.dispose();
    }
	...
	*/
	...
	private void makeActions(){
		...
		removePaymentAction=new Action("Buchung/Zahlung löschen"){
			...
			@Override
			public void run(){
				AccountTransaction at=(AccountTransaction)GlobalEvents.getInstance().getSelectedObject(AccountTransaction.class);
				if(at!=null){
					if(SWTHelper.askYesNo("Zahlung wirklich löschen?", "Bitte beachten Sie, dass manuelles Löschen von Zahlungen zu Buchhaltungsfehlern führen kann. Wirklich löschen?")){
						at.delete();
						setPatient(actPatient);
					}
				}

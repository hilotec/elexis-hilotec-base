...
public class BillSummary extends ViewPart implements IActivationListener,
		ElexisEventListener, ISaveablePart2 {
	...
	public void createPartControl(Composite parent) {
		...
		// bills
		billsViewer = new TableViewer(form.getBody(), SWT.SINGLE
				| SWT.FULL_SELECTION);
			...
			public Object[] getElements(Object inputElement) {
				if (actPatient == null) {
					return new Object[] { Messages
							.getString("BillSummary.NoPatientSelected") //$NON-NLS-1$
					};
				}
		...
		billsViewer.addSelectionChangedListener(GlobalEventDispatcher
	...
	public void dispose() {
		...
		billsViewer.removeSelectionChangedListener(GlobalEventDispatcher
				.getInstance().getDefaultListener());
		super.dispose();
	}
	...
	private void setGeneralText() {
		...
		if (totalLabel.isDisposed() || paidLabel.isDisposed()
				|| openLabel.isDisposed()) {
			return;
		}
	...
	private void makeActions() {
		exportToClipboardAction = new Action(Messages
				.getString("BillSummary.exportToClipboard")) { //$NON-NLS-1$
			{
				setToolTipText(Messages
						.getString("BillSummary.SummaryToClipboard")); //$NON-NLS-1$
			}

 ...
 * 
 */
public class PatientenListeView extends ViewPart implements
		IActivationListener, ISaveablePart2, HeartListener {
	...
	ElexisEventListenerImpl eeli_user = new ElexisEventListenerImpl(
	...
	public void createPartControl(final Composite parent) {
		...
		if (Hub.userCfg.get(PreferenceConstants.USR_PATLIST_SHOWDOB, true)) {
			fields
					.add(Patient.BIRTHDATE
							+ Query.EQUALS
							+ Messages
									.getString("PatientenListeView.PatientBirthdate")); //$NON-NLS-1$
		}
		...
		vc = new ViewerConfigurer(
				...
				plcp, new PatLabelProvider(), new DefaultControlFieldProvider(
						cv, fields.toArray(new String[0])),
		...
		menus.createControlContextMenu(cv.getViewerWidget().getControl(),
				new PatientMenuPopulator(this));
		plcp.startListening();
	...
	class PatLabelProvider extends DefaultLabelProvider implements
			ITableColorProvider {
		...
		public Image getColumnImage(final Object element, final int columnIndex) {
			if (element instanceof Patient) {
				...
				if (Reminder.findRemindersDueFor(pat, Hub.actUser, false)
						.size() > 0) {
	...
	private void makeActions() {
		...
		filterAction = new Action(Messages
				.getString("PatientenListeView.FilteList"), Action.AS_CHECK_BOX) { //$NON-NLS-1$
			{
				setImageDescriptor(Desk.getImageDescriptor(Desk.IMG_FILTER));
				setToolTipText(Messages
						.getString("PatientenListeView.FilterList")); //$NON-NLS-1$
			}
		...
		newPatAction = new Action(Messages
				.getString("PatientenListeView.NewPatientAction")) { //$NON-NLS-1$
			{
				setImageDescriptor(Desk.getImageDescriptor(Desk.IMG_NEW));
				setToolTipText(Messages
						.getString("PatientenListeView.NewPationtToolTip")); //$NON-NLS-1$
			}
			...
			public void run() {
				...
				if (!Hub.acl.request(AccessControlDefaults.PATIENT_INSERT)) {
					SWTHelper
							.alert(
									Messages
											.getString("PatientenListeView.MissingRights"), Messages.getString("PatientenListeView.YouMayNotCreatePatient")); //$NON-NLS-1$ //$NON-NLS-2$
					return;
				}
				...
				PatientErfassenDialog ped = new PatientErfassenDialog(
						getViewSite().getShell(), ctlFields);
	...
	public void activation(final boolean mode) {
		if (mode == true) {
			newPatAction.setEnabled(Hub.acl
					.request(AccessControlDefaults.PATIENT_INSERT));
			heartbeat();

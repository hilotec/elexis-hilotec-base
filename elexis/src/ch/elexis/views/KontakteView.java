...
public class KontakteView extends ViewPart implements ControlFieldListener,
	...
	public void createPartControl(Composite parent) {
		...
		loader.setOrderFields(new String[] { Kontakt.FLD_NAME1,
				Kontakt.FLD_NAME2, Kontakt.FLD_STREET, Kontakt.FLD_PLACE });
		vc = new ViewerConfigurer(loader, new KontaktLabelProvider(),
		...
		cv.addDoubleClickListener(new CommonViewer.DoubleClickListener() {
			public void doubleClicked(PersistentObject obj, CommonViewer cv) {
				try {
					...
					kdv.kb.catchElexisEvent(new ElexisEvent(obj,
							obj.getClass(), ElexisEvent.EVENT_SELECTED));
		...
		if (elements != null && elements.length > 0) {
			...
			if (element instanceof PersistentObject) {
				...
				ElexisEventDispatcher
						.fireSelectionEvent((PersistentObject) element);
			}
		}
	}
			...
			public void run() {
				...
				if (o != null) {
					...
					if (k.istPerson()) {
						...
						dup = new Person(p.getName(), p.getVorname(),
								p.getGeburtsdatum(), p.getGeschlecht());
					} else {
						Organisation org = Organisation.load(k.getId());
						dup = new Organisation(org.get(Organisation.FLD_NAME1),
		...
		createKontakt = new Action(Messages.getString("KontakteView.create")) { //$NON-NLS-1$
			...
			public void run() {
				String[] flds = cv.getConfigurer().getControlFieldProvider()
						.getValues();
				String[] predef = new String[] { flds[1], flds[2],
						StringConstants.EMPTY, flds[3], flds[4], flds[5] };
		...
		printList = new Action("Markierte Adressen drucken") {
			...
			public void run() {
				...
				if (sel != null && sel.length > 0) {
					GenericPrintDialog gpl = new GenericPrintDialog(
							getViewSite().getShell(), "Adressliste",
							"Adressliste");
					gpl.create();
					for (int i = 0; i < sel.length; i++) {
						Kontakt k = (Kontakt) sel[i];
						String[] f = new String[] { Kontakt.FLD_NAME1,
								Kontakt.FLD_NAME2, Kontakt.FLD_NAME3,
								Kontakt.FLD_STREET, Kontakt.FLD_ZIP,
								Kontakt.FLD_PLACE, Kontakt.FLD_PHONE1 };
						String[] v = new String[f.length];
						k.get(f, v);
						adrs[i] = new String[4];
						adrs[i][0] = new StringBuilder(v[0])
								.append(StringConstants.SPACE).append(v[1])
								.append(StringConstants.SPACE).append(v[2])
								.toString();
						adrs[i][1] = v[3];
						adrs[i][2] = new StringBuilder(v[4])
								.append(StringConstants.SPACE).append(v[5])
								.toString();
						adrs[i][3] = v[6];
					}
	...
	class KontaktLabelProvider extends DefaultLabelProvider {
		...
		public String getText(Object element) {
			String[] fields = new String[] { Kontakt.FLD_NAME1,
					Kontakt.FLD_NAME2, Kontakt.FLD_NAME3, Kontakt.FLD_STREET,
		...
		/*
		 * @Override public String getColumnText(Object element, int
		 * columnIndex) { Kontakt k=(Kontakt)element; switch(columnIndex){ case
		 * 0: return k.get(Kontakt.FLD_NAME1); case 1: return
		 * k.get(Kontakt.FLD_NAME2); case 2: return k.get(Kontakt.FLD_NAME3);
		 * case 3: return k.get(Kontakt.FLD_STREET); case 4: return
		 * k.get(Kontakt.FLD_ZIP); case 5: return k.get(Kontakt.FLD_PLACE); case
		 * 6: return k.get(Kontakt.FLD_PHONE1); } return "?"; }
		 */
	}
}

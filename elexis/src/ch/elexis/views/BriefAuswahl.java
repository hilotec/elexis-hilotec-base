	...
	class sPage extends Composite {
		...
		sPage(final Composite parent, final String cat) {
			...
			bLoad.addSelectionListener(new SelectionAdapter() {
				...
				public void widgetSelected(final SelectionEvent e) {
					try {
						TextView tv = (TextView) getViewSite().getPage()
	...
	private void makeActions() {
		briefNeuAction = new Action("Neu...") {
			...
			public void run() {
				TextView tv = null;
				try {
					tv = (TextView) getSite().getPage()
							.showView(TextView.ID /*
												 * ,StringTool.unique("textView")
					...
					if (bs.open() == Dialog.OK) {
						tv.createDocument(bs.getSelectedDocument(), bs
								.getBetreff());
						tv.setName();
						CTabItem sel = ctab.getSelection();
						if (sel != null) {
		...
		briefLadenAction = new Action("Öffnen") {
			...
			public void run() {
				try {
					TextView tv = (TextView) getViewSite().getPage().showView(
							TextView.ID);
					CTabItem sel = ctab.getSelection();
					if (sel != null) {
						CommonViewer cv = (CommonViewer) sel.getData();

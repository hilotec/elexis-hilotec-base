...
public class KGPrintView extends ViewPart {
	...
	CTabItem addItem(final String template, final String title,
		...
		Brief actBrief = text.createFromTemplateName(Konsultation
				.getAktuelleKons(), template, Brief.UNKNOWN, adressat, Messages.getString("KGPrintView.EMR")); //$NON-NLS-1$
	...
	public void useItem(int idx, String template, Kontakt adressat) {
		...
		if (template != null) {
			Brief actBrief = text.createFromTemplateName(Konsultation
					.getAktuelleKons(), template, Brief.UNKNOWN, adressat,
					betreff);

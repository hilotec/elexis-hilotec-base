 ...
 *
 */
public class FieldDisplayView extends ViewPart implements ActivationListener, SelectionListener, HeartListener{
	...
	public void init(IViewSite site, IMemento memento) throws PartInitException {
		...
		Integer canEdit=null;
		if(memento!=null){
			subid=site.getSecondaryId();
			if(subid!=null){
				nx=memento.getString(NODE+":"+subid);
				canEdit=memento.getInteger(NODE+":"+subid+":canEdit");
			}
		}
		setField(nx==null ? "Patient.Diagnosen" : nx, canEdit==null ? false : (canEdit!=0));
	}
...
*/
	...
	public void selectionEvent(PersistentObject obj) {
		if(myClass.isInstance(obj)){
			...
			if(val==null){
				SWTHelper.showError("Fehler bei der Felddefinition", "Das Feld "+myField+" kann nicht aufgelöst werden");
				text.setText("");
			}else{
	...
	private void setField(String field,boolean canEdit){
		...
		if(def.length!=2){
			SWTHelper.showError("Falsche Felddefinition", "Bitte Feld in der Form 'Datentyp.Feldname' angeben");
		}else{
			...
			if(myClass!=null){
				...
				bCanEdit=canEdit;
				setPartName(myField);
				Hub.userCfg.set("FieldDisplayViewData/"+subid,myClass.getSimpleName()+"."+myField);
	...
	private Class resolveName(String k){
		...
		if(k.equalsIgnoreCase("Mandant")){
			...
			try{
				...
				SWTHelper.showError("Feldtyp falsch", "Der gewünschte Datentyp "+k+" konnte nicht erkannt werden");
				ret= null;
			}
		}
		return ret;
		...
		newViewAction=new Action("Neues Fenster"){
			...
			public void run(){
				try {
					...
					FieldDisplayView n=(FieldDisplayView) getViewSite().getPage().showView(ID, StringTool.unique("DataDisplay"), IWorkbenchPage.VIEW_VISIBLE);
	...
	class SelectDataDialog extends TitleAreaDialog{
		...
		public void create() {
			super.create();
			setTitle("Datentyp");
			setMessage("Geben Sie bitte einen Ausdruck der Form Objekttyp.Feldname ein (z.B. Patient.Diagnosen)",IMessageProvider.INFORMATION);
		}

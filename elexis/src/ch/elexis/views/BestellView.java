	...
	public void createPartControl(final Composite parent){
		...
		tv.addDropSupport(DND.DROP_COPY, types, new DropTargetAdapter() {
			...
			public void drop(final DropTargetEvent event){
				...
				if (actBestellung == null) {
					InputDialog dlg =
						new InputDialog(getViewSite().getShell(), "Neue Bestellung anlegen",
							"Geben Sie bitte einen Titel für die neue Bestellung an", "", null);
		...
		countAction = new Action("Zahl ändern") {
			...
			public void run(){
				...
				if ((sel != null) && (!sel.isEmpty())) {
					...
					int old = it.num;
					InputDialog in =
						new InputDialog(getViewSite().getShell(), "Zahl ändern",
							"Geben Sie bitte an, wieviele Sie bestellen wollen", Integer
								.toString(old), null);

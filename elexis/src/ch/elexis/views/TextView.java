...
public class TextView extends ViewPart implements IActivationListener {
	...
	public void createPartControl(Composite parent) {
		...
		textContainer = txt.getPlugin().createContainer(parent,
				new SaveHandler());
		if (textContainer == null) {
			SWTHelper
					.showError(
							Messages
									.getString("TextView.couldNotCreateTextView"), Messages.getString("TextView.couldNotLoadTextPlugin")); //$NON-NLS-1$ //$NON-NLS-2$
		} else {
			makeActions();
	...
	public boolean createDocument(Brief template, String subject) {
		...
		actBrief = txt.createFromTemplate(Konsultation.getAktuelleKons(),
				template, Brief.UNKNOWN, null, subject);
		setName();
		if (actBrief == null) {
			return false;
		}
	...
	private void makeActions() {
		briefLadenAction = new Action(Messages.getString("TextView.openLetter")) { //$NON-NLS-1$
			...
			public void run() {
				...
				DocumentSelectDialog bs = new DocumentSelectDialog(
						getViewSite().getShell(), actPatient,
		...
		loadSysTemplateAction = new Action(Messages
				.getString("TextView.openSysTemplate")) { //$NON-NLS-1$
			@Override
			public void run() {
				DocumentSelectDialog bs = new DocumentSelectDialog(
						getViewSite().getShell(), Hub.actMandant,
		...
		loadTemplateAction = new Action(Messages
				.getString("TextView.openTemplate")) { //$NON-NLS-1$
			@Override
			public void run() {
				DocumentSelectDialog bs = new DocumentSelectDialog(
						getViewSite().getShell(), Hub.actMandant,
		...
		saveTemplateAction = new Action(Messages
			...
			public void run() {
				if (actBrief != null) {
					txt.saveTemplate(actBrief.get(Messages
							.getString("TextView.Subject"))); //$NON-NLS-1$
				} else {
		...
		showMenuAction.setToolTipText(Messages
	...
	class SaveHandler implements ITextPlugin.ICallback {
		...
		public void save() {
			...
			if (actBrief != null) {
				actBrief.save(txt.getPlugin().storeToByteArray(), txt
						.getPlugin().getMimeType());
			}
		}

		public boolean saveAs() {
			log.log(Messages.getString("TextView.saveAs"), Log.DEBUGMSG); //$NON-NLS-1$
			InputDialog il = new InputDialog(
			...
			if (il.open() == Dialog.OK) {
				...
				return actBrief.save(txt.getPlugin().storeToByteArray(), txt
						.getPlugin().getMimeType());
			}
			return false;
		}
	...
	public void activation(boolean mode) {
		if (mode == false) {
			...
			saveTemplateAction.setEnabled(Hub.acl

 ...
 *    
 *  $Id: RecordElement.java 5080 2009-02-03 18:28:58Z rgw_ch $
 *******************************************************************************/
...
public class RecordElement extends XChangeElement {
	 ...
	 * kid=getAttributeValue("responsible"); String auth=getAttributeValue("author"); Fall[]
	 * faelle=p.getFaelle(); Fall fall; if((faelle==null) || (faelle.length==0)){
	 * fall=p.neuerFall("Import", "unbekannt", "unbekannt"); }else{ fall=faelle[0]; } Konsultation
	 * k=fall.neueKonsultation(); parent.putExtToIntIDMapping(getAttributeValue("id"),k.getId());
	 * Kontakt kResp=parent.findContact(kid); kResp.set("istMandant", "1"); k.set("MandantID",kid);
	 * k.setDatum(getAttributeValue("date"), true); Samdas samdas=new Samdas(); Samdas.Record
	 * rec=samdas.getRecord(); Element eText=getChild("text", XChangeContainerImpl.ns);
	 * if(eText!=null){ String text=eText.getText(); if(text!=null){ rec.setText(text); }else {
	 * rec.setText(""); } }
	 * 
	 * VersionedResource ve=VersionedResource.load(null); ve.update(samdas.toString(), auth);
	 * k.setEintrag(ve, true);
	 * 
	 * // TODO parent.anamnesis.doImport(this, k); return new Result<String>("OK"); }
	 */

...
/**
 * Interface to specify a Java (SWT) connection to scanner devices e.g. via
 * TWAIN or SANE Implementing class must implement IScannerAccess and
 * IScannerAccess.ISource.
 * 
 * It must return all available Sources on a call to getSources(), let the user
 * configure a source via configureSource(), and must aquire an image from the
 * source on a call to aquire
 * 
 * @author gerry
 * 
 */
public interface IScannerAccess {
	 ...
	 * 
	 * @return an Array of SourceDefinitions, or null if there is no available
	 *         Sources
	 */
	...
	/**
	 * Aquire an Image from the Scanner and create an SWT image from it. As
	 * every ISource has its own configuration stored, the TWAIN device will use

...
public class LaborTaxpunktkorrektur {
	...
	/**
	 * Rechne alle Konsultationen seit einem Stichdatum (inklusive) auf einen
	 * neuen Taxpunktwert um
	 * 
	 * @param dateFrom
	 * @param newTP
	...
	public String recalc(String dateFrom, double newTP) {
		File file = new File(System.getProperty("user.home") + File.separator
				+ "elexis" + File.separator + "taxpunktkorrektur.log");
		try {
			writer = new FileWriter(file);
			if (SWTHelper.askYesNo("WARNUNG",
					"Wirklich alle Konsultationen seit dem " + dateFrom
				...
				del.append("DELETE FROM VK_PREISE WHERE TYP='EAL'").append(
						" and DATUM_VON>=").append(
				...
				del.append("UPDATE VK_PREISE SET DATUM_BIS=")
						.append(
								JdbcLink.wrap(yesterday
										.toString(TimeTool.DATE_COMPACT)))
						.append(" WHERE TYP='EAL' AND DATUM_BIS>").append(
								JdbcLink.wrap(yesterday
										.toString(TimeTool.DATE_COMPACT)));
				...
				del
						...
						.append(
								JdbcLink.wrap(ttFrom
										.toString(TimeTool.DATE_COMPACT)))
						.append(",").append("'99991231','EAL',").append(

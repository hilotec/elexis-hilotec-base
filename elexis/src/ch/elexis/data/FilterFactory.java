...
public class FilterFactory {
	...
	public static class Filter extends ViewerFilter implements IFilter{
		...
		private Filter(Class clazz, String...strings ){
			mine=clazz;
			fields=strings;
			operators=new OPERATORS[fields.length];
			values=new String[fields.length];
		}
		public Query createQuery(){
			Query qbe=new Query(mine);
			modifyQuery(qbe);
			return qbe;
		}
		public void modifyQuery(Query qbe){
			for(int i=0;i<fields.length;i++){
				if(!ch.rgw.tools.StringTool.isNothing(values[i])){
					qbe.and();
					qbe.add(fields[i],operators[i].name(),values[i]);
				}
			}
		}
		public boolean select(Object toTest) {
			if(toTest.getClass().equals(mine)){
				PersistentObject p=(PersistentObject)toTest;
				for(int i=0;i<fields.length;i++){
					if(!StringTool.isNothing(values[i])){
						String val=p.get(fields[i]);
						if(StringTool.isNothing(val)){
							return false;
						}
						switch(operators[i]){
						case BEGINNT:
							if(!val.startsWith(values[i])){
								return false;
							}
							break;
						case ENTHAELT:
							if(val.indexOf(values[i])==-1){
								return false;
							}
							break;
						case ENDET:
							if(!val.endsWith(values[i])){
								return false;
							}
							break;
						case ENTHAELTNICHT:

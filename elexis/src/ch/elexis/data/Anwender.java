 ...
 * Software zu benützen. Ein Anwender hat Username und Passwort, sowie ein AgendaLabel. Jeder Anwender
 * gehört zu mindestens einer Gruppe.
 * 
...
public class Anwender extends Person {
	 ...
	 * <p>
	 * We check wheter the object exists in the database and whether the login name
	 * ("Label") is available. 
	 * </p>
	 */
	@Override
    ...
    /**
     * Den ersten Benutzer anlegen unt initiale Zugriffsrechte setzen
     * Wird von PersistentObject() aufgerufen, wenn die Datenbank neu 
     * angelegt wurde.
     */
	...
	protected static void init(){
    	...
    	admin.create(null);
    	admin.set(new String[]{"Name","Label","istAnwender"},"Administrator","Administrator","1");
        Hub.actUser=admin;
        Hub.acl.grant(admin,"WriteInfoStore","LoadInfoStore","WriteGroups","ReadGroups");
        Hashtable hash=admin.getInfoStore();
		hash.put("UsrPwd","admin");
		hash.put("Groups","Admin,Anwender");
		admin.flushInfoStore(hash);
		Hub.acl.grant("Admin","ReadUsrPwd","WriteUsrPwd","CreateAndDelete","WriteGroups");
     ...
     * (Jeder Anwender ist entweder selber ein Mandant oder ist einem Mandanten
     * zugeordnet)
     * @param text Kurzname
	...
	public static boolean login(final String text, final String text2) {
		...
		if(km==null){
			...
			MessageDialog.openError(null,"Interner Fehler","Die Datenstruktur ExtInfo von "+a.getLabel()+" ist beschädigt.");
		...
		if(pwd.equals(text2)){
			...
			if(!StringTool.isNothing(MandantLabel)){
				MandantID=new Query<Mandant>(Mandant.class).findSingle("Label","=",MandantLabel);
			}
			if(MandantID!=null){
				...
				if((m!=null) && m.isValid()){
					...
					if((ml!=null) && (ml.size()>0)){
						...
						SWTHelper.showError("Kein Mandant definiert", "Sie können Elexis erst normal benutzen, wenn Sie mindestens einen Mandanten definiert haben");
						//new ErrorDialog(Desk.theDisplay.getActiveShell(),"Kein Mandant definiert","Sie können Elexis erst benutzen, wenn Sie mindestens einen Mandanten definiert haben",
			...
			try{
				...
				ExHandler.handle(ex);
				SWTHelper.showError("Perspektive nicht gefunden", "Konnte die eingestellte Startperspektive "+perspektive+" nicht laden.");
				return true;
			}

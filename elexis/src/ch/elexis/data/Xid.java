...
public class Xid extends PersistentObject {
	...
	static {
		...
		if (storedDomains == null) {
			...
			for (String dom : storedDomains.split(";")) {
				...
				domains.put(
						spl[0],
						new XIDDomain(spl[0], simpleName, Integer
	 ...
	 *             or quality already exists. if the domain was not rgeistered
	 */
	...
	public String getLabel() {
		...
		ret.append(text).append(": ").append(get(DOMAIN)).append("->")
				.append(get(ID_IN_DOMAIN));
		return ret.toString();
	}
	...
	private static void storeDomains() {
		...
		for (String k : domains.keySet()) {
			...
			sb.append(k).append("#").append(xd.getQuality()).append("#")
					.append(xd.getSimpleName()).append("#")

...
public class LabGroup extends PersistentObject implements Comparable{
	...
	/**
	 * Creates an new LabGroup with an initial list of entries. Set items to <code>null</code>
	 * toe create an empty LabGroup
	 * @param name the name of the LabGroup
	 * @param items the initiall list of items (may be null)
	 */
    ...
    public void addItem(LabItem item) {
    	if (item != null && (item.existence()==EXISTS)) {
    		...
    		if (StringTool.isNothing(exists)) {
    			...
    			.append(item.getWrappedId()).append(")");
    			j.exec(sql.toString());
    		}
    	}
    }
    ...
    public void removeItem(LabItem item) {
    	...
    	sql.append("DELETE FROM " + GROUP_ITEM_TABLENAME + " WHERE GroupID = ")
    		...
    		.append("ItemID = ").append(item.getWrappedId());
    	j.exec(sql.toString());
    }

...
/**
 * Ein Kontakt ist der kleinste gemeinsame Nenner anller Arten von Menschen und
 * Institutionen und somit die Basisklasse für alle Kontakte. Ein Kontakt hat
 * eine Anschrift und beliebig viele zusätzliche Bezugsadressen, sowie Telefon,
 * E-Mail und Website. Zu einem Kontakt können ausserdem Reminders erstellt
 * werden. Schliesslich hat jeder Kontakt noch einen "Infostore", einen im
 * Prinzip unbegrenzt grossen Speicher für beliebig viele parameter=wert -
 * Paare, wo Informationen aller Art abgelegt werden können. Jedem Element des
 * Infostores können Zugriffsrechte zugeteilt werden, die definieren, wer dieses
 * Element lesen und Schreiben darf.
 * 
 * @author gerry
 * 
 */
public class Kontakt extends PersistentObject {
	...
	public static final String[] DEFAULT_SORT = { FLD_NAME1, FLD_NAME2,
			FLD_STREET, FLD_PLACE };
	...
	static {
		addMapping(
				...
				FLD_REMARK, FLD_PHONE1,
				FLD_PHONE2,
				"E-Mail=EMail", FLD_WEBSITE, FLD_EXTINFO, //$NON-NLS-1$
				FLD_IS_ORGANIZATION, FLD_IS_PERSON, FLD_IS_PATIENT,
				FLD_IS_USER, FLD_IS_MANDATOR, FLD_IS_LAB, FLD_STREET, FLD_ZIP,
				FLD_PLACE, FLD_COUNTRY, FLD_FAX, FLD_ANSCHRIFT, FLD_MOBILEPHONE);
	}
	 ...
	 * and "Bezeichnung1", "Bezeichnung2", "Strasse", "Plz" and "Ort", separated
	 * with a comma, for the long label.
	 * 
	 * Subclasses can overwrite this method and define their own label(s). If
	 * short is true, they should return a short label suitable for addresses.
	 * If short is false, they should return a long label describing all
	 * important properties of this Kontakt for unique identification by the
	 * user.
	 * 
	 * @param shortLabel
	...
	public String getLabel(boolean shortLabel) {
		...
		if (shortLabel) {
			...
			get(new String[] { FLD_NAME1, FLD_NAME2, FLD_NAME3, FLD_STREET,
					FLD_ZIP, FLD_PLACE }, ret);
			bld.append(ret[0]).append(StringTool.space)
					.append(checkNull(ret[1]));
			...
			bld.append(", ").append(checkNull(ret[3])).append(", ") //$NON-NLS-1$ //$NON-NLS-2$
					.append(checkNull(ret[4])).append(StringTool.space)
					.append(checkNull(ret[5]));
		}
	 ...
	 *            die Adresse
	 * @param sBezug
	 *            ein Text, der die Beziehung dieser Adresse zum Kontakt
	 *            definiert (z.B. "Geschäftlich" oder "Orthopäde" oder so)
	 */
	...
	/**
	 * Ein Element aus dem Infostore auslesen Der Rückgabewert ist ein Object
	 * oder Null. Wenn die Rechte des aktuellen Anwenders zum Lesen dieses
	 * Elements nicht ausreichen, wird ebenfalls Null zurückgeliefert. 2.9.2007
	 * We remove the checks. they are useless at this moment better check
	 * permissions on inout fields. gw
	 */
	 ...
	 * Namen schon existiert, wird es überschrieben. Wenn die Rechte des
	 * angemeldeten Anwenders nicht für das Schreiben dieses Elements
	 * ausreichen, wird die Funktion still ignoriert.
	 * 
	 * @param elem
	 *            Name des Elements
	 * @param val
	 ...
	 * mehrere Werte gelesen oder geschrieben werden sollen, da damit das
	 * wiederholte entpacken/packen gespart wird. Nach Änderungen muss der Store
	 * mit flushInfoStore() explizit gesichert werden. ACHTUNG: Nicht
	 * Thread-Safe. Konkurriende Schreiboperationen, während ein Thread den
	...
	public Hashtable getInfoStore() {
		...
		/*
		 * if(Hub.acl.request("LoadInfoStore")==true){ return
		 * getHashtable("ExtInfo"); } else{
	...
	/**
	 * Den mit getInfoStore geholten Infostore wieder zurückschreiben. Dies muss
	 * immer dann geschehen, wenn nach getInfoStore() schreiboperationen
	 * durchgeführt wurden.
	 * 
	 * @param store
	...
	public void flushInfoStore(Hashtable store) {
		 ...
		 * setHashtable("ExtInfo",store); }else{
		 * log.log("Unzureichende Rechte zum Schreiben des Infostore"
		 * ,Log.WARNINGS); }
		 */
	}
	 ...
	 * @return Ein Objekt der Klasse clazz, welches einen Infostore-Eintrag
	 *         field mit dem Inhalt value enthält, oder null wenn kein solches
	 *         Objekt existiert.
	 */
	@SuppressWarnings("unchecked")
	public static Kontakt findKontaktfromInfoStore(Class clazz, String field,
			String value) {
	 ...
	 * @return eine Liste mit Objektbezeichnern, die zwischen 0 und 30 nach
	 *         Häufigkeit sortierte Elemente enthält.
	 */
	@SuppressWarnings("unchecked")
	public List<String> getStatForItem(String typ) {
		...
		ArrayList<String> ret = new ArrayList<String>(al == null ? 1
				: al.size());
		if (al != null) {
			for (statL sl : al) {
				ret.add(sl.v);
	...
	/**
	 * Eine Statistik für ein bestimmtes Objekt anlegen. Es wird gezählt, wie
	 * oft diese Funktion für dieses Objekt schon aufgerufen wurde, und Objekte
	 * desselben Typs aber unterschiedlicher Identität werden in einer Rangliste
	 * aufgelistet. Diese Rangliste kann mit getStatForItem() angerufen werden.
	 * Die Rangliste enthält maximal 40 Einträge.
	 * 
	 * @param lst

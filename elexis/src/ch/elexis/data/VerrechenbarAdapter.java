...
public abstract class VerrechenbarAdapter extends PersistentObject implements
	...
	public void setVKMultiplikator(final TimeTool von, TimeTool bis,
			final double factor, final String typ) {
		StringBuilder sql = new StringBuilder();
		String eoue = new TimeTool(TimeTool.END_OF_UNIX_EPOCH)
		...
		sql.append("UPDATE VK_PREISE SET DATUM_BIS=").append(
				JdbcLink.wrap(from)).append(" WHERE (DATUM_BIS=").append(
				JdbcLink.wrap(eoue)).append(
				" OR DATUM_BIS='99991231') AND TYP=")
				.append(JdbcLink.wrap(typ));
		stm.exec(sql.toString());
		sql.setLength(0);
		sql
				.append(
						"INSERT INTO VK_PREISE (DATUM_VON,DATUM_BIS,MULTIPLIKATOR,TYP) VALUES (")
				.append(JdbcLink.wrap(von.toString(TimeTool.DATE_COMPACT)))
				.append(",").append(
						JdbcLink.wrap(bis.toString(TimeTool.DATE_COMPACT)))
				.append(",").append(JdbcLink.wrap(Double.toString(factor)))
				.append(",").append(JdbcLink.wrap(typ)).append(");");
		stm.exec(sql.toString());
	...
	private double getMultiplikator(final TimeTool date, final String table,
			final String typ) {
		String actdat = JdbcLink.wrap(date.toString(TimeTool.DATE_COMPACT));
		StringBuilder sql = new StringBuilder();
		sql.append("SELECT MULTIPLIKATOR FROM ").append(table).append(
				" WHERE TYP=").append(JdbcLink.wrap(typ)).append(
				" AND DATUM_VON <=").append(actdat).append(" AND DATUM_BIS >")
				.append(actdat);
		String res = getConnection().queryString(
				sql.toString() + " AND ID=" + getWrappedId());
		if (res == null) {
	...
	private void makeActions(final ICodeElement el) {
		addToBlockAction = new Action("Zu Leistungsblock...") {
			@Override
			public void run() {
				AddElementToBlockDialog adb = new AddElementToBlockDialog(Desk
						.getTopShell());
				if (adb.open() == Dialog.OK) {
					ICodeElement ice = (ICodeElement) GlobalEvents
							.getInstance().getSelectedObject(el.getClass());
					Leistungsblock lb = adb.getResult();
					lb.addElement(ice);
					GlobalEvents.getInstance().fireUpdateEvent(

 ...
 *    
 *  $Id: MoneyInput.java 5321 2009-05-28 12:06:28Z rgw_ch $
 *******************************************************************************/
...
public class MoneyInput extends Composite {
	...
	public MoneyInput(final Composite parent, final String label,
			final ch.rgw.tools.Money money) {
		this(parent, label);
	...
	private void prepare() {
		text.addFocusListener(new FocusAdapter() {
			...
			public void focusLost(final FocusEvent e) {
				try {
					...
					if (t.length() == 0) {
						text.setText(new ch.rgw.tools.Money()
								.getAmountAsString());
					} else {
					...
					SWTHelper
							.alert(Messages.getString("MoneyInput.InvalidAmountCaption"), //$NON-NLS-1$
		 ...
		 * verifyText(VerifyEvent e) { if(e.character==SWT.DEL ||
		 * e.character==SWT.BS){ e.doit=true; }else{ String
		 * t=text.getText()+e.character; if(t.length()<2 ||
		 * t.matches("[0-9]+[\\.,]?[0-9]{0,2}")){ e.doit=true; }else{
		 * e.doit=false; } } }});
		 */

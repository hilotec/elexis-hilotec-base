...
/**
 * A class representing Money (as you might have guessed)
 * Simplifies and standardizes calculations with money (rounding, converting to and from strings etc.)
 * The accepted formats of the string representation of an amount depends on the current locale by default
 * the format of can be set to another locale if necessary (globally)
...
public class Money extends Number implements Comparable<Money> {
	...
	/**
	 * Parse an amount given as string
	 * This might fail if the string doesn't conform to the current locale's
	 * standard currency format
	 * @param val an amount
	...
	/** get the money out of a SWT Text-field
	 * This might fail if the text field's contents doesn't conform to the current locale's
	 * standard currency format 
	 * @throws ParseException
	 */
	public Money(Text textField) throws ParseException{

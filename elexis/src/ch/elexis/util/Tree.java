...
/**
 *	Eine Baumförmige rekursive Datenstruktur. Ein Tree ist gleicheitig ein node.
 *  Ein Tree hat children (die allerdings auch null sein können) und Geschwister,
 *  (die ebenfalls null sein können), sowie ein Parent, welches ebenfalls null
 *  sein kann (dann ist dieses Tree-Objekt die Wurzel des Baums)
 *  Jeder Tree trägt ein beliebiges Datenobjekt (contents). 
 */
public class Tree<T> {
	 ...
	 * @param p der Parent, oder null, wenn dies die Wurzel werden soll.
	 * @param elem das zugeordnete Datenobjekt
	 */
	 ...
	 * @param parent Parent
	 * @param elem Datenobjekt
	 * @param comp Ein Comparator für das Fatenobjekt
	 */
	...
	/**
	 * Ein neues Tree-Objekt mit einem Filter erstellen. Wenn ein Filter gesetzt
	 * wird, dann werden von getChildren() nur die geliefert, die dem Filter entsprechen
    ...
    /**
     * Filter nachträglich setzen. Der Filter wird für dieses und alle
     * Children gesetzt.
     * @param f der Filter
     */
    ...
    /**
     * Ein Datenobjekt als Kind-element zufügen. Dies (Das Datenobjekt wird implizit
     * in ein Tree-Objekt gepackt. obj.add(t) ist dasselbe wie new Tree(obj,t))
     * @param elem Das Datenobjekt
	 ...
	 * @param newParent der neue Elter
	 */
	...
	/**
	 * Ähnlich wie add, aber wenn das übergebene Child schon existiert, werden nur
	 * dessen Kinder mit den Kindern des existenten childs ge'merged' (Also im
	 * Prinzip ein add mit Vermeidung von Dubletten
	 */
	...
	/**
	 * Erstes Kind-element liefern. Null, wenn keine Kinder. 
	 * Dies macht im Gegensatz zu hasChildren() keine synchronisation!
	 * @return
	 */
	...
	/**
	 * Nächstes Geschwister liefern oder null wenn keine mehr da sind.
	 * getParent().getFirstChild() liefert den Start der Geschwisterliste.
	 * @return
	 */

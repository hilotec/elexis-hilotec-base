 ...
 *    
 *  $Id: TreeContentProvider.java 2208 2007-04-13 09:05:39Z danlutz $
 *******************************************************************************/
...
/**
 * ContentProvider, der einen Tree füllen kann. Datenquelle muss ein
 * TreeLoader sein.
 * @author Gerry
 *
 */
public class TreeContentProvider implements ITreeContentProvider, BackgroundJobListener, CommonContentProvider{
	BackgroundJob job;
	CommonViewer viewer;
    ...
    {
        if(element instanceof ch.elexis.util.Tree){
            ch.elexis.util.Tree tr = (ch.elexis.util.Tree) element;
    ...
    {
        ch.elexis.util.Tree result=
            (ch.elexis.util.Tree) job.getData();
        if(result==null){
        	JobPool.getJobPool().activate(job.getJobname(),Job.SHORT);
            return new String[]{Messages.getString("TreeContentProvider.loadData")}; //$NON-NLS-1$
        }
        else{
        	if(viewer.getConfigurer().getControlFieldProvider().isEmpty()){
        		result.setFilter(null);
        	}else {
        		result.setFilter(((DefaultControlFieldProvider)viewer.getConfigurer().getControlFieldProvider()).createIFilter());
        	}

...
public abstract class ImporterPage implements IExecutableExtension{
	...
	/** 
	 * Die Dialogseite erstellen, um ggf. eine Datenquelle auszuwählen oder
	...
	/**
	 * An Importer that lets the user select a directory to import from. Can be built
	 * into derived ImporterPages
	 * @author gerry
	 *
	 */
	...
	static public class ODBCBasedImporter extends Composite{
		...
		public ODBCBasedImporter(final Composite parent, final ImporterPage home){
			...
			tSource.setText(Hub.localCfg.get("ImporterPage/"+home.getTitle()+"/ODBC-Source","")); //$NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$
			home.results=new String[1];
			...
			bSource.addSelectionListener(new SelectionAdapter(){
				...
				public void widgetSelected(SelectionEvent e) {
					...
					if(in.open()==Dialog.OK){
						...
						Hub.localCfg.set("ImporterPage/"+home.getTitle()+"/ODBC-Source",home.results[0]); //$NON-NLS-1$ //$NON-NLS-2$
					}
    ...
    static public class DBBasedImporter extends Composite{
        ...
        public DBBasedImporter(final Composite parent, final ImporterPage home){
            ...
            setLayout(new GridLayout());
            final Label lSource=new Label(this,SWT.NONE);
            final String[] preset=Hub.localCfg.getStringArray("ImporterPage/"+home.getTitle()+"/database"); //$NON-NLS-1$ //$NON-NLS-2$
            tSource=new Text(this,SWT.BORDER);
            lSource.setText(Messages.getString("ImporterPage.source")); //$NON-NLS-1$
            tSource.setEditable(false);
            lSource.setLayoutData(SWTHelper.getFillGridData(1,true,1,false));
            ...
            bSource.addSelectionListener(new SelectionAdapter(){
                ...
                public void widgetSelected(SelectionEvent e) {
                    ...
                    if (wd.open() == Dialog.OK) {
                        ...
                        Hub.localCfg.set("ImporterPage/"+home.getTitle()+"/database", StringTool.join(home.results, ",")); //$NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$
                        Hub.localCfg.flush();
                    }
                }
            });

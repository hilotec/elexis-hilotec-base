 ...
 * In der jetzigen Version wird nur eine flat table ähnlich wie bei cfgSettings
 * verwendet. Mehrere Anwendungen können dieselbe Datenbank verwenden, wenn
...
public class SqlSettings extends Settings {
	...
	public static final String Version(){return "1.2.0";}
	volatile JdbcLink j;
	volatile String tbl;
	...
	{	Iterator it=iterator();
    	...
    	{  String a=(String)it.next();
    	   String v=get(a,null);
    	   StringBuilder sql=new StringBuilder(300);
    	   sql.append("SELECT ").append(valueColumn).append(" FROM ").append(tbl).append(" WHERE ");
    	   if(constraint!=null){
    		   sql.append(constraint).append(" AND ");
    	   }
    	   sql.append(paramColumn).append("=").append(JdbcLink.wrap(a));
    	   //String sql="SELECT wert FROM "+tbl+" WHERE "+constraint+" AND param="+JdbcLink.wrap(a);
    	   if(stm.queryString(sql.toString())!=null){
    		   ...
    		   if(v==null){
    			   ...
    			   sql.append(paramColumn).append("=").append(JdbcLink.wrap(a));
    	       		//sql=new StringBuilder("DELETE from "+tbl+" WHERE "+constraint+" AND param="+JdbcLink.wrap(a))
    	   		}else{
    	   			sql.append("UPDATE ").append(tbl).append(" SET ").append(valueColumn).append("=").append(JdbcLink.wrap(v)).append(" WHERE ");
    	   			if(constraint!=null){
    	   				sql.append(constraint).append(" AND ");
    	   			}
    	   			sql.append(paramColumn).append("=").append(JdbcLink.wrap(a));
    	   		    //sql=new StringBuilder("UPDATE "+tbl+" SET wert="+JdbcLink.wrap(v)+" WHERE "+constraint+" AND param="+JdbcLink.wrap(a));    
    	   		}
    	   }
    	   else
    	   {	if(v==null){
    	   		...
    	   		sql=new StringBuilder(200);
    	   		String[] cn=null;
    	   		sql.append("INSERT INTO ").append(tbl).append("(").append(paramColumn).append(",").append(valueColumn);
    	   		if(constraint!=null){
    	   			cn=constraint.split("=");
    	   			sql.append(",").append(cn[0]);
    	   		}
    	   		sql.append(") VALUES (").append(JdbcLink.wrap(a)).append(",").append(JdbcLink.wrap(v));
    	   		if(constraint!=null){

 ...
 * Änderungen der Settings sind immer volatil. Erst mit dem Auruf von flush() erfolgt
 * eine persistierende Speicherung (deren genaues Ziel Sache der konkreten Implementation
 * ist. mit undo() kann jeweils der Stand nach dem letzten flush() wiederhergestellt werden.
 * Es erfolgt euch keine implizite Speicherung beim Programmende! Alle Änderungen, die
 * nicht explizit mit flush() gesichert werden, sind verloren.
 */
...
public ArrayList nodes()
   ...
   * direkt auf einem Zweig abgebildet werden, ein XML-File kann einen entsprechenden
   * Node erstellen, ein Flatfile wird einfach Einträge des Typs präfix/eintrag
   * erstellen.          
   * @param name Der Name des Zweigs
   * @param CreateIfNotExist Der Zweig wird erstellt, wenn er noch nicht existiert
...
private void addNode(ArrayList dest, String name, Hashtable n)
   ...
   * @param l2 ein ch.rgw.tools.Log oder null (dann Ausgabe auf die Konsole)
   */    
   ...
   * @param ex Eintrag, welcher nicht einbezogen wird
   * @return den hashcode
   ...
   * @param ex Eintrag, der den hashcode enth�lt
  ...
  { if(rec==null) return;
    String v=Integer.toString(rec.x)+","+Integer.toString(rec.y)+","+
   ...
   * @param key	Schl�ssel
   * @param d Datum/Zeit als ch.rgw.tools.timeTool
   */
   ...
   * @param key	Schl�ssel
   * @param defvalue Defaultwert, falls der Schl�ssel nicht existiert
   ...
   * @param key Schl�ssel
   * @return ein ch.rgw.timeTool oder null, wenn der Schl�ssel nicht existiert oder
   * ein ung�ltiges Format hat.
   */
   ...
   * @param key Schl�ssel
   * @return das Rectangle oder null, wenn der Schl�ssel nicht existiert oder 
   * der Eintrag ein ung�ltiges Format hat.
   */
   ...
   * Bei Programmabbruch ohne flush werden alle Änderungen seit dem letzten flush()
   * resp. Programmstart verworfen
   *
   */
  public void flush() {
      if(dirty==true) {

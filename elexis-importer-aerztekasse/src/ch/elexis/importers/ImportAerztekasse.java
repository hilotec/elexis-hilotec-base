 ...
 * 
 * In Mediwin CBpro können die Patienten mittels "Module/Export" exportiert werden. Es
 * sollten alle Felder ausgewählt und unter "Optionen" die Kolonnen-Titel aktiviert
 * werden. Die Datei kann dann mittels dieses Importers importiert werden.
 * Importierte Patienten werden speziell markiert, so dass der Importer diese
 * wieder erkennen kann. Die Patienten können zu jeder Zeit erneut importiert
 * werden. Falls ein Patient nicht mehr importiert werden soll, kann in Elexis
 * im Feld "Bemerkungen" der Text "AEK:elexis" eingegeben werden, dann wird er
 * ignoriert. Falls ein Patient in Elexis erstellt wurde, und in Mediwin CBpro
 * ein Patient mit der gleichen Nummer exisitert, gibt es eine Warnung, und
 * der Patient wird nicht importiert. Soll er trotzdem importiert werden, kann
 * der Patient mit der gleichen Nummer in Elexis im Feld "Bemerkungen" mit dem
 * Text "AEK:aek" markiert werden. Der in Elexis erfasste Patient wird dann
 * mit den Daten aus Mediwin CBpro ersetzt.
 * 
 * 
...
public class ImportAerztekasse extends ImporterPage {
    ...
    /**
     * Patient Numer as of Mediwin CBpro, stored in InfoStore.
     * This helps us recognize an already imported patient.
     */
    private static final String AEK_PATIENT_NR = "AEKPatientNr";
    
    /**
     * Preference values indicating which patient should be preferred
     * (AKE or Elexis), given in the field Bemerkungen.
     * At initial import, PREFER_AEK is used. Can be changed by the user,
     * so that a patient is no longer imported.
     */
    ...
    /**
     * 
     * @param columns the column names in the header
    ...
    private void alertConflict(String nummer, String existingLabel, String importedLabel) {
        ...
        String message = "Ein Patient mit der Nummer " + nummer
                + " existiert bereits. Bitte geben Sie dem "
                + "existierenden (" + existingLabel
                + ") oder dem zu importierenden (" + importedLabel
    ...
    /**
     * 
     * @param fields the single fields of a line
    ...
    public IStatus doImport(IProgressMonitor monitor) throws Exception {
        ...
        long l = file.length();
        InputStreamReader ir=new InputStreamReader(new FileInputStream(file),"iso-8859-1");  // TODO always this charset?
        ...
        while ((line = br.readLine()) != null) {
            ...
            for (int i = 0; i < values.length; i++) {
                if (MAX_FIELD_LENGTHS[i] > 0) {
                    if (values[i].length() > MAX_FIELD_LENGTHS[i]) {
                        // shorten field
                        values[i] = values[i]
                                .substring(0, MAX_FIELD_LENGTHS[i]);

                        log.log("Patient " + values[NUMMER] + ": Feld "
                                + ELEXIS_FIELDS[i]
            ...
            if (id == null) {
                // create new patient
                Patient patient = new Patient(values[NAME], values[VORNAME], values[GEBURTSDATUM], values[SEX]);
                ...
                if (!oldAekNumber.equals(aekNumber)) {
                    ...
                    if (bemerkung.startsWith(PREFER_AEK)) {
                        ...
                        String message = "Der Patient " + patient.getLabel()

 ...
 *    
 *  $Id: ESRRecordDialog.java 1718 2007-02-02 16:30:57Z rgw_ch $
 *******************************************************************************/
...
public class ESRRecordDialog extends TitleAreaDialog {
	...
	private InputData[] fields={
			...
			new InputData("Rechnung","RechnungsID",new LabeledInputField.IContentProvider(){
				...
				public void reloadContent(PersistentObject po, InputData ltf) {
					InputDialog id=new InputDialog(getShell(),"Rechnungsnummer ändern","Bitte geben Sie die neue Rechnungsnummer ein",ltf.getText(),null);
					if(id.open()==Dialog.OK){
						String rnid=new Query<Rechnung>(Rechnung.class).findSingle("RnNummer", "=", id.getValue());
						if(rnid!=null){
							Rechnung r=Rechnung.load(rnid);
							rec.set("RechnungsID",r.getId());
							ltf.setText(r.getNr());
						}else{
							SWTHelper.showError("Rechnung nicht gefunden", "Es wurde keine Rechnung mit der Nummer "+id.getValue()+" gefunden.");
						}
					}
				}
			...
			new InputData("Patient","PatientID",new LabeledInputField.IContentProvider(){
				...
				public void reloadContent(PersistentObject po, InputData ltf) {
					KontaktSelektor ksl=new KontaktSelektor(getShell(),Patient.class,"Patient auswählen","Bitte wählen Sie einen Patienteneintrag");
	...
	protected Control createDialogArea(Composite parent) {
		...
		ret.setLayout(twl);
		LabeledInputField.AutoForm af=new LabeledInputField.AutoForm(ret,fields);
	...
	protected void okPressed() {
		if(bBook.getSelection()){
			...
			if(bBooked){
				Money zahlung=rec.getBetrag();
				Rechnung rn=rec.getRechnung();
				rn.addZahlung(zahlung.negate(), "stornierter VESR für rn "+rn.getNr()+" / "+rec.getPatient().getPatCode());
				rec.set("Gebucht","");
			}
		}
		super.okPressed();

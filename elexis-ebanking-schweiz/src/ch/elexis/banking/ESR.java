...
/**
 * Repräsentation eines ESR Einzahlungsscheins
 * tn ist die Teilnehmernummer. id kann null sein, dann ist es ein VESR,
 * oder kann die subid des Bankkunden sein, dann ist es ein BESR.
 * @author gerry
 *
 */
public class ESR {
	 ...
	 * @param usr individueller Identifikationscode des EZ-Scheins (z.B. aus PatNr, und RnNummer aufgebaut)
	 ...
	 * @param withSeparators true: Bindestriche an geeigneten Stellen, wie im KOnstruktor eingegeben
	...
	public int getESRLineX(){
		int printerCorrectionX = Hub.localCfg.get(ESR_PRINTER_CORRECTION_X, ESR_PRINTER_CORRECTION_X_DEFAULT);
	...
	public int getESRLineY(){
		int printerCorrectionY = Hub.localCfg.get(ESR_PRINTER_CORRECTION_Y, ESR_PRINTER_CORRECTION_Y_DEFAULT);
	...
	public boolean printBESR(Kontakt bank, Kontakt schuldner, Kontakt empfaenger, String betragInRappen, TextContainer text){
		...
		int yOff=192;	// Offset Einzahlungsschein 19.2cm (absolut vom Papierrand)
		...
		if (bank != null && bank.isValid()) {
			...
			badr.append(bank.get("Bezeichnung1")).append(" ").append(bank.get("Bezeichnung2"))
				.append("\n").append(bank.get("Plz")).append(" ").append(bank.get("Ort"));
			p.insertTextAt(baseX,yOff+8,xGiro-5,hAdr-2,badr.toString(),SWT.LEFT);
			p.insertTextAt(xGiro,yOff+8,xGiro-5,hAdr-2,badr.toString(),SWT.LEFT);

			// Empfaenger
			p.insertTextAt(baseX,yOff+20,xGiro-5,hBeg-1,empfaenger.getPostAnschrift(true),SWT.LEFT);
			p.insertTextAt(xGiro,yOff+20,xGiro-5,hBeg-1,empfaenger.getPostAnschrift(true),SWT.LEFT);
		} else {
			// ESR
			
			int height = hAdr + 2 + hBeg;
			p.insertTextAt(baseX,yOff+8,xGiro-5,height,empfaenger.getPostAnschrift(true),SWT.LEFT);
			p.insertTextAt(xGiro,yOff+8,xGiro-5,height,empfaenger.getPostAnschrift(true),SWT.LEFT);
		}
		...
		p.insertTextAt(xGiro+40+baseX,yOff+50,wRp,hFr-3,Rappen,SWT.RIGHT);
		
		p.insertTextAt(117+baseX,yOff+yRef,wRef,hRef,makeRefNr(true),SWT.RIGHT);
		String konto=makeParticipantNumber(true);
		p.insertTextAt(xKonto+baseX,yOff+yKonto,wKonto,hKonto,konto,SWT.LEFT);
		p.insertTextAt(xGiro+xKonto+baseX,yOff+yKonto,wKonto,hKonto,konto,SWT.LEFT);

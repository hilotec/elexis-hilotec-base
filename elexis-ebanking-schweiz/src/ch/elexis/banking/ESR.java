...
public class ESR {
	...
	public boolean printBESR(Kontakt bank, Kontakt schuldner, Kontakt empfaenger,
		...
		if (bank != null && bank.isValid()) {
			...
			// Empfaenger
			p.insertTextAt(baseX, yOff + 20, xGiro - 5, hBeg - 1,
				empfaenger.getPostAnschrift(true), SWT.LEFT);
			p.insertTextAt(xGiro, yOff + 20, xGiro - 5, hBeg - 1,
				empfaenger.getPostAnschrift(true), SWT.LEFT);
		} else {
			// ESR
			
			int height = hAdr + 2 + hBeg;
			p.insertTextAt(baseX, yOff + 8, xGiro - 5, height, empfaenger.getPostAnschrift(true),
				SWT.LEFT);
			p.insertTextAt(xGiro, yOff + 8, xGiro - 5, height, empfaenger.getPostAnschrift(true),
				SWT.LEFT);
		}
		...
		p.insertTextAt(xRef + baseX, yGarant2 + yOff, wRef, 25, schuldner.getPostAnschrift(true),
			SWT.LEFT);

...
public class ICDImporter extends ImporterPage {
	...
	public IStatus doImport(IProgressMonitor monitor) throws Exception {
		...
		InputStreamReader ir = new InputStreamReader(
				new FileInputStream(codes), "iso-8859-1"); //$NON-NLS-1$
		...
		while ((in = br.readLine()) != null) {
			...
			if (kapitel == null) { // Neues Kapitel anlegen
				...
				LineFeeder lf = new LineFeeder(results[0] + File.separator
						+ "KAP" + kap + ".asc"); //$NON-NLS-1$ //$NON-NLS-2$
				while (lf.peek() == '0') {
					String t = lf.nextLine();
					switch (t.charAt(1)) {
					case 'T':
						kapitel.set(
								"Text", t.substring(2).replaceAll("\n", " - "));break; //$NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$
					case 'I':
			...
			if (mygroup == null) {
				...
				if (fields[0].equals("3")) { //$NON-NLS-1$
					...
					if (icdSuper == null) {
						node = new ICD10(mygroup.getId(), fields[ICD10.CODE],
								in);
					} else {
						node = new ICD10(icdSuper.getId(), fields[ICD10.CODE],
								in);
					}
				}
			}

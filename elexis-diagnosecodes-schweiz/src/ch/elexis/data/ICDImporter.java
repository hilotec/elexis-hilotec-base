...
/**
 * Der ICDImporter importiert den ICD-Code, wie er beispielsweise von der WHO bereitgestellt wird.  Die
 * "EDV-Fassung" kann z.B. hier bezogen werden:
 * http://www.dimdi.de/dynamic/de/klassi/downloadcenter/icd-10-who/version2006/systematik
 * Sie benötigen die ICD-10-WHO 2006 Systematik EDV-Fassung ASCII (x1vea2006.zip)
 * und  die ICD-10-WHO 2006 Systematik Metadaten ASCII (x1vma2006.zip)
 * Entpacken Sie beide in dieselbe Ebene desselben Verzeichnisses. (Die "liesmich.txt"-Datei wird dabei
 * überschrieben, da sie in beiden Archiven vorhanden ist. Sie können die entsprechende Warung ignorieren)
 * 
 * Dieser Importer liest aus den Metadaten "codes.txt" die einzelnen ICD-Ziffern zeilenweise ein und baut die
 * kompletten Codes mithilfe der KAPxx-.asc - Kapiteldateien aus der Systematik auf.
 * 
 * Aufbau von codes.txt:
 *                  Feld 1 : Klassifikationsebene, 1 Zeichen
                             3 = Dreisteller
                             4 = Viersteller
                             5 = Fünfsteller
                    Feld 2 : Ort der Schlüsselnummer im Klassifikationsbaum,
                             1 Zeichen
                             T = terminale Schlüsselnummer (kodierbarer
                                 Endpunkt)
                             N = nichtterminale Schlüsselnummer (kein
                                 kodierbarer Endpunkt)
                    Feld 3 : Generiert?
                             X: explizit in der Klassifikation aufgeführt
                             S: über Subklassifikation generiert
                    Feld 4 : Art der Viersteller
                             X = explizit aufgeführt (präkombiniert)
                             S = per Subklassifikation (postkomibiniert)
                    Feld 5 : Kapitelnummer, 2 Zeichen
                    Feld 6 : erster Dreisteller der Gruppe, 3 Zeichen
                    Feld 7 : Dreisteller zur Schlüsselnummer, 3 Zeichen
                    Feld 8 : Schlüsselnummer ohne eventuelles Kreuzchen, bis
                             zu 7 Zeichen
                    Feld 9 : Schlüsselnummer ohne Strich, Sternchen und 
                             Ausrufezeichen, bis zu 6 Zeichen
                    Feld 10: Schlüsselnummer ohne Punkt, Strich, Sternchen und
                             Ausrufezeichen, bis zu 5 Zeichen
                    Feld 11: Klassentitel, bis zu 255 Zeichen
                    Feld 12: Bezug zur Mortalitätsliste 1
                    Feld 13: Bezug zur Mortalitätsliste 2
                    Feld 14: Bezug zur Mortalitätsliste 3
                    Feld 15: Bezug zur Mortalitätsliste 4
                    Feld 16: Bezug zur Morbiditätsliste
                    Feld 17: Geschlechtsbezug der Schlüsselnummer
                             9 = kein Geschlechtsbezug
                             M = maennlich
                             W = weiblich
                    Feld 18: Art des Fehlers bei Geschlechtsbezug
                             9 = irrelevant
                             M = Muß-Fehler
                             K = Kann-Fehler
                    Feld 19: untere Altersgrenze für eine Schlüsselnummer
                             999     = irrelevant
                             000     = bis unter 1 Tag
                             ...
                             300-324 = 100 Jahre bis unter 124 Jahre
                    Feld 20: obere Altersgrenze für eine Schlüsselnummer
                             wie bei Feld 19
                    Feld 21: Art des Fehlers bei Altersbezug
                             9 = irrelevant
                             M = Muß-Fehler
                             K = Kann-Fehler
                    Feld 22: Krankheit in Mitteleuropa sehr selten?
                             J = Ja (--> Kann-Fehler auslösen!)
                             N = Nein
                    Feld 23: Schlüsselnummer als Grundleiden in der
                             Todesursachenkodierung zugelassen?
                             J = Ja
                             M = Nein 
...
public class ICDImporter extends ImporterPage {
	...
	public IStatus doImport(IProgressMonitor monitor) throws Exception {
		...
		while((in=br.readLine())!=null){
			...
			if(kapitel==null){						// Neues Kapitel anlegen
				kapitel=new ICD10("NIL",kap,"1;N;X;X;"+kap+";"+kap+";"+kap+";"+kap+";"+kap+";"+kap+";Kapitel "+kap+";;;;;"); //$NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$ //$NON-NLS-4$ //$NON-NLS-5$ //$NON-NLS-6$ //$NON-NLS-7$ //$NON-NLS-8$ //$NON-NLS-9$
					...
					case 'G': 
						...
						while(match.find()==true){
							...
							if(groups.get(gs[0])==null){
								ICD10 Group=new ICD10(kapitel.getId(),code,"1;N;X;X;"+kap+";"+code+";"+gs[0]+";"+gs[0]+";"+code+";"+code+";"+match.group(2)); //$NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$ //$NON-NLS-4$ //$NON-NLS-5$ //$NON-NLS-6$ //$NON-NLS-7$
		...
		GlobalEvents.getInstance().fireUpdateEvent(ICD10.class);
	...
	class LineFeeder{
		...
		LineFeeder(String fname) throws Exception{
			br=new BufferedReader(new InputStreamReader(new FileInputStream(fname),"iso-8859-1")); //$NON-NLS-1$
			prev=br.readLine();
		}
		char peek(){

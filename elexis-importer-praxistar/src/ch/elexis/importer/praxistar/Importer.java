...
public class Importer extends ImporterPage {
	...
	private void importMandanten(final IProgressMonitor moni) throws Exception{
		...
		while ((res != null) && res.next()) {
			...
			an.set(new String[] {
				...
				.get("dt_Geburtsdatum").split(" ")[0]).toString(TimeTool.DATE_GER), row.get(
				"Geschlecht_ID").equals("1") ? "m" : "w", row.get("tx_Strasse"), row.get("tx_PLZ"),
				row.get("tx_Ort"));
		...
		while ((res != null) && res.next()) {
			...
			if (an != null) {
				...
				}, row.get("Telefon_1"), row.get("Telefon_2"), row.get("Telefon_N"), row
					.get("Telefax"));
	...
	private void importGaranten(final IProgressMonitor moni) throws Exception{
		...
		while ((res != null) && res.next()) {
			...
			}, StringTool.unNull(res.getString("tx_Strasse")), StringTool.unNull(res
				.getString("tx_PLZ")), StringTool.unNull(res.getString("tx_Ort")), StringTool
				.unNull(res.getString("tx_Telefon")), StringTool.unNull(res.getString("tx_Fax")));
			moni.subTask(name);
	...
	private void importPatienten(final IProgressMonitor moni) throws Exception{
		...
		while ((res != null) && res.next()) {
			...
			}, row.get("tx_Strasse"), StringTool.limitLength(land, 3), StringTool.limitLength(plz,
				5), row.get("tx_Ort"), row.get("tx_TelefonP"), row.get("tx_TelefonG"), row
				.get("tx_TelefonN"), row.get("tx_Titel"));
			StringBuilder sb = new StringBuilder();
			...
			if (!StringTool.isNothing(kkid)) {
				...
				if ((kk != null) && kk.isValid()) {
					...
					fall.setRequiredString(TarmedRequirements.INSURANCE_NUMBER, row
						.get("tx_KK_MitgliedNr"));
				}
			...
			if (!StringTool.isNothing(uvgid)) {
				...
				if ((uvg != null) && uvg.isValid()) {
					...
					fall.setRequiredString(TarmedRequirements.ACCIDENT_NUMBER, row
						.get("tx_UVG_MitgliedNr"));
				}
			}
			moni.worked(PORTION);
		}

...
public class RnPrintView extends ViewPart {
	...
	public Result<Rechnung> doPrint(final Rechnung rn){
		...
		for (Konsultation k : kons) {
			...
			header.append("Anzahl").append("\t")
				.append("MWSt.").append("\t").append("Einzelpreis").append("\t").append("Betrag")
				.append("\n\n");
			pos =
				tc.getPlugin().insertText(pos, header.toString(), SWT.LEFT);
			...
			for (Verrechnet vv : k.getLeistungen()) {
				...
				sb.append(zahl).append("\t").append(
					getVatRate(vv, subtotal, vatSummer)).append("\t").append(preis.getAmountAsString())
					.append("\t").append(subtotal.getAmountAsString())
					.append("\n");
	...
	/**
	 * Get the correct VAT value based on the Verrechent and the info if
